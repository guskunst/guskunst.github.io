...
sap.ui.define([ "testUtils/sinonEnhanced",
                 ...
                 ], function(sinon, testableHelper, IappStateHandler, SelectionVariant){
	"use strict";
	...
	var oController = {
		...
		restoreCustomAppStateDataExtension: jQuery.noop,
		modifyStartupExtension: jQuery.noop
	};
...
//		backendDefaultValues: not provided in interface, but already applied to SFB, but only if no userDefaultVariant exists
		if (!oUserDefaultVariant || jQuery.isEmptyObject(oUserDefaultVariant)){}
...
//		iAppstate
		if (oIAppState && !jQuery.isEmptyObject(oIAppState)){
...
//		navigationParameters: a) directly in URL
		if (oNavigationParameterURL && !jQuery.isEmptyObject(oNavigationParameterURL)){
...
//		navigationParamsters b) in xAppState
		if (oNavigationParameterXAppState && !jQuery.isEmptyObject(oNavigationParameterXAppState)){
...
//		userDefaultValues
		if (oUserDefaultValues && !jQuery.isEmptyObject(oUserDefaultValues)){
			sNavType = sap.ui.generic.app.navigation.service.NavType.xAppState;
...
//							(oUserDefaultVariant ? jQuery.extend({}, oUserDefaultVariant, oNavigationParameterURL, oNavigationParameterXAppState) :
//								jQuery.extend({}, oBackendDefaultValues, oUserDefaultValues, oNavigationParameterURL, oNavigationParameterXAppState)) :
//								(oUserDefaultVariant ? oUserDefaultVariant : jQuery.extend({}, oBackendDefaultValues, oUserDefaultValues)) );
				var oExpectedValues =
					oIAppState ||
					((oNavigationParameterURL || oNavigationParameterXAppState) ?
							(oUserDefaultVariant ? jQuery.extend({}, oNavigationParameterURL, oNavigationParameterXAppState) :
								jQuery.extend({}, oUserDefaultValues, oNavigationParameterURL, oNavigationParameterXAppState)) :
