...
//  typical case on (main) ListReport
		...
		var oIsNewHistoryEntryRequiredStub = sandbox.stub(oServices.oApplication, "isNewHistoryEntryRequired");
		oIsNewHistoryEntryRequiredStub.returns(true);
		...
		assert.equal(bReplace, false, "No replace in history");
...
// table in object page
		...
		oIsNewHistoryEntryRequiredStub.returns(false);
		oCommonUtils.navigateFromListItem(oContext, oTable);
		assert.equal(oNavigationContext, oContext, "Navigate to context as given");
		assert.equal(sNavigationProperty, sNavProp, "Navigation property according to binding info of given table");
		assert.equal(bReplace, true, "replace in history");
	});

	QUnit.test("navigatefromlistitem with new interface (context + table) and forwardNaviagtionProperty", function(assert){
		// typical case on (main) ListReport with forwardNaviagtionProperty
		sPath = "/MainObject(objectkey)";
		var sNavProp = "/to_navigation";
		var getForwardNavigationProperty = function() {
			return sNavProp;
		};
		var oIsNewHistoryEntryRequiredStub = sandbox.stub(oServices.oApplication, "isNewHistoryEntryRequired");
		
		oServices.oApplication.getForwardNavigationProperty = getForwardNavigationProperty;
		
		var oTargetContext = {};
		
		var oListItemContext = {
				getPath: function(){
					return "/SubItem(subitemkey)"; // selected path
				},
				getModel: function(){
					return {
						createBindingContext: function(sPath, oContext, mParameters, fnCallBack) {
							assert.strictEqual(sPath, "/to_navigation", "BindingContext must be created with path given by the navigation property");
							assert.strictEqual(oContext, oListItemContext, "oContext from ListItem must be given as parameter value");
							fnCallBack(oTargetContext);
						}
					};
				}
		};
		...
		oIsNewHistoryEntryRequiredStub.returns(false);
		oCommonUtils.navigateFromListItem(oListItemContext, oTable);
		assert.equal(bReplace, true, "replace in history");
	});
	...
	QUnit.test("navigateToContext", function(assert) {
		testableHelper.observeConstructor(Context, jQuery.noop, true);
	...
	QUnit.test("Non-Draft case with changes - user confirms - no Promise from App provided", function(assert) {
		bIsDraftEnabled = false;
		var oModel = {
				hasPendingChanges: jQuery.noop,
				resetChanges: jQuery.noop
		};
		sandbox.stub(oController, "getView", function() {
			return {
				getModel: function() {
	...
	.test(
			...
			function(assert) {
				...
				sandbox.stub(oBusyHelper, "setBusy", function(oPromise) {
					oPromise.then(jQuery.noop, function() {
