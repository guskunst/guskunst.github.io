...
function(sinon, MoveElements, ChangeHandlerUtils, AnnotationChangeHandler, MoveControls) {
	...
	QUnit.test("MoveElements test", function(assert) {
	...
	QUnit.test("Utils.isRevert causes function to terminate", function(assert) {
		//Arrange
		ChangeHandlerUtils.isRevert = true;
		this.oCreateCustomChangesStub.returns({ testValue: "ok" });
		...
		//Assert
		assert.equal(this.oChange.getContent().testValue, undefined, "Function has been terminated.");
	});
});