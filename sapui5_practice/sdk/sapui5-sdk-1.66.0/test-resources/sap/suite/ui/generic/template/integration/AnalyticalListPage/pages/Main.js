	...
	Opa5.createPageObjects({
		onTheMainPage: {
			...
			actions: {
				...
				iClickTheKPI: function (position) {
					return this.waitFor({
						controlType: "sap.m.GenericTag",
						timeout: 45,
						success: function (kpis) {
							kpis[position].$().trigger("click");
						},
			...
			assertions: {
				...
				iCheckNumberofFractionalDigit: function (nId, decimal) {
					return this.waitFor({
						controlType: "sap.m.GenericTag",
						success: function (kpis) {
							QUnit.ok(kpis[nId].getValue() != undefined &&
								(kpis[nId].kpiSettings.dataPoint.ValueFormat.NumberOfFractionalDigits.Int == decimal));
						},
						errorMessage: "The KPI are not being displayed"
					});
				},

				iCheckKpiScaleFactor: function (nId, scale) {
					return this.waitFor({
						controlType: "sap.m.GenericTag",
						success: function (kpis) {
							QUnit.ok(kpis[nId].getValue() != undefined && (kpis[nId].getValue().getNumber().indexOf(scale) > 0));
						},
						errorMessage: "The KPI are not being displayed"
					});
				},

				iCheckKpiValue: function (nId, value) {
					return this.waitFor({
						controlType: "sap.m.GenericTag",
						success: function (kpis) {
							QUnit.ok(kpis[nId].getValue().getNumber() === value, "The KPI has proper values");
						},
						errorMessage: "The KPI are not being displayed"
					});
				},
				...
				checkTableRowCount: function (expectedTotalCount) {
					return this.waitFor({
					...
					return this.waitFor({
						...
						check: function (aContainer) {
							...
							for (var i = 0; i < headerContents.length; i++) {
								...
								if (oVBox.getWidth() === oVBox.getItems()[0].$().css("width") &&
									oVBox.getWidth() === oVBox.getItems()[2].$().css("width")) {
									bSuccess = true;
								} else {
									bSuccess = false;
									break;
				...
				isTheDynamicHeaderRenderingGlobalActions: function () {
					return this.waitFor({
						...
						success: function (aControl) {
							...
							var aButton = jQuery.grep(titleBar, function (oControl) {
								return oControl.getMetadata().getName() === "sap.m.Button";
							});
							var mProductBreakoutActions = OpaManifest.demokit["sample.analytical.list.page.ext"].getProperty("/sap.ui5/extends/extensions/sap.ui.controllerExtensions/sap.suite.ui.generic.template.AnalyticalListPage.view.AnalyticalListPage/sap.ui.generic.app/ZCOSTCENTERCOSTSQUERY0020/Actions/");
							var oi18nResourceBundle = OpaResourceBundle.demokit["sample.analytical.list.page.ext"].i18n;
				...
				isTheFooterBarHasDeterminingButtonsCorrectly: function () {
					return this.waitFor({
						...
						success: function (aControl) {
							...
							var aButton = jQuery.grep(footerBar, function (oControl) {
								return oControl.getMetadata().getName() === "sap.m.Button";
							});
				...
				checkForRequiresSelectionButtons: function (oControl, oControlName, expectedResult) {
					...
					var iButtonIndex = 0;
					var aButton;
					aButton = jQuery.grep(oControl[0].getContent(), function (oControl) {
						return oControl.getMetadata().getName() === "sap.m.Button";
					});
				...
				theCustomToolbarForTheSmartTableIsRenderedWithRequireSelectionCorrectly: function (expectedResult) {
					return this.waitFor({
						...
						success: function (oControl) {
							this.checkForRequiresSelectionButtons(oControl, "table", expectedResult);
						},
				...
				isSegmentedButtonNotPresentInToolbar: function (oControlName) {
					...
					return this.waitFor({
						controlType: "sap.m.OverflowToolbar",
						success: function (aControl) {
							var aButton = jQuery.grep(aControl, function (oControl) {
