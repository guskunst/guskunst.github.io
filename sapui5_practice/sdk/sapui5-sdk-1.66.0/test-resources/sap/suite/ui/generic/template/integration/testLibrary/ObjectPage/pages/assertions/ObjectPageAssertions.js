					...
					return this.waitFor({
						...
						success: function() {
							...
							for (var i=0; i<=iLength-1; i++) {
								if (!!iLength) {
						        	bMatch= !!(document.getElementById("OpaFrame").contentWindow.document.getElementsByClassName("sapMMessageToast")[i].innerText == sExpectedText);
						        }
							}
							Opa5.assert.equal(true,bMatch,"Correct Toast is seen");
						},
						errorMessage: "Either the toast or the toast message is wrong"
					});
				},
