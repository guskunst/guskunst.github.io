...
sap.ui.define([
	...
	function(sinon, ChangeHandlerUtils, DesigntimeUtils) {
		"use strict";
		...
		QUnit.test("ignoreAllProperties", function() {
			...
			// Assert
			assert.ok(jQuery.isEmptyObject(oProperties), "reacts properly on empty property list");
		});

		QUnit.test("ignoreAllProperties", function() {
			// Arrange
			this.allProperties = {
				firstProperty: { someAttribute: "someValue" }
			};
			...
			// Assert
			assert.deepEqual(oProperties, {firstProperty: {ignore: true}}, "sets ignore for a single property");
		});
		...
		QUnit.test("ignoreAllProperties", function() {
			...
			this.allProperties = {
				firstProperty: { someAttribute: "someValue" },
				secondProperty: { someAttribute: "someValue" }
			};
		...
		QUnit.test("modifyDataPointForChart", function() {
			// Arrange
			var aCustomChanges = [],
				oMeasure = {
					Measure: {
				...
				oDataEntityTypeStub = {
					...
					"com.sap.vocabularies.UI.v1.DataPoint#Product": {
						TargetValue:  {Path: "Height", EdmType: "Edm.Decimal"},
						Title: {String: "Weight"},
						Value: {Path: "Weight", EdmType: "Edm.Decimal"}
					}
				};
			...
			assert.deepEqual(aCustomChanges, aExpectedChanges, "does not return any change if there is no annotationPath specified");
		});
		...
		QUnit.test("modifyDataPointForChart", function() {
			...
			assert.deepEqual(aCustomChanges, aExpectedChanges, "returns actualized DataPoint");
		});
			...
			var sQualifier = "MyQualifier",
				oExpected = {
					...
					"MeasureAttributes": [
						{
							"DataPoint": {
								"AnnotationPath": "@com.sap.vocabularies.UI.v1.DataPoint" + sQualifier
							},
							"Measure": {
								"PropertyPath": ""
							},
		...
		QUnit.test("fixQualifierForAnnotationPath", function() {
			...
			// Assert
			assert.equal(sTermWithQualifier, "Term#Rating", "returns the right term with qualifier if a basic qualifier is specified and no annotation records exist");
		});
