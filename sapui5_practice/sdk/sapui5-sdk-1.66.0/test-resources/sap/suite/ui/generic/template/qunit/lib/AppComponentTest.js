...
sap.ui.define(["testUtils/sinonEnhanced", "sap/ui/generic/app/ApplicationController", "sap/suite/ui/generic/template/lib/AppComponent", "sap/suite/ui/generic/template/lib/CRUDHelper", "sap/suite/ui/generic/template/lib/testableHelper"], 
	function(sinon, ApplicationController, AppComponent, CRUDHelper, testableHelper) {
	"use strict";
	...
	function fnGeneralSetup(){
			...
			oSandbox.stub(CRUDHelper, "enableAutomaticDraftSaving", jQuery.noop);
	...
	QUnit.test("Test that createContent works correct in the 'real' scenario", function(assert) {
		...
		setTimeout(function(){
			assert.ok(oBusyHelper.setBusyReason.calledTwice, "setBusyReason was called");
			var aFirstCallArgs = oBusyHelper.setBusyReason.args[0];
			assert.ok(aFirstCallArgs[1], "First set busy");
			assert.ok(aFirstCallArgs[2], "Set busy immediately");
			assert.ok(oBusyHelper.setBusyReason.secondCall.calledWithExactly(aFirstCallArgs[0], false), "Busy must have been reset");
			done();
		});
	});
	...
	var sManifestUrl110 = "test-resources/sap/suite/ui/generic/template/qunit/lib/testdata/manifest_v110.json",
		...
		oManifestMultipleViewsDifferentEntitySets = jQuery.sap.syncGetJSON(sManifestUrlMultipleViewsDifferentEntitySets).data;
	...
	QUnit.test("Test getConfig transformation from app.descriptor 1.3.0 to 1.1.0 with getConfig", function(assert) {
		...
		oConfig = oAppComponent.getConfig();
	...
	QUnit.test("Test getConfig transformation from app.descriptor 1.3.0 to 1.1.0 with getInternalManifest", function(assert) {
		...
		//Simulate a read only property for "sap.ui.generic.app"
		delete oManifest130["sap.ui.generic.app"];
		Object.defineProperty(oManifest130, "sap.ui.generic.app", {
			value: oConfig,
			writable: false
		});
		...
		try {
			oManifest130["sap.ui.generic.app"] = null;
		} catch (e) {
			bIsReadOnly = true;
		}
		assert.ok(bIsReadOnly, "namespace sap.ui.generic.app set to read-only");
		...
		oManifest = oAppComponent.getInternalManifest();
		//Fix version so it looks completely as 1.1.0 for the runtime
		oManifest["sap.ui.generic.app"]._version = "1.1.0";
		assert.deepEqual(oManifest, oManifest110,
			"App. descriptor namespace 'sap.ui.generic.app' version 1.3.0 must match version 1.1.0 when returned by getInternalManifest");
	});
