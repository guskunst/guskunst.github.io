sap.ui.define([
	...
	function getImagePath(sFileName){
		return sFileName
			? getAbsolutePath(getDemokitPath(sFileName) || jQuery.sap.getModulePath("sap.ui.demoapps.rta.fiorielements/") + '/localService/img/' + sFileName)
			: sFileName
	}
				...
				if (Object.prototype.hasOwnProperty.call(oDataSources, property)) {
					...
					if (dataSource.settings && dataSource.settings.localUri) {
						if (typeof dataSource.type === "undefined" || dataSource.type === "OData") {
							...
							sMetadataUrl = jQuery.sap.getModulePath(_sAppModulePath + dataSource.settings.localUri.replace(".xml", ""), ".xml");
