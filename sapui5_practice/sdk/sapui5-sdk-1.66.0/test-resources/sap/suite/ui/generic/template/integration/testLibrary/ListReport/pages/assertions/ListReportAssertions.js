	...
	function(Opa5, jQuery, BaseObject, PropertyStrictEquals, AggregationFilled, Press, ApplicationSettings, Common) {
		return function (sViewNameListReport, sViewNamespaceListReport) {
			return {
				...
				* @param {int} iTabIndex The index of the table tab in case of multiple views in the ListReport. Just leave empty if no tabs are provided.
				* @throws {Error} Throws an error if the wrong number of items are loaded or if the table could not be found
				* @public
				*/
				theResultListContainsTheCorrectNumberOfItems: function(iItems, iTabIndex) {
					var aMatchers = [
				...
				iWaitForResponsiveTableInListReport: function(aMatchers, fnSuccess, iTabIndex) {
					...
					var sTable = (iTabIndex > 0) ? oAppParams.LRPrefixID + "--responsiveTable-" + iTabIndex : oAppParams.LRPrefixID + "--responsiveTable";
					return this.waitFor({
						id: sTable,
				...
				* @public
				*/
				theResultListFieldHasTheCorrectValue: function (oItem, iTabIndex) {
					var oAppParams = ApplicationSettings.getAppParameters();
					var sId = ((iTabIndex != undefined) && (iTabIndex > 0)) ? (oAppParams.LRPrefixID + "--responsiveTable" + "-" + iTabIndex) : (oAppParams.LRPrefixID + "--responsiveTable");
					return this.waitFor({
						id: sId,
						viewName: sViewNameListReport,
						viewNamespace: sViewNamespaceListReport,
				...
				*/
				theResultListFieldHasTheCorrectObjectMarkerEditingStatus: function (oInput) {
					var oAppParams = ApplicationSettings.getAppParameters();
					return this.waitFor({
//						id: oAppParams.LRPrefixID + "--responsiveTable", // not working for table-tabs
