sap.ui.define([
	...
	QUnit.test("Get status text for screen reader support", function (assert) {
		...
		assert.deepEqual(testText, this.processFlowLaneHeader2._oResBundle.getText('PF_ARIA_STATUS') + " 25% Positive, 60% Negative, 15% Critical", "Text was generated correctly with 3 status values");

		//Assertion for test 4 values case.
		testText = this.processFlowLaneHeader3._getAriaText();
		assert.deepEqual(testText, this.processFlowLaneHeader3._oResBundle.getText('PF_ARIA_STATUS') + " 5% Positive, 50% Negative, 20% Neutral, 25% Critical",
		...
		assert.deepEqual(testText, this.processFlowLaneHeader4._oResBundle.getText('PF_ARIA_STATUS') + " 3% Positive, 3% Positive, 70% Negative, 20% Planned, 5% Critical",
