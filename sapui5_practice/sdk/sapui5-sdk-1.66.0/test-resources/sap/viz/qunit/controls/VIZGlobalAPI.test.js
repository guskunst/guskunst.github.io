...
QUnit.test("a1a2m1m1 Viz Bar", function(assert){
  assert.ok(true, "a1a2m1m1 Viz Bar chart.");
...
QUnit.test("a1a2m1m1 VizFrame Bar", function(assert){
	    ...
	    oVizFrame.attachEventOnce('renderComplete', function(){
	        ...
	        assert.equal($('#vizframeChart .v-m-title text').text(), "Title of Chart", 'Title Default language is en.');
	    	assert.equal($('#vizframeChart .v-m-legend .v-title').text(), "YEAR / All Measures", 'All measures Default language is en.');
	        done();
	    });
});

QUnit.test("Change language to de", function(assert){
    var done = assert.async();
    sap.ui.getCore().getConfiguration().setLanguage("de_DE");
    assert.ok(true, "a1a2m1m1 Bar chart.");
    
    var checkLanguage = function() {
        assert.equal($('#vizChart .v-m-title text').text(), "Diagrammtitel", 'Title Default language is de.');
          assert.equal($('#vizChart .v-m-legend .v-title').text(), "Alle Kennzahlen - YEAR", 'All Measures Default language is de.');
         ...
         assert.equal($('#vizframeChart .v-m-legend .v-title').text(), "YEAR / Alle Kennzahlen", 'All Measures Default language is de.');
...
if (!(Device.os.macintosh && Device.browser.chrome)){
	QUnit.test("Change language to en", function(assert){
      var done = assert.async();
      ...
      var checkLanguage = function() {
          ...
          if ($('#vizChart .v-m-legend .v-title title').length > 0) {
              assert.equal($('#vizChart .v-m-legend .v-title title').text(), "All Measures - YEAR", 'All Measures Default language is en.');
          } else {
              assert.equal($('#vizChart .v-m-legend .v-title').text(), "All Measures - YEAR", 'All Measures Default language is en.');
          }
