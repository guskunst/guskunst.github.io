...
function testSetViz(applicationSet, assert) {
    ...
    vizFrame.attachEvent('renderComplete', function() {
        if (step === 0) {
            ...
            assert.equal(valueAxisColor, "sapUiChartPaletteSequentialHue1Dark1", "valueAxis color check");
            ...
            var valueAxisColor = vizFrame.getVizProperties().valueAxis.color;
            assert.equal(valueAxisColor, "#ffffff" , "valueAxis color check");

            var chartTitleColor = vizFrame.getVizProperties().title.style.color;
            assert.equal(chartTitleColor, "#ffffff", "high contrast theme applied");
