sap.ui.define([
...
], function (jQuery, sinon, testableHelper, mockServerHelper) {
	"use strict";

	/* Borrow definitions from DraftModel to avoid code duplications */
	var oStaticStub = testableHelper.startTest() && testableHelper.getStaticStub();

	function filterEditState(sEditState, aData) {
		switch (sEditState) {
			case oStaticStub.EDITSTATE.ALL:
				return aData;
			case oStaticStub.EDITSTATE.UNCHANGED:
				return aData.filter(function (oEntry) {
					return oEntry.IsActiveEntity && !oEntry.HasDraftEntity;
				});
			case oStaticStub.EDITSTATE.OWN_DRAFT:
				return aData.filter(function (oEntry) {
					return !oEntry.IsActiveEntity;
				});
			case oStaticStub.EDITSTATE.LOCKED:
				return aData.filter(function (oEntry) {
					return oEntry.IsActiveEntity && oEntry.DraftAdministrativeData && oEntry.DraftAdministrativeData.InProcessByUser !== '';
				});
			case oStaticStub.EDITSTATE.UNSAVED_CHANGES:
				return aData.filter(function (oEntry) {
					return oEntry.IsActiveEntity && oEntry.DraftAdministrativeData && oEntry.DraftAdministrativeData.InProcessByUser === '';
				});
			default:
	...
	function logMockedRequests(url) {
		...
		jQuery.sap.log.info("sap.fe mock-served url: " + aParts[0], aParts[1] && ("\n\t" + aParts[1].replace(/[\&]/g, "\n\t")), "sap.fe.MockServer");
	}
	...
	function fnGetResponseData(sUrl, oApp) {
		...
		for (var i = 0; i < oApp.mockDataSets.length; i++) {
			if (sUrl.indexOf(oApp.mockDataSets[i]) > -1) {
				var oFilteredData = jQuery.extend({}, JSON.parse(oApp.mockData[oApp.mockDataSets[i]])),
					oParams = jQuery.sap.getUriParameters(sUrl), sKey;
				//filter
				if (sUrl.indexOf("$filter") > -1) {
					...
					aEditState = Object.keys(oStaticStub.EDITSTATE);
					for (var i = 0; i < aEditState.length; i++) {
						sFilter = oStaticStub.getFilterForEditState(i.toString());
					...
					if (sEditStateKey) {
						sEditState = oStaticStub.EDITSTATE[sEditStateKey];
					}
					sFilter = oStaticStub.getFilterForEditState(sEditState);
				...
				if (sUrl.indexOf("$search") > -1) {
					var sSearchString = oParams.get("$search"),
						...
						sEntityType, aSearchableProperties;
		...
		Object.keys(aAppProcessedParams).forEach(function (sApp) {
			...
			fServer.respondWith('GET', RegExp(oApp.serviceRegex, 'i'), function (xhr, id) {
				...
				if (xhr.url.indexOf("metadata") > -1) {
					...
					return xhr.respond(200, {"Content-Type": "application/xml", "ETag": oApp.metadataETag, "Last-Modified": oApp.metadataLastModified}, oApp.metadata);
				} else {
					//Getting the JSON data
					var oFilteredData = fnGetResponseData(xhr.url, oApp);
					//Trigger the final response
					return xhr.respond(200, {"Content-Type": "application/json", "OData-Version": "4.0"}, JSON.stringify(oFilteredData));
				}
			});
					...
					if (!!sBoundary) {
						...
						aBatchRequests.forEach(function (sBatchRequest, index) {
							...
							if (rGet.exec(sBatchRequest)) {
								...
								sRespondData = sRespondData + "\r\nContent-Type: application/http\r\n" + "Content-Length: " + sResponseString.length + "\r\n" + "content-transfer-encoding: binary\r\n\r\n" + sResponseString + "--ejjeeffe0";
							}
						});
						sRespondData += "--";
						//Trigger the final response
						return xhr.respond(200, {"Content-Type": "multipart/mixed; boundary=ejjeeffe0", "OData-Version": "4.0"}, sRespondData);
					}
				}
			});
		});
		...
		var oPreProcessingPromise = new Promise(function (resolve, reject) {
			...
			Object.keys(oApps).forEach(function (sApp) {
				...
				if (isAppWithMockData(oApp)) {
					...
					aManifestPromises.push(jQuery.get(sProject + "/manifest.json").then(function (data, status, jqXHR) {
						...
						if (oDataSources.hasOwnProperty(property)) {
							...
							if (oDataSource.settings && oDataSource.settings.localUri) {
								if (property === "mainService") {
									aAppPreProcessingParams[sApp].localUri = oDataSource.settings.localUri.slice(0, oDataSource.settings.localUri.indexOf("metadata") - 1);
									aDataSourceMetadataPromises.push(jQuery.get(aAppPreProcessingParams[sApp].url + "/" + oDataSource.settings.localUri).then(function (data, status, jqXHR) { // eslint-disable-line no-loop-func
										aAppPreProcessingParams[sApp].metadata = oXmlSerializer.serializeToString(data);
										aAppPreProcessingParams[sApp].metadataETag = jqXHR.getResponseHeader('etag');
										aAppPreProcessingParams[sApp].metadataLastModified = jqXHR.getResponseHeader('last-modified');
										...
										aAppPreProcessingParams[sApp].entitySetToTypeMap = [];
										aEntitySets.each(function () {
											aAppPreProcessingParams[sApp].entitySetToTypeMap[this.getAttribute("Name")] = this.getAttribute("EntityType").match(/[a-zA-Z0-9\_]+$/g)[0];
										});
										return Promise.resolve(data);
									}));
								}
				...
				Promise.all(aDataSourceMetadataPromises).then(function (aMetadata) {
					...
					Object.keys(aAppPreProcessingParams).forEach(function (sApp) {
						...
						oApp.mockDataSets.forEach(function (sKey) {
							aDataPromises.push(jQuery.get(oApp.url + "/" + oApp.localUri + "/" + sKey + ".json").then(function (data, status, jqXHR) {// eslint-disable-line no-loop-func
					...
					Object.keys(aExtraPaths).forEach(function (sApp) {
						...
						Object.keys(oAppPaths).forEach(function (sKey) {
							aDataPromises.push(jQuery.get(oAppPaths[sKey]).then(function (data, status, jqXHR) {// eslint-disable-line no-loop-func
								...
								var oMetadata = jQuery.parseXML(oApp.extraData[sKey]),
								...
								aEntitySets.each(function () {
									oApp.entitySetToTypeMap[sKey][this.getAttribute("Name")] = this.getAttribute("EntityType").match(/[a-zA-Z0-9\_]+$/g)[0];
								});
