sap.ui.require(['jquery.sap.global',
	...
	function (jQuery, OperatorBuilder, OperatorProvider, AstExpressionLanguage, Stack, FunctionBuilder, FunctionProvider, TermsBuilder,
		...
		QUnit.test("Autocomplete Literal simple expressions", function (assert) {
			...
			var suggestions = _oExpressionLanguage.getSuggesstions(tokens, false);
			...
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, false);
		...
		QUnit.test("Autocomplete Sugeestions expressions", function (assert) {
			...
			tokens = getTokens(inputString);
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			...
			inputString = "./40ff80b379e64fc1a6e2cc2357f2b9e9/2dfa645c94d64670adbea943c6353860";
			tokens = getTokens(inputString);
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			...
			inputString = "'raghu' = ./40ff80b379e64fc1a6e2cc2357f2b9e9";
			tokens = getTokens(inputString);
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			terms = suggestions.autoComplete.categories.terms;
			var hasOnlyStringSuggestions = true;
			var text;
			var term
			for (iterator = 0; iterator < terms.length; iterator++) {
			...
			assert.equal(true, hasOnlyStringSuggestions, "'raghu' = CUSTOMER." + "should have only String attributes");
		...
		QUnit.test("Autocomplete Suggestions to display Operators", function (assert) {
			...
			arithmeticops = suggestions.autoComplete.categories.operators.arithmetic;
			assert.equal(arithmeticops.length, 1, "1 Arithmetic Operators Should be Displayed");
			for (iterator = 0; iterator < arithmeticops.length; iterator++) {
				assert.equal(["+"].includes(arithmeticops[iterator].name), true, "Arithmetic Operator " + arithmeticops[iterator].name +
		...
		QUnit.test("Autocomplete Suggestions for Add", function (assert) {
			...
			tokens = getTokens(inputString);
			var suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
		...
		QUnit.test("Autocomplete Suggestions for is equal", function (assert) {
			...
			inputString =
				"./9e1095dfbb4f4b919f5f24cc459a9687/f238309662694bcaa8942010611d02f4 + ./9e1095dfbb4f4b919f5f24cc459a9687/f238309662694bcaa8942010611d02f4 =  ./9e1095dfbb4f4b919f5f24cc459a9687"; // S = (S)
			tokens = getTokens(inputString);
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			attributes = suggestions.autoComplete.categories.terms;
			assert.equal(attributes.length, 4, "4 Terms  CLASS , CUSTOMID , BOOKID, CONNID Should be displayed ");
			...
			inputString = "./9e1095dfbb4f4b919f5f24cc459a9687/18b1568b83794d84b3ef093cf43b742a = ./9e1095dfbb4f4b919f5f24cc459a9687"; //D = (D)
			tokens = getTokens(inputString);
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			attributes = suggestions.autoComplete.categories.terms;
			assert.equal(attributes.length, 2, "2 Terms  FLDATE , ORDER_DATE Should be displayed ");
			for (iterator = 0; iterator < attributes.length; iterator++) {
				assert.equal(["FLDATE", "ORDER_DATE"].includes(attributes[iterator].name), true, "Term " + attributes[iterator].name +
					" Should be displayed");
			}

			inputString = "'15:02:02' = ./9e1095dfbb4f4b919f5f24cc459a9687"; //T = (T)
			tokens = getTokens(inputString);
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			attributes = suggestions.autoComplete.categories.terms;
			assert.equal(attributes.length, 2, "0 Terms  Should be displayed ");

			inputString =
				"((MAX(./868863c80fb74d7f9ae8c15a96cb66d3,/9156f9eee8ff4316acd314ba2a5f2b93) * MIN(./868863c80fb74d7f9ae8c15a96cb66d3,/9156f9eee8ff4316acd314ba2a5f2b93) + 's')   MATCHES (AVG(./1af007c980e04b55b453dfddcf2cff62, /7ea2962a34da48e2b029671afb4add6e) + 's' )) = ./868863c80fb74d7f9ae8c15a96cb66d3"; //B = (B)
			tokens = getTokens(inputString);
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			attributes = suggestions.autoComplete.categories.terms;
			assert.equal(attributes.length, 1, "1 Terms  DiscountBool Should be displayed ");
			for (iterator = 0; iterator < attributes.length; iterator++) {
				assert.equal(["DiscountBool"].includes(attributes[iterator].name), true, "Term " + attributes[iterator].name +
					" Should be displayed");
			}
		...
		QUnit.test("Autocomplete Suggestions for is greaterthan", function (assert) {
			...
			suggestions = _oExpressionLanguage.getSuggesstions(tokens, true);
			attributes = suggestions.autoComplete.categories.terms;
			assert.equal(attributes.length, 0, "0 Terms  DiscountBool Should be displayed ");
		...
		QUnit.test("Autocomplete Suggestions for  AVG  ", function (assert) {
			...
			for (iterator = 0; iterator < attributes.length; iterator++) {
				assert.equal(["CLASS", "CUSTOMID", "BOOKID", "CONNID", "CARRID","ORDER_DATE","FLDATE"].includes(attributes[iterator].name), true, "Term " + attributes[
					iterator].name + " Should be displayed");
			}
