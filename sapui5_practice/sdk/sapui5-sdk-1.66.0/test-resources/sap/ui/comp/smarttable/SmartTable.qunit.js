...
(function() {
	...
	sap.ui.define([
		...
		QUnit.test("Shall switch export type", function(assert) {
			...
			assert.ok(this.oSmartTable._oUseExportToExcel.isA("sap.m.MenuButton"), "MenuButton for GW export");
		});
		...
		QUnit.test("test UI5 client excel export", function(assert) {
			var iRowCount = 512, sHeader = "TestHeader", oModel = sinon.createStubInstance(ODataModel);
			this.oSmartTable.setModel(oModel);
			this.oSmartTable._getRowCount = function() {
				return iRowCount;
			};
			...
			this.oSmartTable.setUseTablePersonalisation(false);
			this.oSmartTable.setUseVariantManagement(false);
			this.oSmartTable.setHeader(sHeader);
			this.oSmartTable.setUseExportToExcel(true);
			this.oSmartTable.bIsInitialised = true;
			this.oSmartTable._createToolbar();
			this.oSmartTable._createToolbarContent();
			...
			var fOriginalfunction = sap.ui.getCore().getLibraryResourceBundle;
			var fnCore = sinon.stub(sap.ui, "getCore");
			var sLib;
			fnCore.returns({
				loadLibrary: function(sLibrary) {
					sLib = sLibrary;
					return fResolvedPromise();
				},
				getLibraryResourceBundle: fOriginalfunction
			});
			var fnCoreTrackEvent = Element._trackEvent ? sinon.stub(Element, "_trackEvent") : null;

			var sFilePath;
			var oSpreadsheetInstance = {
				build: sinon.stub(),
				destroy: sinon.stub()
			};
			oSpreadsheetInstance.build.returns(Promise.resolve());

			var oSpreadsheet = sinon.stub();
			oSpreadsheet.returns(oSpreadsheetInstance);
			...
			var fOrgRequire = sap.ui.require;
			sap.ui.require = function(sFP, fCallback) {
				if (sFP[0] && sFP[0] === "sap/ui/export/Spreadsheet" && sFP[1] && sFP[1] === "sap/ui/export/ExportUtils") {
					sFilePath = sFP;
					fCallback(oSpreadsheet, oExportUtils);
				} else {
					fOrgRequire(sFP, fCallback);
				}
			};
			...
			assert.ok(oExportUtils.count > 0, "interceptUrl called for UI5Client export");
			assert.ok(fnCore.called, "loadLibrary was called");

			assert.equal(sLib, "sap.ui.export", "Export library has to be loaded");

			assert.equal(sFilePath ? sFilePath.join(" ") : "", "sap/ui/export/Spreadsheet sap/ui/export/ExportUtils", "sap/ui/export/Spreadsheet and sap/ui/export/ExportUtils were required");

			assert.equal(sDownloadUrlType, "json", "DownloadType has to be xlsx");

			assert.ok(oSpreadsheet.calledOnce, "Spreadsheet was instantiated");

			assert.ok(oSpreadsheet.calledWith(sinon.match({
				dataSource: {
					type: "odata",
					dataUrl: sUrl,
					count: iRowCount
				...
				fileName: sHeader
			})), "Spreadsheet was constructed with provided settings");

			assert.ok(oSpreadsheet.calledWith(sinon.match.has("workbook", sinon.match.has("columns", sinon.match(function(aCols) {
				// Test of colums in match/contain the below array
			...
			})))), "Spreadsheet was constructed with matching columns");

			assert.ok(oSpreadsheetInstance.build.calledOnce, "Spreadsheet.build was triggered");

			// Test for TreeTable:
			var sLevel = "Level";
			this.oSmartTable._isTreeTable = true;
			this.oSmartTable._getRowBinding = sinon.stub();
			var oTreeBinding = sinon.createStubInstance(ODataTreeBinding);
			oTreeBinding.getDownloadUrl.returns(sUrl);
			oTreeBinding.getTreeAnnotation.returns(sLevel);
			this.oSmartTable._getRowBinding.returns(oTreeBinding);

			// Mock user action
			this.oSmartTable._oUseExportToExcel.fireDefaultAction();

			assert.ok(oSpreadsheet.calledTwice, "Spreadsheet was re-instantiated");

			assert.ok(oTreeBinding.getDownloadUrl.calledOnce);
			assert.ok(oTreeBinding.getTreeAnnotation.calledOnce);
			assert.ok(oTreeBinding.getTreeAnnotation.calledWith("hierarchy-level-for"));
			...
			assert.ok(oSpreadsheet.calledWith(sinon.match({
				dataSource: {
					...
					dataUrl: sUrl,
					count: undefined
				// count is sent as undefined for TreeTable
				},
				workbook: {
					hierarchyLevel: sLevel
				},
				fileName: sHeader
			})), "Spreadsheet was constructed with provided settings");
			...
			if (fnCoreTrackEvent) {
				fnCoreTrackEvent.restore();
			}
			sap.ui.require = fOrgRequire;
			fnGetColumns.restore();
		});

		QUnit.test("test UI5 client excel export - mobile columns order", function(assert) {
			var iRowCount = 512, sHeader = "TestHeader", oModel = sinon.createStubInstance(ODataModel);
			this.oSmartTable.setModel(oModel);
			this.oSmartTable._getRowCount = function() {
				return iRowCount;
			...
			assert.ok(this.oSmartTable._triggerUI5ClientExport.calledOnce, "this.oSmartTable._triggerUI5ClientExport has to be called for UI5Client export");
			assert.ok(oExportUtils.count > 0, "interceptUrl called for UI5Client export");
			assert.ok(fnCore.calledOnce, "loadLibrary was called");
			fnCore.restore();
			...
			assert.ok(oSpreadsheet.calledWith(sinon.match.has("workbook", sinon.match.has("columns", sinon.match(function(aCols) {
				// Test of colums in match/contain the below array --> columns are ordered as per getOrder!
				return jQuery.sap.equal([
					{
						label: "Prop A",
						property: "PropA",
		...
		QUnit.test("test UI5 client excel export - no visible columns", function(assert) {
			...
			sinon.spy(this.oSmartTable, "_triggerUI5ClientExport");
			var oCoreOrg = sap.ui.getCore();
			sinon.spy(oCoreOrg, "getLibraryResourceBundle");
			var fnCore = sinon.stub(sap.ui, "getCore");
			var sLib;
			fnCore.returns({
				loadLibrary: function(sLibrary) {
				...
				getLibraryResourceBundle: function(sLib) {
					return oCoreOrg.getLibraryResourceBundle(sLib);
				}
			});
			...
			var fOrgRequire = sap.ui.require;
			sap.ui.require = function(sFP, fCallback) {
				if (sFP[0] && sFP[0] === "sap/ui/export/Spreadsheet") {
					sFilePath = sFP;
					fCallback(oSpreadsheet);
				} else {
					fOrgRequire(sFP, fCallback);
				}
			};
			...
			assert.ok(this.oSmartTable._triggerUI5ClientExport.calledOnce, "this.oSmartTable._triggerUI5ClientExport has to be called for UI5Client export");
			assert.ok(fnCore.calledTwice, "getCore was called twice - once for loadLibrary, once for getLibraryResourceBundle");
			fnCore.restore();
			...
			assert.equal(sFilePath, undefined, "sap/ui/export/Spreadsheet was not required");
			assert.equal(sDownloadUrlType, undefined, "API not called due to missing columns");
			assert.ok(oSpreadsheet.notCalled, "Spreadsheet was not instantiated due to missing columns");

			assert.ok(MessageBox.error.calledOnce, "Error mesage was shown due to mising columns");
			assert.ok(oCoreOrg.getLibraryResourceBundle.calledOnce, "Error mesage was loaded from resource bundle");
			assert.equal(MessageBox.error.getCall(0).args[0], oCoreOrg.getLibraryResourceBundle("sap.ui.comp").getText("SMARTTABLE_NO_COLS_EXPORT"), "Error message Text matches");

			MessageBox.error.restore();
			fnGetColumns.restore();
		});

		QUnit.test("test UI5 client excel export with export settings dialog", function(assert) {
			...
			var aColumns = [
				new UIColumn({
					...
					columnKey: "SomeCurrency",
					leadingProperty: "SomeCurrency",
					edmType: "Edm.String",
					type: "string"
				})
			];
			...
			assert.equal(this.oSmartTable.getExportType(), "UI5Client", "UI5Client is the default on SmartTable");
			assert.ok(this.oSmartTable._oUseExportToExcel.isA("sap.m.MenuButton"), "Export button is an instance of sap.m.MenuButton");
			var oQuickExportMenuItem = this.oSmartTable._oUseExportToExcel.getMenu().getItems()[0];
			var oExportMenuItem = this.oSmartTable._oUseExportToExcel.getMenu().getItems()[1];
			assert.ok(oQuickExportMenuItem, "Export option available");
			assert.ok(oExportMenuItem, "Export As... option available");
			...
			var oExportUtils = {
				...
				openExportSettingsDialog: sinon.stub(),
				parseFilterConfiguration: sinon.stub()
			};
			var oUserInput = {
				fileName: "Test",
				splitCells: true,
			...
			var oHandler = {
				getUserInput: sinon.stub()
			};
			var oFilterConfig = {
				name: "Filter",
				items: []
			};
			oHandler.getUserInput.returns(fResolvedPromise(oUserInput));
			oExportUtils.openExportSettingsDialog.returns(fResolvedPromise(oHandler));
			oExportUtils.parseFilterConfiguration.returns(fResolvedPromise(oFilterConfig));

			var fOrgRequire = sap.ui.require;
			sap.ui.require = function(sFP, fCallback) {
				if (sFP[0] && sFP[0] === "sap/ui/export/ExportUtils") {
					fCallback(oExportUtils);
				} else if (sFP[0] && sFP[0] === "sap/ui/export/Spreadsheet" && sFP[1] && sFP[1] === "sap/ui/export/ExportUtils") {
					fCallback(oSpreadsheet, oExportUtils);
				} else {
					fOrgRequire(sFP, fCallback);
				}
			...
			// Mock user action
			oExportMenuItem.firePress();

			assert.ok(oExportUtils.openExportSettingsDialog.called, "openExportSettingsDialog function called");
			assert.ok(this.oSmartTable._triggerUI5ClientExport.calledWith(oUserInput, oFilterConfig), "export was trigger with correct user input");
			assert.ok(oSpreadsheet.calledOnce, "Spreadsheet was instantiated");
		...
		QUnit.test("test bActiveHeaders for ResponsiveTable & creation of the sap.m.ResponsivePopover", function(assert) {
			...
			assert.ok(this.oSmartTable._oResponsivePopover, "ResponsivePopover created");
			this.oSmartTable._oResponsivePopover.close();
			...
			sinon.spy(this.oSmartTable._oResponsivePopover, "close");
			sinon.spy(this.oSmartTable, "_onCustomFilter");
			// filter button
			this.oSmartTable._oFilterButton.firePress();
			assert.ok(bOpenFilterDialog, "Filter dialog opened via _oResponsivePopover");
			assert.ok(this.oSmartTable._oResponsivePopover.close.calledBefore(this.oSmartTable._onCustomFilter), "Popover should be closed before action is triggered");
			...
			// sort button
			this.oSmartTable._oSortButton.firePress();
			...
			assert.ok(this.oSmartTable._oResponsivePopover.close.calledBefore(this.oSmartTable._onCustomSort), "Popover should be closed before action is triggered");
			this.oSmartTable._oSortButton.firePress();
