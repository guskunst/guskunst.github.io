sap.ui.require([
...
], function(Log, library, ValueHelpDialog, FilterBar, FilterGroupItem, FilterProvider, StringDate, TokenParser, TokenSuggestProvider,
	...
	//oTokenParser.setMaxLength(4);
	//oTokenParser.setDisplayFormat("UpperCase");

	/*  	oTokenParser.removeAllOperations();
        oTokenParser.addOperation("StartsWith", "starts with", "foo*", "*", /^\w+\*$/, "$0*", function(sText) {
            return [sText.slice(0, sText.length - 1).trim()];
        });
	 */
	oTokenParser.associateInput(theTokenInput);
	var oTokenSuggestProvider = new TokenSuggestProvider();

	oTokenSuggestProvider.associateInput(theTokenInput, oTokenParser);
