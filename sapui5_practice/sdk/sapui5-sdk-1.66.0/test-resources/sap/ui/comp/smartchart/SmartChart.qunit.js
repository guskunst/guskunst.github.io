...
sap.ui.define([
	"sap/ui/comp/library",
	"sap/ui/fl/library" // Needed to be loaded earlier... TODO: Check the details and improve on fl side

], function (
	compLibrary,
	flLibrary // Needed to be loaded earlier... TODO: Check the details and improve on fl side
	...
	sap.ui.require([
		...
		QUnit.test("_updateInResultDimensions", function (assert) {
			...
			this.oSmartChart._oChart.setInResultDimensions(["requestField2", "requestField3", "requestField5"]);
			...
			this.oSmartChart._aAlwaysSelect = ["requestField1", "requestField4"];

			//Act
			this.oSmartChart._updateInResultDimensions();

			//Assert
			var aUniqueInResultFields = ["requestField1", "requestField2", "requestField3", "requestField4", "requestField5"];
		...
		function assyncBreadcrumbsTest(assert, fnTest) {
			if (sap.ui.require("sap/m/Breadcrumbs") && sap.ui.require("sap/m/Link")
				&& this.oSmartChart._oDrillBreadcrumbs) {
				fnTest.call(this, assert);
			} else {
				var fnDone = assert.async();
				sap.ui.require(["sap/m/Breadcrumbs", "sap/m/Link"], function () {
					fnTest.call(this, assert);
					fnDone();
				}.bind(this));
			}
		...
		QUnit.test("_toggleZoomButtonEnablement", function (assert) {
			...
			sinon.stub(this.oSmartChart._oChart, "_getZoomInfo").callsFake(function () {
				return { currentZoomLevel: iZoomInfo };
			});
		...
		QUnit.test("_getDrillStackDimensions", function (assert) {
			...
			var aStackLevel1 = { dimension: ["Customer", "Currency"] };
			var aStackLevel2 = { dimension: ["Customer", "Currency", "Status", "Supplier"] };
			var aStackLevel3 = { dimension: ["City"] };
			var aStackLevel4 = { dimension: ["Age", "Unit"] };
			var aDrillStack = [aStackLevel1, aStackLevel2, aStackLevel3, aStackLevel4];
			...
			//Assert
			var aReturnVal = ["Customer", "Currency", "Status", "Supplier", "City", "Age", "Unit"];
		...
		QUnit.test("_toggleMeasureUnitBinding", function (assert) {
			...
			//Measures
			var oMeasure1 = new Measure({ name: "measure1", label: "measure 1", unitBinding: "Euro" });
			...
			var oField1 = { "name": "measure1", "Org.OData.Measures.V1.ISOCurrency": { Path: "Euro" } };
			var oField2 = { "name": "measure2", "Org.OData.Measures.V1.ISOCurrency": { Path: "Dollar" } };
			var oField3 = { "name": "measure3", "Org.OData.Measures.V1.ISOCurrency": { Path: "Pieces" } };
			var oField4 = { "name": "measure4", "Org.OData.Measures.V1.ISOCurrency": { Path: "Groups" } };//with no measure instance
		...
		QUnit.test("test noData property", function (assert) {
			...
			var oSmartFilter = new SmartFilterBar({ id: 'filter' });
