...
"use strict";
	...
	QUnit.test("it should create the composite TextArrangementString data type", function(assert) {
		...
		var oControlFactoryBase = new ControlFactoryBase(undefined, this.oParent);
		var oConfiguration = new Configuration({
			displayBehaviour: sExpectedDisplayBehaviour
		});
		this.stub(this.oParent, "getTextInEditModeSource").returns(library.smartfield.TextInEditModeSource.NavigationProperty);
		this.stub(this.oParent, "getControlFactory").returns(oControlFactoryBase);
		this.stub(this.oParent, "getConfiguration").returns(oConfiguration);
		var oProperty = {
			property: {
				type: "Edm.String"
			},
		...
		// act
		var oStringType = this.oMetaData.getType(oProperty, null, null, mSettings);
		...
		// cleanup
		oConfiguration.destroy();
		oControlFactoryBase.destroy();
	});
	...
	QUnit.test("getCurrencyType", function(assert) {
		...
		assert.strictEqual(oFormatOptions.maxFractionDigits, 2);
