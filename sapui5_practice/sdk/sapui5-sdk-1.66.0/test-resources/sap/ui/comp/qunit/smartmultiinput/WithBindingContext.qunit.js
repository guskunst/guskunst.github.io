...
sap.ui.define("test.sap.ui.comp.qunit.smartmultiinput.WithBindingContext", [
	...
	QUnit.test("_addToken", function (assert) {
		...
		function onRequestCompleted() {
			...
			assert.equal(oCreateSpy.args[0][0], "/Products('1')/Categories", "create was called with correct path");
			assert.deepEqual(oCreateSpy.args[0][1].properties, mAfterCreateData, "create was called with correct data");

			assert.equal(Object.keys(oCreateSpy.args[0][1]).length, 5
	...
	QUnit.test("_removeToken", function (assert) {
		...
		var oCreateSpy = sinon.spy(this.oModel, "remove");
		...
		function onRequestCompleted() {
			...
			assert.ok(oCreateSpy.calledOnce, "remove called once");
			assert.equal(oCreateSpy.args[0][0], "/Categories(CategoryId='PR',ProductId='1')",
				"remove called with correct path");

			assert.equal(Object.keys(oCreateSpy.args[0][1]).length, Object.keys(mBeforeRemove).length + 1,
				"remove called with correct number of parameters");

			for (var sParam in mBeforeRemove) {
				assert.equal(oCreateSpy.args[0][1][sParam], mBeforeRemove[sParam], sParam + " is correct");
			}
