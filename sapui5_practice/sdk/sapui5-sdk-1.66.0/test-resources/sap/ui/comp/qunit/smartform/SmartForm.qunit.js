...
sap.ui.define([
	...
	function useHorizontalLayoutWithoutLayout(assert) {
		...
		var oFormLayoutId = oFormLayout ? oFormLayout.getId() : "";
		...
		assert.notOk(sap.ui.getCore().byId(oFormLayoutId), "ResponsiveLayout destroyed");
		oLayoutData = oGroup1.getLayoutData();
	...
	function useColumnLayout(assert) {
		...
		oFormLayout = oForm.getLayout();
		assert.ok(oFormLayout && oFormLayout instanceof ResponsiveGridLayout, "ResponsiveGridLayout used after reset");
		assert.notOk(sap.ui.getCore().byId(oFormLayoutId), "ColumnLayout destroyed");
	...
	QUnit.test("editable", function(assert) {
		...
		sinon.spy(oGroup1, "setEditMode");
		sinon.spy(oGroup2, "setEditMode");
		sinon.spy(oGroupElement1, "setEditMode");
		sinon.spy(oGroupElement2, "setEditMode");
		sinon.spy(oGroupElement3, "setEditMode");
		...
		assert.ok(oGroup1.setEditMode.called, "setEditMode called on Group1");
		assert.ok(oGroup1.setEditMode.calledWith(true), "setEditMode called on Group1 with \"true\"");
		assert.ok(oGroup2.setEditMode.calledWith(true), "setEditMode called on Group2 with \"true\"");
		assert.ok(oGroupElement1.setEditMode.calledWith(true), "setEditMode called on GroupElement1 with \"true\"");
		assert.ok(oGroupElement2.setEditMode.calledWith(true), "setEditMode called on GroupElement2 with \"true\"");
		assert.ok(oGroupElement3.setEditMode.calledWith(true), "setEditMode called on GroupElement3 with \"true\"");
	});
