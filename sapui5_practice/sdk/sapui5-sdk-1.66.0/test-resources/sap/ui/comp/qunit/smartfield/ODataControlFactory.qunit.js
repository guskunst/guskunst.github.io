...
sap.ui.define([
	...
	QUnit.test("it should create the inner controls synchronous if the meta model is loaded", function(assert) {
		...
		var oSpyTriggerCreationOfInnerControls = this.spy(oFactory, "triggerCreationOfInnerControls");

		// act
		oFactory.bind();

		// assert
		assert.strictEqual(oSpyTriggerCreationOfInnerControls.callCount, 1, "The .triggerCreationOfInnerControls method was called");
	...
	QUnit.test("it should not try to initialize the inner controls after the SmartField control is destroyed", function(assert) {
		...
		oFactory.bind().finally(function() {
			...
			// assert
			assert.strictEqual(oSpyTriggerCreationOfInnerControls.callCount, 0);
			done();
		});
	...
	QUnit.test("_createEdmUOM - unit suppressed, no currency", function(assert) {
		...
		oFactory.bind().finally(function() {
			oFactory._oMetaData.annotations.uom.property.property["sap:semantics"] = "";
			var oControl = oFactory._createEdmUOM();
	...
	QUnit.test("_createEdmUOMAttributes for amount", function(assert) {
		...
		oFactory.bind().finally(function() {
			...
			var mParams = oFactory._createEdmUOMAttributes();
	...
	QUnit.test("_handleEventingForEdmString", function(assert) {
		...
		oFactory.bind().finally(function() {
			oFactory._handleEventingForEdmString(oControl, oFactory._oMetaData.property);
			fChange();
			fChange({
	...
	QUnit.module("getBoundPropertiesMapInfoForInnerControl");
	...
	QUnit.test('it should return the correct mapping information for "sap.m.Input"', function(assert) {
		...
		// act
		var sProperty = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME, mSETTINGS)[0];
		var mMap = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME);
	...
	QUnit.test('it should return the correct mapping information for "sap.m.Link" (test case 1)', function(assert) {
		...
		// act
		var sProperty1 = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME, mSETTINGS)[0];
		var sProperty2 = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME, mSETTINGS)[1];
		var mMap = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME);
	...
	QUnit.test('it should return the correct mapping information for "sap.m.ObjectStatus"', function(assert) {
		...
		var sProperty1 = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME, mSETTINGS)[0];
		var sProperty2 = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME, mSETTINGS)[1];
		var sProperty3 = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME, mSETTINGS)[2];
		var mMap = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME);
	...
	QUnit.test("calling the getBoundPropertiesMapInfoForInnerControl method should not raise an exception", function(assert) {
		...
		try {
			oExpectedReturnValue = ODataControlFactory.getBoundPropertiesMapInfoForInnerControl(CONTROL_NAME, mSETTINGS);
		} catch (oEvent) {
	...
	QUnit.module("Text arrangment annotation utilities");

	QUnit.test("calling ODataControlFactory.getTextArrangmentPaths() should return the corrects paths (test case 1)", function(assert) {
		...
		// act
		var mTextArrangmentPaths = ODataControlFactory.getTextArrangementPaths(library.smartfield.TextInEditModeSource.NavigationProperty, oMetadata);

		// assert
		assert.strictEqual(mTextArrangmentPaths.keyField, "IDNavigationProperty");
		assert.strictEqual(mTextArrangmentPaths.descriptionField, "DescriptionNavigationProperty");
		assert.strictEqual(mTextArrangmentPaths.entitySetName, "Category");
	});

	QUnit.test("calling ODataControlFactory.getTextArrangmentPaths() method should return the corrects paths (test case 2)", function(assert) {
		...
		// act
		var mTextArrangmentPaths = ODataControlFactory.getTextArrangementPaths(library.smartfield.TextInEditModeSource.ValueList, oMetadata);

		// assert
		assert.strictEqual(mTextArrangmentPaths.keyField, "IDValueList");
		assert.strictEqual(mTextArrangmentPaths.descriptionField, "DescriptionValueList");
		assert.strictEqual(mTextArrangmentPaths.entitySetName, "VL_Suppliers");
	});
