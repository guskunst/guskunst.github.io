...
sap.ui.define([
	...
	QUnit.test("getControlProperties", function(assert) {
	...
	QUnit.test("_getEditable - static value from updatable attribute on property", function(assert) {
		...
		assert.strictEqual(oResult.formatter("fieldContent", 7, false), false);
	});
	...
	QUnit.test("_getEditable - with/out additional binding expression", function(assert) {
		...
		aPaths = oResult.path();
		assert.strictEqual(oResult.formatter(7, false), true);
	});
	...
	QUnit.test("getBindablePropertiesNames", function(assert) {
		var aResult;

		aResult = this.oMetaData.getBindablePropertiesNames();
		assert.strictEqual(aResult.length, 3);

		aResult = this.oMetaData.getBindablePropertiesNames(true);
