...
sap.ui.define([
	...
	QUnit.test("Editing entity with creatable false and updatable not specified", function (assert) {
		...
		return TestUtils.createDataModel().then(function (oData) {
			...
			var oFistNameField = oContainer.getFields()[0];
			oFistNameField.setSelectedIndex(1);
			sap.ui.getCore().applyChanges();

			assert.equal(oFistNameField.getSmartField().getEditable(), true, "Smart Filed should be editable.");
