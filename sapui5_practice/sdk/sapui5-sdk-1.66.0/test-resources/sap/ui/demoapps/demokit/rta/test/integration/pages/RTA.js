	...
	Opa5.createPageObjects({
		onTheMasterPageWithRTA: {
				...
				iUseTheStorageFromIFrame: function(sId, sViewName, bReset) {
					return this.waitFor({
						...
						success: function() {
							...
							FakeLrepSessionStorage.setStorage(Opa5.getWindow().sessionStorage);
							if (bReset) {
								FakeLrepSessionStorage.deleteChanges();
							}
						}
			...
			assertions: {
				...
				iShouldSeeTheFLPToolbarAndChangesInLRep: function(iCount) {
					return this.waitFor({
						id: "shell-header",
						success: function(oToolbar) {
							assert.ok(oToolbar.getVisible(), "the FLP Toolbar is shown");
							assert.equal(FakeLrepSessionStorage.getNumChanges(), iCount, "the number of changes is correct");
						},
				...
				iShouldSeeChangesInLRepWhenTheBusyIndicatorIsGone: function(sId, sViewName, iCount) {
					return this.waitFor({
						id: sId,
						viewName: sViewName,
						success: function() {
