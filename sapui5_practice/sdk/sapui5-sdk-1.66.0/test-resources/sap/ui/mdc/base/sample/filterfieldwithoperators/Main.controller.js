...
//		ResourceModel,
	...
	return Controller.extend("my.Main", {
		...
		onInit: function() {
			...
			// add some custom operations to the Conditionmodel FilterOperatorConfig
			this.addCustomOperators(oCM.getFilterOperatorConfig());
			...
			oConditionChangeBinding.attachChange(function(oEvent) {
				...
				if (this.byId("liveupdate").getSelected()) {
					var bValid = oCM.applyFilters(this.byId("movieTable").getBinding("items"), false);
		...
		addCustomOperators: function(oFilterOperatorConfig) {
			//add the LASTYEAR and LASTXYEARS operation to the FilterOperatorConfig
			...
			// assign the new Operators to the Date type
			oFilterOperatorConfig.addOperatorsToType("sap.ui.model.type.Date", ["LASTYEAR", "LASTXYEARS", "LASTXYYEARS"]);
		},

		onSaveVariant: function() {
			var oTable = this.byId("movieTable");
			var oListBinding = oTable.getBinding("items");
			var sVariant = ConditionModel.serialize(oListBinding);
		...
		onShowMetadata: function() {
			var oTable = this.byId("movieTable");
			var oListBinding = oTable.getBinding("items");
			var sVariant = ConditionModel.serializeMeta(oListBinding);
			this.byId("variant").setValue(sVariant);
		},

		onLoadVariant: function() {
			ConditionModel.parse(this.byId("variant").getValue());

			var bValid = this.getView().getModel("cm").applyFilters(this.byId("movieTable").getBinding("items"), false);
			this.byId("movieTable").setShowOverlay(!bValid);
		},
		...
		onGo: function() {
			...
			var bValid = oCM.applyFilters(this.byId("movieTable").getBinding("items"), true);
			this.byId("movieTable").setShowOverlay(!bValid);
...
//				sResult = oOperator.format(aValues, oContext, that._getDataType());
//			}
//			return sResult;
//		},
//
...
//		},
		...
		onConditionModelToExpr: function() {
			var oTable = this.byId("movieTable");
			var oListBinding = oTable.getBinding("items");
			var cm = ConditionModel.getFor(oListBinding);
			this.byId("expert").setValue(BoolExprTool.prettyPrintCM(cm));
		},
		...
		onExprToConditionModel: function() {
			...
			var ast = BoolExprTool.booleanExprParser(sExpr, cm);
			cm.removeAllConditions();
			BoolExprTool.ASTtoCM(ast, cm);
//			var bValid = this.getView().getModel("cm").applyFilters(this.byId("movieTable").getBinding("items"), false);
		...
		onFormatAst: function() {
			...
			if (sValue) {
				...
				this.byId("expert").setValue(BoolExprTool.booleanExprFormatter(ast, cm));
			}
		},
		...
		onParseExpr: function() {
			...
			try {
				...
				var ast = BoolExprTool.booleanExprParser(sValue, cm); //"a and b or !(f1: =c)");
				window.console.log("stringify(ast) : " + JSON.stringify(ast));
				window.console.log("format(ast) : " + BoolExprTool.booleanExprFormatter(ast, cm));
