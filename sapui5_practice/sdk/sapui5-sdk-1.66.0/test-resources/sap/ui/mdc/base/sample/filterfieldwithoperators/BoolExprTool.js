 ...
 */
sap.ui.define(["jquery.sap.global", "sap/ui/base/ManagedObject", "sap/ui/mdc/base/Condition"],
	function(jQuery, ManagedObject, Condition) {
		...
		BoolExprTool.booleanExprParser = function(parseText, oConditionModel) {
			...
			var complexvalue = function() {
				...
				if (cm && cm.getFilterField(sfieldPath)) {
					var oType = cm.getFilterField(sfieldPath)._getDataType();
					try {
						sValue.values[0] = oType.parseValue(sValue.values[0], "string");
						if (sValue.values.length > 1) {
							sValue.values[1] = oType.parseValue(sValue.values[1], "string");
						}
					} catch (error) {
		...
		BoolExprTool.booleanExprFormatter = function(ast, oConditionModel) {
			...
			var format = function(ast) {
				...
				switch (ast.type) {
					...
					case "complexvalue":
						if (ast.fieldPath) {
							oType = cm ? cm.getFilterField(ast.fieldPath)._getDataType() : null;
		...
		BoolExprTool.prettyPrintCM = function(oConditionModel) {
			...
			for (var fieldPath in oConditions) {
				...
				for (var index = 0; index < aConditions.length; index++) { //function(oCondition, index) {
					...
					if (index >= 1) { sExpr += " OR "; }
					sExpr += fieldPath + ":= ";
					var sValue1 = oType.formatValue(oCondition.values[0], "string");
					...
					switch (oCondition.operator) {
						...
						case "NotEmpty":
							...
							break;
						case "BT":
							sExpr += sValue1 + "..." + oType.formatValue(oCondition.values[1], "string");
							break;
						default:
							break;
					}
