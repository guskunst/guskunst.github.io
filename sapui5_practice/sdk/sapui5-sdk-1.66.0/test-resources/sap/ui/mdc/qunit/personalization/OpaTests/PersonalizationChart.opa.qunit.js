sap.ui.define([
	...
	opaTest("When I start the 'appUnderTestChart' app, the chart with some dimentions and measures appears", function(Given, When, Then) {
		Given.iStartMyAppInAFrame('test-resources/sap/ui/mdc/qunit/personalization/OpaTests/appUnderTestChart/start.html');
		...
		Then.iShouldSeeButtonWithIcon(Arrangement.P13nDialog.Filter.Icon);
	...
	opaTest("When I press on 'Define Sort Properties' button, sort dialog should open", function(Given, When, Then) {
		...
		Then.iShouldSeeItemOnPosition("Custom Aggregate (Amount)", 0);
		...
		Then.iShouldSeeItemOnPosition("Product Name", 2);
	...
	opaTest("When I press on 'Define Filter Properties' button, filter dialog should open", function(Given, When, Then) {
		...
		Then.iShouldSeeItemOnPosition("CategoryId", 0);
		Then.iShouldSeeItemOnPosition("CategoryName", 1);
		Then.iShouldSeeItemOnPosition("Color", 2);
		Then.iShouldSeeItemOnPosition("Country", 3);
		Then.iShouldSeeItemOnPosition("CurrencyCode", 4);
		Then.iShouldSeeItemOnPosition("CurrencyName", 5);
		Then.iShouldSeeItemOnPosition("CustomerId", 6);
		Then.iShouldSeeItemOnPosition("CustomerName", 7);
		Then.iShouldSeeItemOnPosition("Date", 8);
		Then.iShouldSeeItemOnPosition("Month", 9);
		Then.iShouldSeeItemOnPosition("Product Name", 10);
		Then.iShouldSeeItemOnPosition("ProductId", 11);
		Then.iShouldSeeItemOnPosition("Quarter", 12);
		Then.iShouldSeeItemOnPosition("SalesOrganizationId", 13);
		Then.iShouldSeeItemOnPosition("SalesOrganizationName", 14);
		Then.iShouldSeeItemOnPosition("SuperOrdinateId", 15);
		Then.iShouldSeeItemOnPosition("TaxRate", 16);
		Then.iShouldSeeItemOnPosition("Year", 17);
	});
	opaTest("When I close the 'Define Filter Properties', the chart has not been changed", function(Given, When, Then) {
	...
	opaTest("When I select the 'Custom Aggregate (Forecast)' measure and move the 'Custom Aggregate (Forecast)' to the top, the chart should be changed", function(Given, When, Then) {
		When.iSelectColumn("Custom Aggregate (Forecast)", Arrangement.P13nDialog.Titles.chart).and.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.MoveToTop);
	...
	opaTest("When I select the 'Country' dimension and move the 'Country' to the top, the chart should be changed", function(Given, When, Then) {
		When.iSelectColumn("Country", Arrangement.P13nDialog.Titles.chart).and.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.MoveToTop);
