/* global QUnit*/

sap.ui.define([
	"sap/ui/vk/Viewport",
	"sap/ui/vk/ViewGallery",
	"test-resources/sap/ui/vk/qunit/utils/ModuleWithContentConnector",
	"sap/ui/vk/threejs/ViewStateManager"
], function(
	Viewport,
	ViewGallery,
	Loader,
	ViewStateManager
) {
	"use strict";

	var viewStateManager = new ViewStateManager();
	var viewport = new Viewport({ viewStateManager: viewStateManager });
	viewport.placeAt("content");

	var viewGallery = new ViewGallery({
		viewport: viewport
	});

	viewGallery.placeAt("content");


	QUnit.moduleWithContentConnector("ViewGallery", "test-resources/sap/ui/vk/qunit/media/nodes_boxes.json", "threejs.test.json", function(assert) {
		var scene = this.contentConnector.getContent().getSceneRef();
		var vpg1 = { name: "test1", modelViews: [ {
			"name": "Step 1",
			"id": "i0000000300000001",
			"description": "sdfsdfsfds",
			"thumbnailData": "",
			"thumbnailType": "png"
		},
		{
			"name": "Step 2",
			"id": "i0000000300000001",
			"description": "sdfsdfsfds",
			"thumbnailData": "",
			"thumbnailType": "png"
		},
		{
			"name": "Step 3",
			"id": "i0000000300000001",
			"description": "",
			"thumbnailData": "",
			"thumbnailType": "png"
		}
		] };
		var vpg2 = { name: "test2", modelViews: [ {
			"name": "Step 2",
			"id": "i0000000300000002",
			"description": "sdfsdfsfds",
			"thumbnailData": "",
			"thumbnailType": "png"
		}
		] };
		scene.userData = { viewportGroups: [ vpg1, vpg2 ] };
		viewport.setContentConnector(this.contentConnector);
		viewStateManager.setContentConnector(this.contentConnector);
		viewGallery.setContentConnector(this.contentConnector);
	});

	QUnit.test("Initial State", function(assert) {
		assert.ok(viewGallery._procedureList.getItems().length === 2, "The procedure list length");
		assert.equal(viewGallery._currentGroupTitle.getText(), "test1", "The current group title");
		assert.equal(viewGallery._stepCount.getText(), "0 of 3", "The step count");
	});

	QUnit.test("Step Description Box Orientation", function(assert){
		viewGallery._stepDescriptionToolbar.setVisible(true);
		viewGallery._stepDescriptionText.setVisible(true);
		viewGallery._stepDescriptionOrientationIcon.firePress();
		assert.ok(viewGallery._stepDescriptionVerticalToolbar.getVisible(), "Vertical step description box visibility");
		assert.ok(viewGallery._stepDescriptionVerticalText.getVisible(), "Vertical step description text visibility");
		assert.notOk(viewGallery._stepDescriptionToolbar.getVisible(), "Step description box visibility");
		assert.notOk(viewGallery._stepDescriptionText.getVisible(), "Step description text visibility");
		viewGallery._stepDescriptionVerticalOrientationIcon.firePress();
		assert.notOk(viewGallery._stepDescriptionVerticalToolbar.getVisible(), "Vertical step description box visibility");
		assert.notOk(viewGallery._stepDescriptionVerticalText.getVisible(), "Vertical step description text visibility");
		assert.ok(viewGallery._stepDescriptionToolbar.getVisible(), "Step description box visibility");
		assert.ok(viewGallery._stepDescriptionText.getVisible(), "Step description text visibility");
	});

	QUnit.test("Step Description Box Expanded", function(assert){
		viewGallery._stepDescriptionIcon.firePress();
		assert.ok(viewGallery._stepDescriptionText.getVisible(), "Step description text visibility");
		assert.ok(viewGallery._stepDescriptionIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Step description icon transformed style class");
		assert.ok(viewGallery._stepDescriptionVerticalIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Vertical step description icon transformed style class");
		viewGallery._stepDescriptionIcon.firePress();
		assert.notOk(viewGallery._stepDescriptionText.getVisible(), "Step description text visibility");
		assert.notOk(viewGallery._stepDescriptionIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Step description icon transformed style class");
		assert.notOk(viewGallery._stepDescriptionVerticalIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Vertical step description icon transformed style class");
		viewGallery._stepDescriptionVerticalIcon.firePress();
		assert.ok(viewGallery._stepDescriptionVerticalText.getVisible(), "Vertical step description text visibility");
		assert.ok(viewGallery._stepDescriptionVerticalIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Vertical step description icon transformed style class");
		assert.ok(viewGallery._stepDescriptionIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Step description icon transformed style class");
		viewGallery._stepDescriptionVerticalIcon.firePress();
		assert.notOk(viewGallery._stepDescriptionVerticalText.getVisible(), "Vertical step description text visibility");
		assert.notOk(viewGallery._stepDescriptionVerticalIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Vertical step description icon transformed style class");
		assert.notOk(viewGallery._stepDescriptionIcon.hasStyleClass("sapVizKitViewGalleryStepDescriptionIconTransform"), "Step description icon transformed style class");
	});

	QUnit.test("Procedure Finished", function(assert){
		viewGallery._playButton.setPressed(true);
		viewGallery._playButton.setIcon("sap-icon://media-pause");
		viewGallery._playButton.setTooltip(sap.ui.vk.getResourceBundle().getText("STEP_NAV_PLAYMENU_PAUSE"));
		viewGallery._finishProcedure();
		assert.notOk(viewGallery._playButton.getPressed(), "Play button pressed state");
		assert.equal(viewGallery._playButton.getIcon(), "sap-icon://media-play", "Play button icon");
		assert.equal(viewGallery._playButton.getTooltip(), sap.ui.vk.getResourceBundle().getText("STEP_NAV_PLAYMENU_PLAY"), "Play button tooltip");
	});

	QUnit.test("View Finished", function(assert){
		viewGallery._playButton.setPressed(true);
		viewGallery._playButton.setIcon("sap-icon://media-pause");
		viewGallery._playButton.setTooltip(sap.ui.vk.getResourceBundle().getText("STEP_NAV_PLAYMENU_PAUSE"));
		viewGallery._finishView();
		assert.notOk(viewGallery._playButton.getPressed(), "Play button pressed state");
		assert.equal(viewGallery._playButton.getIcon(), "sap-icon://media-play", "Play button icon");
		assert.equal(viewGallery._playButton.getTooltip(), sap.ui.vk.getResourceBundle().getText("STEP_NAV_PLAYMENU_PLAY"), "Play button tooltip");
	});

	QUnit.test("Set SelectedItem", function(assert){
		var arr = viewGallery.getItems();
		viewGallery.setSelectedItem(arr[ 0 ], false);
		assert.equal(viewGallery._stepCount.getText(), "1 of 3", "The step count");
		assert.notOk(viewGallery._userActivated, "User activated step");
		assert.equal(viewGallery._currentStepTitle.getText(), "Step 1", "Current step title");
		assert.equal(viewGallery._currentStepTitle.getTooltip(), "Step 1", "Current step tooltip");
		assert.equal(viewGallery._separatorTitle.getText(), "/", "Seperator title");
		assert.equal(viewGallery._stepDescriptionText.getHtmlText(), "sdfsdfsfds", "Step description text");
		assert.equal(viewGallery._stepDescriptionVerticalText.getHtmlText(), "sdfsdfsfds", "Vertical step description text");
		assert.ok(viewGallery._stepDescriptionToolbar.getVisible(), "Step description visibility");
		assert.notOk(viewGallery._stepDescriptionVerticalToolbar.getVisible(), "Vertical step description visibility");
		assert.notOk(viewGallery._previousOrientationVertical, "Previous orientation variable");
		var arr2 = viewGallery._selectedItem.getItems();
		var text = arr2[ 0 ];
		var img = arr2[ 1 ];
		assert.ok(text.hasStyleClass("sapVizKitViewGalleryStepNumberTextSelected"), "Text style class added");
		assert.ok(img.hasStyleClass("selected"), "Image style class added");
		viewGallery._previousOrientationVertical = true;
		viewGallery.setSelectedItem(arr[ 1 ], false);
		assert.notOk(text.hasStyleClass("sapVizKitViewGalleryStepNumberTextSelected"), "Text style class removed");
		assert.notOk(img.hasStyleClass("selected"), "Image style class removed");
		assert.ok(viewGallery._stepDescriptionVertical.getVisible(), "Vertical step description visibility");
		assert.notOk(viewGallery._stepDescriptionToolbar.getVisible(), "Step description visibility");
		assert.ok(viewGallery._stepDescriptionVerticalToolbar.getVisible(), "Vertical step description box visibility");
		assert.ok(viewGallery._previousOrientationVertical, "Previous orientation variable");
		viewGallery.setSelectedItem(arr[ 2 ], false);
		assert.notOk(viewGallery._stepDescriptionVertical.getVisible(), "Vertical step description visibility");
		viewGallery._previousOrientationVertical = false;
		viewGallery.setSelectedItem(arr[ 2 ], false);
		assert.notOk(viewGallery._stepDescriptionToolbar.getVisible(), "Step description visibility");
		assert.notOk(viewGallery._previousOrientationVertical, "Previous orientation variable");
	});

	QUnit.done(function() {
		jQuery("#content").hide();
	});
});