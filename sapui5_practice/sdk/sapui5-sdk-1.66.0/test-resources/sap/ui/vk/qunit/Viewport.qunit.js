...
sap.ui.define([
	...
	QUnit.test("Loading content resources", function(assertMain) {
		...
		var truncateValues = function(obj) {
				...
				// We are truncating the camera position values to 2 decimals.
		...
		var testViewportWasCreated = function(viewport) {
			QUnit.test("Constructor", function(assert) {
				...
				assert.ok(viewport instanceof sap.ui.vk.dvl.Viewport, "The current viewport is an instance of sap.ui.vk.dvl.Viewport");
			});
		};
		...
		var testGetAndSetGraphicsCore = function(viewportNoGraphicsCore, viewport) {
			QUnit.test("setGraphicsCore & getGraphicsCore", function(assert) {
				...
				assert.ok(viewport.getGraphicsCore() instanceof sap.ui.vk.dvl.GraphicsCore, "The viewport returns an instance of sap.ui.vk.dvl.GraphicsCore when calling 'getGraphicsCore()'.");
			});
		};
		...
		var graphicsCore = new GraphicsCore({ totalMemory: 16777216 }, {
			antialias: true,
			alpha: true,
					...
					setTimeout(function() {
						...
						var defaultInfo = {
							...
							camera: {
								...
								bindingType: sap.ui.vk.CameraFOVBindingType.Minimum,
								projectionType: sap.ui.vk.CameraProjectionType.Orthographic,
