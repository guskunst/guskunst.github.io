	...
	QUnit.test("Three JS Material Interface", function(assert) {
		...
		var testMaterial = function() {
			...
			var dc = { red: sourceData.diffuseR,
							...
							alpha: 1.0 };
			material.setDiffuseColour(sap.ui.vk.colorToCSSColor(dc));

			var sc = { red: sourceData.specularR,
							green: sourceData.specularG,
							blue: sourceData.specularB,
							alpha: 1.0 };
			material.setSpecularColour(sap.ui.vk.colorToCSSColor(sc));

			var ec = { red: sourceData.emissiveR,
							green: sourceData.emissiveG,
							blue: sourceData.emissiveB,
							alpha: 1.0 };
			material.setEmissiveColour(sap.ui.vk.colorToCSSColor(ec));
			...
			var diffuseColour = sap.ui.vk.cssColorToColor(diffuseC);
			...
			var specularColour = sap.ui.vk.cssColorToColor(specularC);
			...
			var emissiveColour = sap.ui.vk.cssColorToColor(emissiveC);
		...
		var testLineMaterial = function() {
			...
			var c = { red: sourceData.lineColourR,
						...
						alpha: 1.0 };
			material.setLineColour(sap.ui.vk.colorToCSSColor(c));

			var lineC = material.getLineColour();
			var lineColour = sap.ui.vk.cssColorToColor(lineC);
		...
		var testMaterialInNodeProxy = function(nodeProxy) {
			...
			nodeProxy.replaceMaterials(currentMaterial, oldMaterial);
			assert.ok(nodeProxy.enumerateMaterials()[0].getMaterialRef() === oldMaterial.getMaterialRef()
							, "NodeProxy replaceMaterials function works ok");
