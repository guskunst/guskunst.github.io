	...
	*/
	...
	var testBuildSceneTree = function(scene) {
		QUnit.test("Test buildSceneTree method", function(assert) {
			...
			assert.ok(scene instanceof sap.ui.vk.dvl.Scene, "buildSceneTree returns an instance of sap.ui.vk.Scene");
		});
	};
	...
	var testGetApi = function(graphicsCore) {
		var api = graphicsCore.getApi(sap.ui.vk.dvl.GraphicsCoreApi.LegacyDvl);
	...
	var graphicsCore = new GraphicsCore({ totalMemory: 16777216 * 2 }, {
		antialias: true,
		alpha: true,
		...
		graphicsCore.loadContentResourcesAsync([ contentResource ], function(sourcesFailedToLoad) {
			if (sourcesFailedToLoad) {
				...
				promise.then(function(data) {
					assert.ok(data.scene instanceof sap.ui.vk.dvl.Scene, "buildSceneTreeAsync promise resolves as an instance of sap.ui.vk.Scene");
					done();
				});
			}
		});
