...
sap.ui.define([
	...
	QUnit.test("Testing importJSON & exportJSON", function(assert) {
		...
		var ellipse = new sap.ui.vk.RedlineElementEllipse({
			originX: 0.5,
			originY: 0.5,
			radiusX: 0.2,
			radiusY: 0.3,
		...
		assert.strictEqual(sap.ui.vk.Redline.ElementType.Ellipse, exportedJson[0].type, "The aggregation and the exported element are both ellipses.");
		...
		assert.strictEqual(sap.ui.vk.Redline.ElementType.Ellipse, jsonEllipse.type, "The imported aggregation and the json element are both ellipses.");
	});
