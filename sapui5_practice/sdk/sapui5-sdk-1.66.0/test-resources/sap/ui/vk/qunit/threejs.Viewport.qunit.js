...
sap.ui.define([
	...
	QUnit.test("Initialization", function(assert) {
		...
		assert.ok(viewport instanceof sap.ui.vk.threejs.Viewport, "The viewport is sap.ui.vk.threejs.Viewport implementation.");
	});
	...
	QUnit.test("Default values", function(assert) {
		...
		assert.equal(viewInfo.visibility.visible.length, 28, "Visibility required - Correct number of visible nodes");
		...
		viewInfo = viewport.getViewInfo({ visibility: { mode: sap.ui.vk.VisibilityMode.Differences }, camera: false });
	...
	QUnit.test("setViewInfo getViewInfo", function(assert) {
		...
		var cameraTests = [
			{ camera: { rotation: { yaw: -48, pitch: 32, roll: -35 }, position: { x: 10, y: 20, z: 30 }, projectionType: sap.ui.vk.CameraProjectionType.Orthographic, zoomFactor: 1.123 } },
			{ camera: { rotation: { yaw: -137, pitch: -32, roll: 45 }, position: { x: 200, y: 300, z: 400 }, projectionType: sap.ui.vk.CameraProjectionType.Perspective, fieldOfView: 45.67 } },
			{ camera: { rotation: { yaw: 134, pitch: 43, roll: -76 }, position: { x: 100, y: 50, z: 25 }, projectionType: sap.ui.vk.CameraProjectionType.Orthographic, zoomFactor: 2.345 } },
			{ camera: { rotation: { yaw: 90, pitch: -5, roll: 10 }, position: { x: 350, y: 0, z: 0 }, projectionType: sap.ui.vk.CameraProjectionType.Perspective, fieldOfView: 34.56 } }
		];
		...
		cameraTests.forEach(function(test) {
			...
			if (camera.projectionType === sap.ui.vk.CameraProjectionType.Orthographic) {
		...
		var visibilityTests = [
			{ visibility: { visible: [ nodes[ 3 ], nodes[ 6 ] ], hidden: [ nodes[ 7 ], nodes[ 9 ] ], mode: sap.ui.vk.VisibilityMode.Complete } },
			{ visibility: { visible: [ nodes[ 7 ], nodes[ 3 ] ], hidden: [ nodes[ 9 ], nodes[ 6 ] ], mode: sap.ui.vk.VisibilityMode.Complete } },
			{ visibility: { visible: [ nodes[ 9 ], nodes[ 6 ] ], hidden: [ nodes[ 7 ], nodes[ 3 ] ], mode: sap.ui.vk.VisibilityMode.Complete } },
			{ visibility: { visible: [ nodes[ 3 ], nodes[ 7 ], nodes[ 9 ] ], hidden: [ nodes[ 6 ] ], mode: sap.ui.vk.VisibilityMode.Complete } }
		];

		visibilityTests.forEach(function(test) {
			viewport.setViewInfo(test, 0);
			var visibility = viewport.getViewInfo({ visibility: { mode: sap.ui.vk.VisibilityMode.Complete } }).visibility;
