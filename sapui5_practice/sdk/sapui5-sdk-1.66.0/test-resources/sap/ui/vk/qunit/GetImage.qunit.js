...
function(
	...
	QUnit.test("Original background", function(assert) {
		...
		img.onload = function() {
			assert.equal(img.width, 100 * window.devicePixelRatio, "Image width is correct");
			assert.equal(img.height, 100 * window.devicePixelRatio, "Image height is correct");
			...
			pixel = getPixel(img, 0, 100 * window.devicePixelRatio - 1);
	...
	QUnit.test("Gradient background", function(assert) {
		...
		img.onload = function() {
			switch (phase) {
				...
				break;
				case 2:
					assert.equal(img.width, 16 * window.devicePixelRatio, "Default image width is correct");
					assert.equal(img.height, 16 * window.devicePixelRatio, "Default image height is correct");

					pixel = getPixel(img, 0, 0);
					assert.ok(Math.abs(pixel[0] - 55) < tolerance && Math.abs(pixel[1] - 55) < tolerance && Math.abs(pixel[2] - 55) < tolerance, "Top is dark grey");

					pixel = getPixel(img, 0, 16 * window.devicePixelRatio - 1);
