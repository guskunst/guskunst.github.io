...
(function () {
	...
	function run(Chart, fnModifyParameters) {
		...
		QUnit.test("When calling getThumbnailContent", function(assert) {
			//arrange
			var chart = this.setup.chart;

			// act
			var thumbnailContent = chart.getThumbnailContent();

			// assert
			assert.ok(chart.thumbnailChart instanceof sap.viz.ui5.controls.VizFrame, "then thumbnail chart is a VizFrame");
			assert.ok(thumbnailContent instanceof sap.ui.layout.HorizontalLayout, "then result is a HorizontalLayout");
		});
