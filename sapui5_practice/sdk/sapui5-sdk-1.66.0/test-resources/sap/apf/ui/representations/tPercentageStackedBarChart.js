...
sap.ui.define("sap/apf/ui/representations/tPercentageStackedBarChart", [
	...
	function _commonInitializationAsserts(chart, assert) {
		...
		assert.strictEqual(chart.thumbnailChart.getVizType(), "100_stacked_bar", "Then thumbnailcontent has Percentage Stacked Bar chart");
	}
	...
	QUnit.test("When there is no required filter", function(assert) {
		...
		var thumbnailContent = this.chart.getThumbnailContent("sample Title", 600, 600);
	...
	QUnit.test("When Percentage Stacked Bar Chart is initialized with multiple parameters", function(assert) {
		...
		assert.strictEqual(this.chart.thumbnailChart.getVizType(), "100_stacked_bar", "Then thumbnailcontent has Percentage Stacked Bar chart");
