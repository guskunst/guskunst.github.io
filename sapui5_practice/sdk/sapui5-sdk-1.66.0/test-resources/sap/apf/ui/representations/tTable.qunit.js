...
sap.ui.define([
	...
	function _commonAssertsForTableWithNoFilter(api, tableRepresentation, assert, requiredParameter) {
		//arrange
		var thumbnailContent = tableRepresentation.getThumbnailContent();
	...
	QUnit.test("When getThumbnailContent is called for the table and there is no data", function (assert) {
		this.tableRepresentation.aDataResponse = [];
		this.tableRepresentation.setData(this.tableRepresentation.aDataResponse, undefined); // no data
		var thumbnailContent = this.tableRepresentation.getThumbnailContent();
		...
		assert.equal(exportToExcelButton.getEnabled(),false,"Then Export to Excel Button is disabled when there is no data");
		assert.strictEqual(thumbnailContent.getContent()[0].getText(), this.api.oApi.getTextNotHtmlEncoded("noDataText"), "Then getThumbnailContent returns no data text");
	});
