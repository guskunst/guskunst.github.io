...
sap.ui.define("sap/apf/ui/representations/tScatterPlotChart", [
	...
	function _commonInitializationAsserts(chart, assert) {
		...
		assert.strictEqual(chart.thumbnailChart.getVizType(), "scatter", "Then thumbnailcontent has scatterplot chart");
	}
	...
	QUnit.test("When Scatter Plot Chart is initialized", function(assert) {
		//assert
		...
		vizPropOnThumbnailChart.plotArea.colorDepth = 1;  // one dimension with a kind: color
		...
		assert.deepEqual(this.chart.thumbnailChart.getVizProperties(), vizPropOnThumbnailChart, "Then vizProperties are applied to the thumbnail chart");
	});
	...
	QUnit.test("When Scatter Plot Chart is initialized with multiple parameters", function(assert) {
		...
		var thumbnailContent = this.chart.getThumbnailContent();
		...
		assert.ok(this.chart.thumbnailChart instanceof VizFrame, "Then thumbnail chart is instance of a vizframe");
		assert.strictEqual(mainContent.getVizType(), "scatter", "Then mainContent is a scatterplot chart");
		assert.ok(thumbnailContent instanceof HorizontalLayout, "Then thumbnailContent is a layout");
		assert.strictEqual(this.chart.thumbnailChart.getVizType(), "scatter", "Then thumbnailcontent has scatterplot chart");
