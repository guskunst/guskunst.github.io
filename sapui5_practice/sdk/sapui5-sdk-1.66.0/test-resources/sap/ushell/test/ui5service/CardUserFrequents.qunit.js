...
sap.ui.require([
    ...
    "sap/ushell/EventHub",
    "sap/ushell/Config"
], function (jQuery, CardUserFrequents, CardUserRecentsBase, EventHub, Config) {
    "use strict";

    QUnit.start();
    QUnit.module("The constructor", {
        beforeEach: function () {
            this.constructorCallStub = sinon.stub(CardUserRecentsBase, "call");
            this.oCardUserFrequents = new CardUserFrequents();
        },
        afterEach: function () {
            this.constructorCallStub.restore();
        }
    });

    QUnit.test("Calls the parent class constructor", function (assert) {
        assert.strictEqual(this.constructorCallStub.callCount, 1, "The constructor called the parent constructor once.");
        assert.ok(this.constructorCallStub.calledWithExactly(this.oCardUserFrequents, "CardUserFrequents"), "The constructor called the parent constructor with the right parameters.");
    });
