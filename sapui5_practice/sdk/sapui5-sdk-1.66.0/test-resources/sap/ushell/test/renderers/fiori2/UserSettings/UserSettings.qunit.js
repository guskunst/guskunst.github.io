...
sap.ui.require([
        ...
        [
            {
                ...
                sExpectedId: "userSettingsErrorPage"
            },
            {
            ...
            oPromise.then(function (sId) {
                if (oFixture.oEntry.contentFunc) {
                    assert.strictEqual(sId, oFixture.sExpectedId, "The returned id is correct when " + oFixture.sTestDescription);
                } else {
                    assert.strictEqual(sId.substring(0, 6), oFixture.sExpectedId, "The returned id is correct when " + oFixture.sTestDescription);
                }
                fnDone();
            });
        });
    ...
    QUnit.test("_handleSettingsSave", function (assert) {
        [
            ...
            assert.strictEqual(fnHandleSettingsDialogClose.called, true, "User Settings Dialog closes gracefully when " + oFixture.sTestDescription);
    ...
    QUnit.test("_toDetail", function (assert) {
        // Arrange
        var fnDone = assert.async();
        var fnCreateEntryContent = sinon.stub(oUserSettingsController, "_createEntryContent").returns(Promise.resolve("userSettingsErrorPage")),
        ...
        window.setTimeout(function () {
            ...
            assert.strictEqual(fnNavToDetail.args[0][0], "userSettingsErrorPage", "1. paramter of call to _navToDetail was correct.");
