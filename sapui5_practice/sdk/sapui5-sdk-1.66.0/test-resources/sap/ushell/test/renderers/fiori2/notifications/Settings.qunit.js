...
(function () {
        ...
        setup: function () {
            ...
            sap.ushell.bootstrap("local").then(function () {
                ...
                getUserSettingsFlagsStub.returns({
                        ...
                        fCallback({previewNotificationEnabled : false, highPriorityBannerEnabled : true});
                    }
                });
                start();
            });
    ...
    test("Init settings view - model initialization", function (assert) {
        ...
        assert.ok(oController.getView().getModel().getProperty("/flags/previewNotificationEnabled") === false, "Correct 1st switch flag in the model");
        assert.ok(oController.getView().getModel().getProperty("/originalFlags/previewNotificationEnabled") === false, "Correct 1st original switch flag 1 in the model");
    ...
    test("OnAfterRendering - verify copy of rows and flags to originalRows and originalFlags", function (assert) {
        ...
        oController.getView().getModel().setProperty("/flags", {previewNotificationEnabled : false, highPriorityBannerEnabled : false});
        ...
        assert.ok(oController.getView().getModel().getProperty("/originalFlags").previewNotificationEnabled === false, "After onAfterRendering: 1st original flag is correct");
    ...
    test("onCancel - verify copy of original data to rows and flags", function (assert) {
        var oOriginalFlags = {
                previewNotificationEnabled : false,
        ...
        assert.ok(oController.getView().getModel().getProperty("/flags").previewNotificationEnabled === oOriginalFlags.previewNotificationEnabled, "After onCancel: 1st flag is correct");
        ...
        assert.ok(oController.getView().getModel().getProperty("/originalFlags").previewNotificationEnabled === undefined, "no 1st originalFlag");
    ...
    test("switchFlagsSave - Notification service called and event published", function (assert) {
        var oView = new sap.ui.view({
                type: sap.ui.core.mvc.ViewType.JS,
                viewName: "sap.ushell.components.shell.Notifications.Settings"
            }),
        ...
        oController.getView().getModel().setProperty("/flags/previewNotificationEnabled", true);
        oController.getView().getModel().setProperty("/flags/highPriorityBannerEnabled", true);
        oController.getView().getModel().setProperty("/originalFlags/previewNotificationEnabled", false);
        oController.getView().getModel().setProperty("/originalFlags/highPriorityBannerEnabled", true);

        oController._handleSwitchFlagsSave();

        assert.ok(oNotificationServiceSetFlagsStub.calledOnce === true, "Notification Service function setUserSettingsFlags called once");
        assert.ok(oEventBusPublishStub.calledOnce === true, "One event published");
        assert.ok(oEventBusPublishStub.args[0][1] === "enablePreviewNotificationChanged", "Published event: enablePreviewNotificationChanged");
    ...
    test("switchFlagsSave - Flags equal original flags", function (assert) {
        ...
        oController.getView().getModel().setProperty("/flags/previewNotificationEnabled", false);
