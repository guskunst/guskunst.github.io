...
sap.ui.require([
...
], function (jQuery, EventHub, Config, AppConfiguration, ActionItem, Button, Bar, AppFinderController, AppLifeCycle, HeaderManager) {
    "use strict";

    /*global QUnit, stop, jQuery, sap, sinon, window */
   ...
   QUnit.test("enable search", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            oRenderer.logRecentActivity(oRecentEntry).then(function () {
                oUserRecentsSrvc.getRecentActivity().done(function (aActivity) {
                    ...
                    assert.strictEqual(aActivity[0].appType, "App");
     ...
     */
    QUnit.test("Floating cotnainer - test setFloatingContainerVisible", function (assert) {
        var done = assert.async(),
            oShellView,
            oUnifiedShell,
