...
sap.ui.require([
...
], function (testUtils, Device, AppLifeCycle, Container, EventHub, Config, JSONModel, BusyDialog, ShellNavigation, Storage, RelatedServices, Ui5History, Ui5HistoryDirection, HeaderManager, Message, SchedulingAgent) {
    "use strict";
    /* eslint-disable */ // TBD: make ESLint conform

    /* global hasher QUnit OData jQuery sap sinon window Promise */
    ...
    function stubHashChangeHandler () {
        // In the Shell-view there is a 6 seconds setTimeout that triggers the
        // HashChangeHandler#handle method. This method makes use of objects
        // that have been deleted, before the 6 seconds, when the test is over
        // (sap.ushell.Container for example).  If we don't stub the handle
        // method, an error appears after QUnit is done with a successful test
        // execution...
        //
        // 1. require in advance to be able to stub, but after bootstrap (see
        //    where this method is called), as it needs sap.ushell.Container to
        //    exist
        jQuery.sap.require("sap.ushell.renderers.fiori2.search.HashChangeHandler");

        // 2. stub method only one time and leave stubbed, because it's
        //    important that the logic does not trigger after the setTimeout
    ...
    QUnit.test("test _getAppContainer called with shell UI service", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            sinon.stub(AppLifeCycle.getShellUIService(), "getInterface").returns({
    ...
    QUnit.test("test _reorderUserPrefEntries", function (assert) {
        ...
        var originalEntries = entries.slice();
        var reorderedEntries = oController._reorderUserPrefEntries(entries);
    ...
    QUnit.test("test _activateFloatingUIActions", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            var oController = sap.ui.getCore().byId("mainShell").getController();
            var enableFunction = sinon.spy();
            var disableFunction = sinon.spy();
    ...
    QUnit.test("test_movingOfFLpActionsToShellHeaderInModel", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            var aStates = oElementsModel.getAllStatesInDelta();


            aStates.forEach(function (sState) {
                var aDeltaActions = oElementsModel.getCustomStateDeltaMember(sState, "actions");
                var aBaseStateActions = oElementsModel.getBaseStateMember(sState, "actions");
                var aBaseStateHeadEndItems = HeaderManager._getBaseStateMember(sState, "headEndItems");
    ...
    QUnit.test("Test floating container docking functionality for Desktop devices", function (assert) {
        ...
        var oGetCurrentRangeStub = sinon.stub(Device.media, "getCurrentRange").returns({
            name: "Desktop"
        });

        var xPositionRightDock = jQuery(window).width() - 63;
        var xPositionLeftDock = 63;
        var xPositionUnDock = 100;
        var rightDockingClassName = "sapUshellShellDisplayDockingAreaRight";
        var leftDockingClassName = "sapUshellShellDisplayDockingAreaLeft";

        var cfg = {
            moveX : xPositionRightDock,
            clone : {
                parentElement : jQuery('<div id="parentElementTest">')
            }
        };
        ...
        var oCloseDockingAreaSpy = sinon.spy(oController, "_closeDockingArea");
        var oIsDockingAreaOpenSpy = sinon.spy(oController, "_isDockingAreaOpen");
        ...
        QUnit.module("sap.ushell.renderers.fiori2.Shell", {
            beforeEach: function (assert) {
                ...
                sap.ushell.bootstrap("local").then(function () {
                    ...
                    oController = new sap.ui.controller("sap.ushell.renderers.fiori2.Shell");
                    oController.oNavigationHistoryMonitor = sinon.stub().returns({
                        reset: sinon.stub()
                    });
                    ...
                    that._oOriginalCurrentApplication = sap.ushell.services.AppConfiguration.getCurrentApplication();
            ...
            afterEach: function () {
                ...
                if (oController._openAppInNewWindow) {
                    testUtils.restoreSpies(
                        oController._openAppInNewWindow
                    );
                }
                ...
                sap.ushell.services.AppConfiguration.setCurrentApplication(this._oOriginalCurrentApplication);
            }
        });
    ...
    [
        ...
        },
        {
            ...
            expectedConfiguration: {
                // we do not allow other application types...
            }
        },
        {
            ...
            expectedConfiguration: {
                // we do not allow TR, correct option is GUI
            }
        }
    ].forEach(function (oFixture) {
    ...
    QUnit.test("test handleNavMenuTitleVisibility", function (assert) {
        var done = assert.async();
        var oTestMediaRange = {name: 'Phone'},
        ...
        AppLifeCycle.init("home", oController.oViewPortContainer, "shell-home", false, {
            ownerComponent: "test"
        });
    ...
    QUnit.test("test getAnimationType", function (assert) {
        var animations = oController.getAnimationType();
        assert.strictEqual(animations, "show", 'Verify animations type');
    ...
    QUnit.test("test togglePane", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            assert.strictEqual(oEvent.getSource().getModel().setProperty.callCount, 1, 'Event source model set properties called 1 time');
            assert.strictEqual(oEvent.getSource().getModel().setProperty.args[0][0], '/currentState/showCurtainPane', 'Event source model set properties called with /currentState/showCurtainPane');

            //id != categories button
            oEvent.getParameter = sinon.stub().returns("other");
            oController.togglePane(oEvent);
            assert.strictEqual(oEvent.getSource().getModel().setProperty.callCount, 2, 'Event source model set properties called 1 time');
            assert.strictEqual(oEvent.getSource().getModel().setProperty.args[1][0], '/currentState/showPane', 'Event source model set properties called with /currentState/showPane');
    ...
    [
      ...
      * Note that the new window is started without component creation
      * and newWindow starting
      */
     {
     ...
     QUnit.test("doHashChange loads application component correctly when " + oFixture.testDescription, function (assert) {
        ...
        var oNTRSResolveHashFragmentStub = sinon.stub(),
         ...
         AppLifeCycle.init("home" ,oViewPortContainer, "shell-home", false, {
             ownerComponent: "test"
         });
    ...
    QUnit.test("test doHashChange with trampolin app", function (assert) {
        ...
        var oShellNavigationFake = {
            toExternal : sinon.spy()
        };
        ...
        var oHandle =  {
                getInstance : function () { return oComponent;}
        };
    ...
    QUnit.test("_resolveHashFragment: forces the root intent to 'embedded' navigationMode", function (assert) {
        var done = assert.async();
        sinon.stub(oController, "_getConfig").returns({
            rootIntent: "Root-intent"
        });
    ...
    QUnit.test("test _loadCoreExtNonUI5 navigation ", function (assert) {
        ...
        oController._loadCoreExtNonUI5("", "", {
            applicationType: "TR"
        });
        oClock.tick(3000);
        assert.strictEqual(loadExtSpy.called, true, "TR hashChangeFailure was not called");
        oClock.restore();
        loadExtSpy.restore();
    });
    ...
    QUnit.test("test not supported type navigation ", function (assert) {
        ...
        oClock.tick(3000);
        assert.strictEqual(loadExtSpy.called, false, "hashChangeFailure was called");
        oClock.restore();
        loadExtSpy.restore();
    });
    ...
    QUnit.test("test NWBC navigation ", function (assert) {
        ...
        oClock.tick(3000);
        assert.strictEqual(loadExtSpy.called, true, "NWBC hashChangeFailure was not called");
        oClock.restore();
        loadExtSpy.restore();
    });
    ...
    QUnit.test("test navigate - navigationMode change", function (assert) {
        ...
        // oNavMode = oResolvedHashFragment.navigationMode;
        ...
        oController.navigate(oParsedShellHash, sFixedShellHash, oMetadata, oResolvedHashFragment);
        assert.strictEqual(navSpy.callCount, 1, "Navigate called twice");
        oResolvedHashFragmentParam = oResolvedHashFragment; // oController.navigate.args[1][3];
        // oNavMode = oResolvedHashFragmentParam.navigationMode;
    ...
    (function () {
        ...
        [
            ...
            QUnit.test("#_handleEmbeddedNavMode sets active containers as expected, when " + oFixture.testDescription, function (assert) {
                ...
                var oMetadata = oStubs.oMetadata;
                var oResolvedHashFragment = oStubs.oResolvedHashFragment;
                var sFixedShellHash = oStubs.sFixedShellHash;
                //var reuseApplicationContainerStub = sinon.stub(AppLifeCycle, "reuseApplicationContainer").returns();

                AppLifeCycle.init("home", oController.oViewPortContainer, "shell-home", false, {
                    ownerComponent: "test"
                });
                oController._handleEmbeddedNavMode(sFixedShellHash, oParsedShellHash, oMetadata, oResolvedHashFragment);
        ...
        [
            ...
            QUnit.test(oFixture.testDescription, function (assert) {
                ...
                var oMetadata = oStubs.oMetadata;
                var oResolvedHashFragment = oStubs.oResolvedHashFragment;
                var oInnerControl = oStubs.oInnerControl;
                var sFixedShellHash = oStubs.sFixedShellHash;
    ...
    QUnit.test("_disableSourceAppRouter: ", function (assert) {
        [
            ...
            {
                sTestDescription: "an empty parameter is removed",
    ...
    [
        ...
        },
        {
            ...
            testIsDirty: true,
            testUi5HistoryAPIDirection: "NewEntry",
        ...
        },
        {
            ...
            testIsDirty: true,
            testUi5HistoryAPIDirection: "Backwards",
            ...
            expectedHandlerResult: "Custom",
            expectedHistoryBackCalls: 0,
            expectedSetHashCalls: 1  // restore via set hash
        },
        {
            testDescription: "user cancels data loss popup after unknown navigation direction + monitor says history entry was added after last navigation",
            testIsDirty: true,
            testUi5HistoryAPIDirection: "Unknown",

            bConfirmDirtyFlagPopup: false,
            vMonitorHistoryEntryAdded: true,
        ...
        {
            testDescription: "user cancels data loss popup after unknown navigation direction + monitor says history entry was not added after last navigation",
            testIsDirty: true,
            testUi5HistoryAPIDirection: "Unknown",

            bConfirmDirtyFlagPopup: false,
            vMonitorHistoryEntryAdded: false,
        ...
        QUnit.test("test data loss navigation handler when " + oFixture.testDescription, function (assert) {
            ...
            var setDirtyFlag = sinon.stub(sap.ushell.Container, "setDirtyFlag");
            var setHash = sinon.stub(window.hasher, "setHash");
            var navBack = sinon.stub(window.history, "back");

            var fOrigConfirm = window.confirm;
            window.confirm = sinon.stub().returns(oFixture.bConfirmDirtyFlagPopup);

            var oGetDirectionStub = sinon.stub();
            oGetDirectionStub.returns(oFixture.testUi5HistoryAPIDirection);
            sinon.stub(Ui5History, "getInstance").returns({
                getDirection: oGetDirectionStub
            });

            sinon.stub(Storage, "get");  // called during destroy, if this test runs individually

            oController.oNavigationHistoryMonitor = {
                wasHistoryEntryAdded: sinon.stub().returns(oFixture.vMonitorHistoryEntryAdded),
                reset: sinon.stub(),
                destroy: sinon.stub()
            };
            ...
            assert.strictEqual(navBack.callCount, oFixture.expectedHistoryBackCalls, "window.history.back was called the expected number of times");
            assert.strictEqual(setHash.callCount, oFixture.expectedSetHashCalls, "setHash was called the expected number of times");
            ...
            if (oFixture.bConfirmDirtyFlagPopup) {
                assert.deepEqual(oGetDirectionStub.getCall(0), null, "UI5 History API was called as expected");
            } else {
                assert.deepEqual(oGetDirectionStub.getCall(0).args[0], "new-hash", "UI5 History API was called as expected");
            }
            ...
            delete oController.oNavigationHistoryMonitor;
    ...
    QUnit.module("sap.ushell.renderers.fiori2.Shell (Part 2)", {
        ...
        QUnit.test("test ShellUIService event provider", function (assert) {
            ...
            assert.strictEqual(oController.isBackNavigationChanged, true, "onBackNavigationChange was called");
        });
        ...
        QUnit.test("test ShellUIService back navigation handler", function (assert) {
            ...
            var publishExternalStub = sinon.stub(sap.ushell.renderers.fiori2.utils,"publishExternalEvent");
            var getWrappedApplicationWithMoreStrictnessInIntentionStub = sinon.stub(oController, "getWrappedApplicationWithMoreStrictnessInIntention").returns({
                getId: function () {},
                getActive: sinon.stub(),
                setActive: sinon.stub()
            });
            var oBackNavigationStub = sinon.stub(RelatedServices, "_historyBackNavigation");
        ...
        QUnit.test("test ShellUIService changes model on app state", function (assert) {
            var done = assert.async();
            var homeHierarcy, homeTitle, homeRelatedApps;
            ...
            fnWaitConfigChange()
                ...
                .then(fnWaitConfigChange)
                .then(function () {
                    AppLifeCycle.init("app", undefined, "Shell-home", false, {
                        ownerComponent: "test"
                    });
                ...
                .then(fnWaitConfigChange)
                .then(function () {
                   /* assert.deepEqual(oController.getModel().getProperty("/currentState/application/hierarchy"), oExpectedHeirarchy, "oHierarchy was updated in the model properly");
                    assert.deepEqual(oController.getModel().getProperty("/currentState/application/title"), sTitle, "title was updated in the model properly");
                    assert.deepEqual(oController.getModel().getProperty("/currentState/application/relatedApps"), oRelatedApps, "oRelatedApps was updated in the model properly");*/
      ...
      { description : "returns undefined",  promise : false, expectedResult : "fail"}
        ...
        QUnit.test("_usesNavigationRedirect decision with nav redirect" + oFixture.description, function (assert) {
            ...
            if (!oFixture.noFunction) {
                oComponent.navigationRedirect = function () {
                    return oNavResResult;
                };
            }
            var oHandle = {
                  getInstance : function () {
    ...
    QUnit.test("The event loadRendererExtensions triggers the loading of the Renderer Plugins", function (assert) {
        ...
        var oPluginManagerStub = {
            loadPlugins: function () {return {
                always: function (handler) {
                    handler();
                }
            }}
        }
        var oPromise = Promise.resolve(oPluginManagerStub);
        var oGetServiceAsyncStub = sinon.stub(sap.ushell.Container, "getServiceAsync").returns(oPromise);
    ...
    QUnit.test("The event loadUsageAnalytics triggers the loading of the Usage Analytics", function (assert) {
        var done = assert.async();
        var oPromise = Promise.resolve({init: function() {}});
        var oGetServiceAsyncStub = sinon.stub(sap.ushell.Container, "getServiceAsync").returns(oPromise);
    ...
    QUnit.module("sap.ushell.renderers.fiori2.Shell.controller onAppOpened", {
        beforeEach: function (assert) {
            ...
            var oGetServiceAsyncPromise = jQuery.Deferred().resolve(oURLParsingStub).promise();
            ...
            this.oSpy = sinon.spy(this.oController, "logOpenAppAction");
        },
        afterEach: function () {
            this.oSpy.restore();
    ...
    QUnit.test("creates correct sAppPart if getAppHash returns non-empty string", function (assert) {
        this.getAppHashStub.returns(this.sAppHash);
        this.oController.onAppOpened("sChannelIdTest", "sEventIdTest", this.oResolvedHashFragment);
        assert.strictEqual(
            this.oSpy.withArgs(this.sFixedShellHash, this.oResolvedHashFragment, "&/" + this.sAppHash).calledOnce,
            true
        );
    });
    ...
    QUnit.test("creates correct sAppPart if getAppHash returns empty string", function (assert) {
        ...
        this.oController.onAppOpened("sChannelIdTest", "sEventIdTest", this.oResolvedHashFragment);
        assert.strictEqual(
            this.oSpy.withArgs(this.sFixedShellHash, this.oResolvedHashFragment, null).calledOnce,
            true
        );
    });
    ...
    QUnit.module("sap.ushell.renderers.fiori2.Shell.controller doHashChange", {
        beforeEach: function (assert) {
            ...
            sinon.stub(AppLifeCycle, "getInMemoryInstance").returns({
            ...
            this.oController.oNavigationHistoryMonitor = {
                reset: sinon.stub()
            };

            /*
             * AppConfiguration memoized metadata about the current
             * application. We need to simulate here that each test is uniquely
             * saved in the app configuration service memory. Therefore we mock
             * _processKey, which is the function responsible to extract the
             * key to use for the memoization. By mocking in this way we ensure
             * uniqueness of this key.
             */
    ...
    [
        ...
        QUnit.test("doHashChange new window when " + oFixture.testDescription, function (assert) {
            ...
            assert.strictEqual(
                this.oLogOpenAppActionSpy.withArgs(
                    this.sFixedShellHash,
                    this.oResolvedHashFragment,
                    oFixture.sAppPart).calledOnce,
                true
            );
            assert.strictEqual(
                this.oOpenAppInNewWindowAndRestoreSpy.withArgs(
                    this.oResolvedHashFragment).calledOnce,
                true);
        });
    });
