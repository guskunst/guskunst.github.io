...
sap.ui.require([
    ...
    [
        ...
        QUnit.test("recalculateState: " + name, function (assert) {
            ...
            HeaderManager.recalculateState(oTestCase.stateName);
    ...
    QUnit.test("test validateShowLogo", function (assert) {
        ...
        fnWaitConfigChange().then(function () {
            return HeaderManager.recalculateState("home");
            ...
            isShowLogo =  Config.last("/core/shellHeader/showLogo");
            assert.ok(isShowLogo, 'Verify Logo exist on tablet');
        }).then(function () {
            oTestMediaRange.name = 'combi';
            HeaderManager.validateShowLogo();
        }).then(fnWaitConfigChange).then(function () {
            isShowLogo =  Config.last("/core/shellHeader/showLogo");
            assert.ok(isShowLogo, 'Verify Logo exist on combi');
        }).then(function () {
    ...
    QUnit.test("handleEndItemsOverflow:", function (assert) {
        [
            ...
            var aResult = oFixture.aEndItems,
                ...
                fnGetCurrentRangeStub = sinon.stub(Device.media, "getCurrentRange").returns({name: oFixture.sDeviceType}),
            ...
            // Act
            HeaderManager.handleEndItemsOverflow();
