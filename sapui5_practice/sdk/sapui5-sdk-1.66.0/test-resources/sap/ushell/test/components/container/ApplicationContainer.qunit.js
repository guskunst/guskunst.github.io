...
sap.ui.require([
    ...
    "sap/ushell/library" // makes sap.ushell.components.container.ApplicationType available
], function (testUtils, utils, hasher) {
    "use strict";
    /*global asyncTest, deepEqual, dispatchEvent, equal, localStorage, module,
    ok, throws, start, strictEqual, stop, test, QUnit,
    document, jQuery, sap, sinon, some, Storage, window, Promise, URI */

    jQuery.sap.require("sap.ushell.services.Container");
    jQuery.sap.require('sap.ushell.components.container.ApplicationContainer');
    jQuery.sap.require("sap.ushell.User");

    var A_ALL_APPLICATION_TYPES = Object.keys(sap.ushell.components.container.ApplicationType).map(function (sKey) {
        return sap.ushell.components.container.ApplicationType[sKey];
    });
     ...
     *   the path to the function of the config to stub
     * @param {variant} vValue
     *   the value to assign to the given path
     * @return {object}
     *   a Sinon stub for the jQuery.sap.getObject.
     *   The stub will be configured to return the given value if
     *   jQuery.sap.getObject is called with the given key. The stub is
     *   configured to throw, if the getObject call is made with an
     *   unconfigured path.
     */
     ...
     *    }
     *
     *   Stubs and default values are used when one or more of the above are
     *   passed.
     *
     * @return {object}
     *   A mocked sap.ushell.Container (to be assigned to the global)
     */
     ...
     *  the container to call _render on
     *
     * @param {object} oPrepareEnv
     *  an environment useful for calling <code>#renderInternally</code>. It's
     *  an object containing instances like: {
     *     renderManager: <instance>,
     *     rendererInterface: <instance>
     *  }
     *
     * @param {string} sUrl
     *  the url property of the container
     *
     * @param {string} sApplicationType
     *  the application type
     *
     * @return {object}
     *  a DIV node containing the rendered iFrame
     */
    function renderInternally (oContainer, oPrepareEnv, sUrl, sApplicationType) {
     ...
     * @param {string} sUrl
     *   the application URL
     * @param {object} [oMockConfigOverrides]
     *   optional overrides
     *   for the mock configuration(see code).This is an object containing paths to override like:
     *   <pre>
     *   {
    ...
    function prepareRenderInternally (oContainer, sUrl, oMockConfigOverrides) {
        ...
        var oMockConfig = testUtils.overrideObject({
            appContainer: {
                fnGetFrameSource: sinon.stub().returns(sUrl)
            },
            utils: {
                bStubLocalStorageSetItem: true
            }
     ...
     *
     * @returns {object}
     *   the rendered iFrame if any or undefined.
     */
    function renderInternallyAndExpectIFrame (oContainer, sApplicationType, sUrl, oMockConfigOverrides) {
        // Arrange
        ...
        // Assert
        testIFrameRendered(oTargetNode, oPrepareEnv, oContainer, sApplicationType, sUrl);

        var oIFrame = oTargetNode.childNodes[0];
     ...
     *   The url
     */
    function testIFrameRendered (oTargetNode, oPrepareEnv, oContainer, sApplicationType, sUrl) {
        var oIframe;
     ...
     *
     * @param {object} oTargetNode
     *   The target Node possibly containing an iframe
     */
    function testIFrameNotRendered (oTargetNode) {
        if (!oTargetNode || !oTargetNode.childNodes || oTargetNode.childNodes.length !== 1) {
    ...
    [
        ...
        QUnit.test("ApplicationContainer#setNewApplicationContext when " + oFixture.description, function (assert) {
            ...
            var setNewApplicationContext = sap.ushell.components.container
                .ApplicationContainer.prototype
        ...
        [
            ...
            QUnit.test("ApplicationContainer#onApplicationOpened: " + oFixture.testDescription, function (assert) {
                ...
                var iNow = Date.now();
                var fnDone = assert.async();
                var oClock = sinon.useFakeTimers(iNow);
    ...
    QUnit.test("ApplicationContainer#setNewTRApplicationContext", function (assert) {
        ...
        // Act
        setNewTRApplicationContext(sInputUrl).then(function () {
            // Assert
            assert.equal(
                oAppContainer.getDomRef.calledOnce,
                true,
    ...
    test("ApplicationContainer control", function () {
        ...
        asyncTest("ApplicationContainer defines all its private aggregations in the design time metadata definition", function () {
            ...
            var aAllPrivateAggregations = Object.keys(oAppContainer.getMetadata().getAllPrivateAggregations());
            var oMetadata = oAppContainer.getMetadata();
    ...
    test("sap.ushell.components.container.render invalid type", function () {
        ...
        oAppContainer._render(oRenderManager,
    ...
    [
        ...
        {type: sap.ushell.components.container.ApplicationType.WDA, additionalInfo: undefined, configuration: undefined},
    ...
    asyncTest("ApplicationContainer logout de-/registration", function () {
        sap.ushell.bootstrap("local").done(function () {
            ...
            renderInternallyAndExpectIFrame(oAppContainer,
                ...
                'http://anyhost:1234/sap/bc/ui2/NWBC/~canvas?sap-client=120',
                oMockOverrides);
            ...
            ["NWBC", "TR"].forEach(function (sNwbcLikeApplicationType) {
                renderInternallyAndExpectIFrame(oAppContainer,
                    ...
                    'http://anyhost:1234/sap/bc/ui2/' + sNwbcLikeApplicationType + '/~canvas',
                    oMockOverrides);
                fnLogout = sap.ushell.Container.attachLogoutEvent.args[0][0];
...
//    });
    ...
    //        // function to be tested
    //        var oApplicationContainer = {
    //            getApplicationType: sinon.stub().returns(sap.ushell.components.container.ApplicationType.URL),
    //            getDomRef: sinon.stub().returns({
    //                tagName: "IFRAME"
    //            }),
    ...
    [
        ...
        },
        {
            ...
            expectedPublicApiCallArgs: {
                ...
                params: {
                    "sap-ui2-wd-app-id": "WDR_TEST_PORTAL_NAV_TARGET"
                    // sap-system is removed completely
                }
            },
            expectedResponse: {
                status: "success"
        ...
        test("handleServiceMessageEvent toExternal: " + oFixture.testDescription, function (assert) {
            ...
            sap.ushell.bootstrap("local").done(function () {
                ...
                sap.ushell.Container = mockSapUshellContainer({
                    services: {
                        CrossApplicationNavigation: oFakeCrossApplicationNavigation
                    },
    ...
    test("handleServiceMessageEvent ShellUIService backToPreviousApp", function (assert) {
        ...
        sap.ushell.bootstrap("local").done(function () {
            ...
            oGetServiceStub
                ...
                .returns({
                    backToPreviousApp: oBackToPreviousAppStub
                });
    ...
    test("handleServiceMessageEvent ShellUIService backToPreviousApp (via sap.ushell.CrossApplicationNavigation.backToPreviousApp)", function (assert) {
        var done = assert.async();
        sap.ushell.bootstrap("local").done(function () {
            //test data
            var oMessage = getServiceRequestMessage({
                    service : "sap.ushell.CrossApplicationNavigation.backToPreviousApp",
                    body: {}
                }),
                oMessageData = JSON.parse(oMessage.data);
    ...
    [
        ...
        test("handleServiceMessageEvent ShellUIService setBackNavigation when " + oFixture.testDescription, function (assert) {
            /*
            * Tests that the sap.ushell.ui5service.ShellUIService.setBackNavigation is
            * handled correctly when made by the GUI. To do this, the test simulates a
            * setBackNavigation call made via postMessage by the GUI application.
            */
            var done = assert.async();
            sap.ushell.bootstrap("local").done(function () {
                var oMessage = getServiceRequestMessage({
                ...
                sinon.stub(oAppContainer, "getShellUIService").returns({
                    setBackNavigation: oSetBackNavigationStub
                });
        ...
        test("ApplicationContainer localStorage eventing", function (assert) {
            ...
            sap.ushell.bootstrap("local").done(function () {
                ...
                renderInternallyAndExpectIFrame(oAppContainer,
                    ...
                    'http://anyhost:1234/sap/bc/ui2/' + sNwbcLikeApplicationType + '/~canvas?sap-client=120',
                    oMockOverrides);
    ...
    test("ApplicationContainer rendering on inactive container", function () {
        ...
        oAppContainer.setApplicationType(sap.ushell.components.container.ApplicationType.TR);
        ...
        testIFrameNotRendered(oTargetNode);
    });
    ...
    [
        ...
        test("isTrustedPostMessageSource works as expected whith "  + oFixture.testDescription, function () {
            ...
            delete oMessage.data.type;
            oMessage.data = JSON.stringify(oMessage.data);
            sinon.spy(sap.ushell.components.container.ApplicationContainer.prototype, "_isTrustedPostMessageSource");

            var oContentWindow = {
                name: "oContentWindow"
            };

            var otherContentWindow = {
                name: "otherContentWindow"
            };
