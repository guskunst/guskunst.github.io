...
sap.ui.require([
...
], function (ThemeSelectorController, Controller, Component) {
    "use strict";
    ...
    QUnit.test("Calls the function writeToPersonalization if the animation mode changed", function (assert) {
        // Arrange
        var oPromiseMock = makePromiseMock();
        sinon.stub(this.oController, "onSaveThemes").returns(makePromiseMock());

        this.oController.originalAnimationModeKey = "A";
        this.oController.currentAnimationModeKey = "B";
        ...
        assert.strictEqual(oWriteToPersonalizationStub.firstCall.args[0], "flp.launchpad.animation.mode", "The writeToPersonalization function has been called with the correct parameter.");
        assert.strictEqual(oWriteToPersonalizationStub.firstCall.args[1], "animationMode", "The writeToPersonalization function has been called with the correct parameter.");
        assert.strictEqual(oWriteToPersonalizationStub.firstCall.args[2], "B", "The writeToPersonalization function has been called with the correct parameter.");
        assert.strictEqual(oPromiseMock.done.callCount, 1, "The done function has been called once.");
        assert.strictEqual(oPromiseMock.fail.callCount, 1, "The fail function has been called once.");
    });
