...
sap.ui.require([
...
], function (HomepageManager, EventHub) {
    "use strict";
    /*global asyncTest, equal, module,
     ok, start, stop, test,
     jQuery, sap, sinon, assert */
    ...
    test("test publishMoveActionEvents function", function () {
        ...
        oHomepageManager.publishMoveActionEvents.call(oMockContext, oEvent, aContexts, "movetile");
        ...
        ok(oScrollToGroupParams[2].hasOwnProperty("focus"), "event object has property focus");
        ok(oScrollToGroupParams[2].hasOwnProperty("groupChanged"), "event object has property groupChanged");
    ...
    test("reset groups after failure", function () {
        oHomepageManager = new HomepageManager("homepageMgr", {model: new sap.ui.model.json.JSONModel(mockData)});
        var oLoadGroupsStub = sinon.stub(oHomepageManager, "loadGroupsFromArray"),
            oHelper = sap.ui.require("sap/ushell/components/MessagingHelper"),
            oRequestFailedStub = sinon.stub(oHelper, "showLocalizedError"),
            oClock = sinon.useFakeTimers();

        oHomepageManager._resetGroupsOnFailureHelper("msg")();
        oClock.tick(100);
    ...
    asyncTest("verify link tile loaded correctly", function () {
        ...
        oHomepageManager.loadGroupsFromArray(mockData.groups);
    ...
    test("verify the order of setting the group model", function (assert) {
        ...
        setTimeout(function () {
            ...
            assert.equal(oHomepageManager.aGroupsFrame.length, mockData.groups.length + 1 - 1, "aGroupsFrame should be reset");
            EventHub.emit("firstSegmentCompleteLoaded", true);
            setTimeout(function () {
                //+1 because default group
                assert.equal(oHomepageManager.oModel.getProperty("/groups").length, mockData.groups.length + 1, "The rest groups is set after firstSegmentCompleteLoaded");
    ...
    test("verify that firstSegmentCompleteLoaded is emitted if there is no blind loading", function (assert) {
        ...
        setTimeout(function () {
            assert.ok(oHomepageManager.bIsFirstSegmentViewLoaded, "The tile view is loaded during loadGroupsFromArray if there is no blind loading");
    ...
    test("Verify featured groups are on the top of groups", function () {
        ...
        fGetDefaultGroupStub.returns(jQuery.Deferred().resolve(mockData.groups[0]));
        oHomepageManager.loadGroupsFromArray(mockData.groups.concat(aFeaturedGroups));
        aGroups = oHomepageManager.getModel().getProperty("/groups");
...
//        oHomepageManager.loadGroupsFromArray(mockData.groups);
    ...
    test("Verify that the header is not hidden if there is only 1 visible group", function (assert) {
        ...
        fGetDefaultGroupStub.returns(jQuery.Deferred().resolve(mockData.groups[0]));
        oHomepageManager.loadGroupsFromArray([]);
    ...
    test("getTileView - load tiles", function () {
    ...
    test("_getTileModel - returns the correct model when a tile is provided", function () {
        var oUidStub,
            oGetTileSizeStub,
            oGetTileIdStub,
        ...
        oHomepageManager = new HomepageManager("homepageMgr", {model: new sap.ui.model.json.JSONModel({groups: {}})});
        oGetIsAppBoxStub = sinon.stub(oHomepageManager, "_getIsAppBox").returns(true);
        oGetPropertyStub = sinon.stub(oHomepageManager.oModel, "getProperty").returns(false);

        // Act
        oReturnValue = oHomepageManager._getTileModel(oDummyTile, bIsGroupLocked, sTileType);

        // Assert
        assert.deepEqual(oExpectedReturnValue, oReturnValue, "The tile model contains the expected data");
    ...
    test("_getTileModel - returns the correct model when a link is provided", function () {
        var oUidStub,
            oGetTileSizeStub,
            oGetTileIdStub,
            oGetCatalogTileIdStub,
        ...
        // Assert
        assert.ok(oReturnValue.content[0] instanceof sap.m.GenericTile, "Generic Tile is in the content");
        assert.strictEqual(oReturnValue.content[0].getMode(), "LineMode", "Generic Tile has the correct mode");
        delete oReturnValue.content; // We can't compare GenericTiles with deepEqual. Since we already asserted the content we can just delete it here
        assert.deepEqual(oExpectedReturnValue, oReturnValue, "The tile model contains the expected data");
    ...
    test("_getTileModel - returns the correct model when a card is provided", function () {
        var oUidStub,
            oGetTileSizeStub,
            oGetTileIdStub,
            oGetCatalogTileIdStub,
        ...
        oGetTileDebugInfoStub = sinon.stub(oLaunchPageService, "getTileDebugInfo").returns("someTileDebugInfo");
        oIsTileIntentSupportedStub = sinon.stub(oLaunchPageService, "isTileIntentSupported").returns(true);
        oGetCardManifestStub = sinon.stub(oLaunchPageService, "getCardManifest", function () {
            return {};
        });
    ...
    test("Verify that _addBookmarkToModel adds bookmark to default group if no target group was specified", function () {
        ...
        fGetTileModelStub = sinon.stub(oHomepageManager, "_getTileModel").returns({});
