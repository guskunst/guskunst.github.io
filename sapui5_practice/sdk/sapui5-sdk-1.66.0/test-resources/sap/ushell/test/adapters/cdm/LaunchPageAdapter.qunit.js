...
sap.ui.require([
...
], function (testUtils, utils, LaunchPageAdapter, oContainer, GenericTile, Link, oUtils, navigationMode, oEventHub, ReadHomePageUtils, oUtilsCdm, fnUriTransform) {
    "use strict";
    /* global assert notStrictEqual Promise asyncTest deepEqual strictEqual module ok sinon start test jQuery sap QUnit */
        ...
        },
        {
            ...
            asyncTest("_ensureGroupTilesResolved works as expected when "
                ...
                function () {
                    ...
                    deepEqual(
                        aActualResult,
                        oFixture.expectedPromiseResult,
        ...
        QUnit.test("_resolveTileByVizId works as expected when " + oFixture.testDescription, function (assert) {
            ...
            var fnDone = assert.async(),
                ...
                if (oFixture.expectedGetInboundArgs) {
                    ...
                    assert.deepEqual(fnGetInbound.firstCall.args[0],
                        oFixture.expectedGetInboundArgs,
                        "expected getInbound to be called with correct arguments");
                }
                if (oFixture.expectedPromiseFailureResult) {
                    oActualResult.done(function () {
                        ...
                        assert.deepEqual(
                            vFailureInfo,
                            oFixture.expectedPromiseFailureResult,
                            "Result is as expected"
                        );
                    });
            }
    ...
    [{
        ...
        output: {
            aExpectedTileIdsInOrder: ["dyna_link_1"]
        }
    }, {
    ...
    [{
        ...
        test("setTileVisible: " + oFixture.testDescription, function () {
            ...
            // assert
            strictEqual(oResolvedTile.visibility, oFixture.expected.cachedVisibilityAfter,
                "visiblity is cached");
            if (oTileComponent && oTileComponent.tileSetVisible) { // tileSetVisible is optional to be implemented
                strictEqual(oTileComponent.tileSetVisible.callCount, bExpectHandlerToBeCalled ? 1 : 0,
                    "tileSetVisible call count");
                if (bExpectHandlerToBeCalled) {
                    strictEqual(oTileComponent.tileSetVisible.firstCall.args[0], oFixture.expected.tileSetVisibleArgument,
                        "tile component notified about visiblity");
                }
            }
    ...
    [{
        ...
        input: {
            ...
            oTileComponent: {
                tileRefresh: sinon.stub()
            }
        },
        expected: {
            tileRefreshCalled: true
        }
    }, {
    ...
    [{
        ...
        output: {
            aExpectedSourceGroupTileIdsInOrder: ["static_tile_1", "static_link_1", "dyna_tile_1"]
        }
    }, {
        ...
        output: {
            aExpectedSourceGroupTileIdsInOrder: ["dyna_tile_1"]
        }
    }, {
    ...
    },
    {
        ...
        output: {
            aExpectedSourceGroupTileIdsInOrder: ["static_tile_0", "static_tile_2", "static_tile_3", "static_tile_4", "static_tile_1"]
        }
    },
    {
        ...
        output: {
            aExpectedSourceGroupTileIdsInOrder: ["static_tile_0", "static_tile_2", "static_tile_3", "static_tile_1", "static_tile_4"]
        }
    },
    {
        ...
        output: {
            aExpectedSourceGroupTileIdsInOrder: ["static_tile_4", "static_tile_0", "static_tile_1", "static_tile_2", "static_tile_3"]
        }
    },
    {
        ...
        output: {
            aExpectedSourceGroupTileIdsInOrder: ["static_tile_0", "static_tile_3", "static_tile_1", "static_tile_2", "static_tile_4"]
        }
    }].forEach(function (oFixture) {
    ...
    },
    {
        ...
        expectedResult: {
            ...
            dataSource: {
                uri: "../component.relative.uri/~12345678910~/test/test.odata.svc/"
            }
        }
    },
    {
    ...
    [
        {
            ...
            input: {
                ...
                oResources: {
                    i18n: {
                        getText: sinon.stub().returns("translatedmode")
                    }
                }
            },
            expected: {
    ...
    [
        ...
        asyncTest("getTileView for tiles: " + oFixture.testDescription, function () {
            var oAdapter = this.oAdapter,
                ...
                oArg = oFixture.expectedCreateComponentCallArgs[0],
                bIsCustomTile = oAdapter.isCustomTile((oArg.ui5ComponentName || oArg.name)),
            ...
            oAdapter.getTileView(oInputTile)
                ...
                .done(function () {
                    ...
                    deepEqual(
                        ...
                        (bIsCustomTile ? "Ui5ComponentLoader#createComponent" : "sap.ui.component") + "was called with the expected arguments"
                    );
                    start();
                });
        });
    ...
    asyncTest("getTileView catches for custom tile throwing an exception", function () {
        ...
        oAdapter.getTileView(oInputTile)
            ...
            .fail(function (sErrorMessage) {
                ...
                ok(
                    (new RegExp(sOriginalError)).test(jQuery.sap.log.error.getCall(0).args[0]),
                    "Rejects its promise when an error is thrown by sap.ui.component"
                );
...
// Need to rework tests as there is no local adapter of the CDM Service and to resolve tile CDM service is needed.
    ...
    test("_getCatalogTileView : Returns correct tile ui", function () {
        ...
        // Arrange
        sinon.stub(oAdapter, "_getTileUiComponentContainerSync").returns({});
            ...
            var aTileActions,
                ...
                oExpectedJsonModelParam = {
                    config: {
                        ...
                        "display_info_text": oFixture.expected.info
                    }
                },
        ...
        },
        {
            ...
            expected: {
                tile: {
                    ...
                    "title": "Bookmark title",
                    "indicatorDataSource": {
                        "path": "/some/service/endpoint/$count"
                    },
                    "target": {
                        url: "https://en.wikipedia.org/wiki/Web_2.0#Web-based_applications_and_desktops"
                    },
            ...
            function addBookmarkToGroup (oGroup) {
                ...
                oPromiseToAddBookmark.done(function () {
                    ...
                    if (oFixture.expected.resolvedTile) {
                        deepEqual(oAdapter._mResolvedTiles[sTileId], oFixture.expected.resolvedTile,
                            "for URL bookmark tile an entry was added to _mResolvedTiles");
                    }
                    strictEqual(oAdapter._mFailedResolvedTiles[sTileId], undefined,
                        "the tile resolution did not fail");
    ...
    [
        {
            ...
            expectedErrorMessage: "Bookmark creation failed because: " +
                "Hash '#UnknownSemanticObject-unknownAction' could not be resolved to a tile. " +
                "stubed CSTR: no resolution result found for '#UnknownSemanticObject-unknownAction'"
        },
        {
            ...
            function addBookmarkToGroup (oGroup) {
                ...
                oAdapter.addBookmark(oFixture.input.oParameters, oGroup)
                    ...
                    .fail(function (sErrorMessage) {
                        ...
                        strictEqual(oAdapter._mFailedResolvedTiles[sTileId], undefined,
                            "do not cache errors for not created tiles");
                        strictEqual(fnSaveSpy.callCount, 0,
                            "Side should not be saved if the target could not be resolved");
                        deepEqual(aOriginalGroupTiles,
                            oAdapter.getGroupTiles(oGroup),
                            "Group must stay untouched as adding failed");
                        start();
                    });
            }
        });
            ...
            assertionsCompleted = [
                ...
                },
                {
                    ...
                    finalState: {
                        ...
                        expectedComponentNotification: {
                            "targetURL": "http://www.sap.com?a=13"
                        }
                    }
                }
                    ...
                    then(function (iUpdatedCount) {
                        ...
                        if (aSetVisualPropertiesSpies.length > 0) {
                            aSetVisualPropertiesSpies.forEach(function (fnSpy) {
                                assert.strictEqual(fnSpy.spy.callCount, 1,
                                    "tileSetVisualProperties called on tile " + fnSpy.tileId);
                                assert.deepEqual(fnSpy.spy.firstCall.args[0],
                                    oFixture.finalState.expectedComponentNotification,
                                    "tileSetVisualProperties arguments for tile " + fnSpy.tileId);
                            });
                        }
                    });
            });
