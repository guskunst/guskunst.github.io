...
sap.ui.require([
    ...
    utils, personalizationUtils, Personalization, PersonalizationContainerVariant,
    PersonalizationContainer, UIComponent, testUtils,
    oPersonalizationTestUtils, AdapterContainerMock
) {
    "use strict";

    /*jslint nomen: true, sub: true*/

    /* global asyncTest strictEqual deepEqual equal module QUnit Promise
      ok parseInt start stop test throws sinon jQuery sap window notDeepEqual
     */
    ...
    var oMock = { // allows to spy on constructor
        AdapterContainerMock: AdapterContainerMock
    };
...
//            }).fail(function () { start() });
    ...
    [
        {
            ...
            oConfig: {
                config: { seed: "1234567890123456789012345678901234567890" }
            },
        ...
        },
        {
            ...
            oConfig: {
                config: { seed: "123456789012345678901234567890123456789" }
            },
    ...
    test("Variant: create a variant with a non-string key", function () {
        var sVariantKey, oUnused;

        sVariantKey = ["0"];
        try {
            oUnused = new PersonalizationContainerVariant(
                sVariantKey, sVARIANT_NAME, {});
            oUnused.hasOwnProperty(""); // eslint
 ...
 // ........... Container Item Tests ...........
    ...
    test("The service is configured with the expected adapters", function () {
        strictEqual(
            this.oService._oAdapterWithBackendAdapter.lazy,
            false,
            "oAdapterWithBackendAdapter is configured to be non-lazy"
        );
        strictEqual(
            Object.prototype.toString.apply(this.oService._oAdapterWithBackendAdapter.instance),
            "[object Object]",
        ...
        strictEqual(
            this.oService._oAdapterWindowOnly.lazy,
            false,
            "oAdapterWindowOnly is configured to be non-lazy"
        );
        strictEqual(
            Object.prototype.toString.apply(this.oService._oAdapterWindowOnly.instance),
            "[object Object]",
        ...
        strictEqual(
            this.oService._oAppVariantAdapterWithBackendAdapter.lazy,
            true,
            "oAppVariantAdapterWithWindowAdapter is configured to be non-lazy"
        );
        strictEqual(
            typeof this.oService._oAppVariantAdapterWithBackendAdapter.create,
            "function",
...
//        oItemValue = undefined; // !!!
    ...
    test("ContextContainer", function () {
        ...
        oUnusedContainer.hasOwnProperty(""); // eslint

        //code under test
        oUnusedContainer = new Personalization.ContextContainer(oService, oAdapter,
            "ContainerKey", oScope, this.oComponent);
        oUnusedContainer = !oUnusedContainer; // because of JSLint complains
    ...
    test("Container: Container constructor with empty key", function () {
        try {
            var oContainer = new Personalization.PersonalizationContainer({}, ""); // oAdapter, sContainerKey
            oContainer.hasOwnProperty(""); // eslint: avoid unused
    ...
    test("Container: Container constructor with non-string key", function () {
        try {
            var oContainer = new PersonalizationContainer({}, {}); // oAdapter, sContainerKey
            oContainer.hasOwnProperty(""); // eslint: avoid unused
            ok(false, "Error: Container with a non-string key was not prohibited");
        } catch (e) {
            //start();
    ...
    test("Variant Set: add variant with an exotic name", function () {
        ...
        try {
            ...
            ok(oVariant instanceof PersonalizationContainerVariant, "addVariant returns a variant object");
        } catch (e) {
            ok(false, "Error: Exception during addVariant");
        }
    });
    ...
    test("Container: Container constructor with empty key", function () {
        try {
            var oContainer = new PersonalizationContainer({}, ""); // oAdapter, sContainerKey
            oContainer.hasOwnProperty(""); // eslint
            ok(false, "Error: Container with an empty key was not prohibited");
        } catch (e) {
            //start();
            ok(true, "Empty sContainerKey led to an exception");
...
//  ............................................................................
    ...
    test("Generated Keys are suitably random in every position 1 to 40", function () {
        ...
        strictEqual(iUnusedCharCount, 0,
            "there were no unused characters from '" + sChars + "' in a sample of "
            + iSampleSize + " random keys");
    });
    ...
    test("all expected functions exist on unmodifiable container", function () {
        var oMockAdapter = {
            ...
            oContainer = new Personalization.ContextContainer(undefined,
