...
], function (appType, WindowAdapter) {
    "use strict";
...
//    });
    ...
    test("AppType display name corresponds to type", function() {
        var oAppTypeRef = appType;

        // Subtitle for those enum values should be equal to the value (string).
        equal(oAppTypeRef.getDisplayName(oAppTypeRef.OVP), oAppTypeRef.OVP, "Subtitle for OVP is `OVP`");
        equal(oAppTypeRef.getDisplayName(oAppTypeRef.SEARCH), oAppTypeRef.SEARCH, "Subtitle for SEARCH is `Search`");
        equal(oAppTypeRef.getDisplayName(oAppTypeRef.FACTSHEET), oAppTypeRef.FACTSHEET, "Subtitle for FACTSHEET is `FactSheet`");
        equal(oAppTypeRef.getDisplayName(oAppTypeRef.COPILOT), oAppTypeRef.COPILOT, "Subtitle for COPILOT is `Co-Pilot`");

        // Subtitle for those enum values should be equal to `App`.
        equal(oAppTypeRef.getDisplayName(oAppTypeRef.APP), "App", "Subtitle for APP should be `App`");
        equal(oAppTypeRef.getDisplayName("None-standard"), "App", "Subtitle for non-standard app type should be `App`");
    ...
    test("Test activity that was not used for 30 days", function (assert) {
        ...
        oUserRecentsService.getFrequentActivity().done(function (aFrequentActivity) {
            ...
            ok(found == false, "The application should not be in the frequently used apps");
            done();
    ...
    test("Test migration of previous data to current data - check backwards compatability", function (assert) {
        ...
        var mockData = [
            { desktop: undefined, tablet: undefined, mobile: undefined, "iCount": 2, "iTimestamp": 1378478828152, "oItem": {"icon": "sap-icon://search","title": "Search application - just to test long text wrapping", "appType": "Search", "appId": "#Action-todefaultapp", url: "#Action-search&/searchterm=Sample%20App", "timestamp": 1378478828152}},
