...
sap.ui.require([
...
], function (AppState, SequentializingAdapter, WindowAdapter, testUtils) {
    "use strict";

    /*jslint nomen: true, sub: true*/
    /*global asyncTest, deepEqual, equal, module, ok, parseInt, QUnit, start,
      stop, test, sinon, jQuery, sap, window, strictEqual */
    ...
    [
        ...
        QUnit.test("Constructor extracts config object, passes original config to window adapter when " + oFixture.testDescription, function (assert) {
            ...
            oAppStateServiceInstance = new AppState(oDummyAdapter, undefined, undefined,
                oFixture.oServiceConfig);
    ...
    asyncTest("constructor, initial keys via promise getAppState  read from window", function () {
        ...
        oService = new AppState(oFakeAdapter, undefined, undefined, { config : {
    ...
    test("test LimitedBuffer", function () {
        var unused = new WindowAdapter(undefined, undefined),
            cut = WindowAdapter.prototype.data,
            i;
    ...
    asyncTest("AppState getAppState fails when no window cache is available and opener is FLP but cannot be loaded from there because WindowAdapter is not reachable and AppState is transient (no BackendAdapter)", function (assert) {
        var oFakeAdapter,
            oService,
            oLogSpy = sinon.spy(jQuery.sap.log, "warning"),
            oInput = {
                sKey: "FooAppState"
            },
            oExpected = {
                sMessage: "Key not found"
            },
            oOriginalOpener = window.opener,
            oFakeOpener = {
                sap: {
