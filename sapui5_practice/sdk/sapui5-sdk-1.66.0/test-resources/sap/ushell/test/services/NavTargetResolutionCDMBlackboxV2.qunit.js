...
sap.ui.require([
     ...
     *
     * @param {object} oSapUshellConfig
     *   The configuration to be added to <code>window["sap-ushell-config"]</code>
     */
    ...
    aResolveHashFragmentFixture.forEach(function (oFixture) {
        asyncTest("resolveHashFragment without sap.ushell.services.AppConfiguration side effects: " + oFixture.sHashFragmentToResolve, function () {
            // When resolveHashFragment is called, we also call
            // sap.ushell.services.AppConfiguration.setCurrentApplication to
            // store the application that was resolved. This is done with the
            // assumption that the application will be opened.
            //
            // In a successive resolution, when the navigation mode is determined, the
            // sap.ushell.services.AppConfiguration.getCurrentApplication method is called
            // and a certain navigation mode is determined. In this test we stub these two
            // methods away, to avoid obtaining different navigation modes when the tests
            // are run in sequence or in order. Another test that checks this should be made
            // explicitly.
            //
            sinon.stub(sap.ushell.services.AppConfiguration, "setCurrentApplication");
            sinon.stub(sap.ushell.services.AppConfiguration, "getCurrentAppliction");
            ...
            sap.ushell.Container.getService("NavTargetResolution").resolveHashFragment(oFixture.sHashFragmentToResolve)
                ...
                .always(function () {
                    ...
                    sap.ushell.services.AppConfiguration.getCurrentAppliction.restore();
                });
    ...
    test("resolveHashFragment with sap.ushell.services.AppConfiguration side effects", function () {
        ...
        var aExpectedNavigationModes = [
            ...
            {   // #Action-toappnavsample
                internal: "newWindowThenEmbedded",  // NOTE: opened in
                                                    // newWindowThenEmbedded mode because the
                                                    // previous app is legacy.
                external: "explace"
            },
            {   // #Action-launchURL
                internal: "newWindow",
                 ...
                 sap.ushell.Container.getService("NavTargetResolution").isIntentSupported(oFixture.aIntents)
                     .done(function (oResult) {
                         ...
                         oFixture.aIntents.forEach(function (sTestIntent) {
                            strictEqual(oResult[sTestIntent].supported, oFixture.expectedSupported[sTestIntent], "got " + oFixture.expectedSupported[sTestIntent] + " for " + sTestIntent);
                         });
                     })
                     .fail(function (sMessage) {
