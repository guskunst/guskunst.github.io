...
sap.ui.require([
    ...
    "sap/ushell/EventHub"
], function (testUtils, oUi5ComponentLoaderUtils, Ui5ComponentHandle, oEventHub) {
    "use strict";
    /*global Promise QUnit sinon start stop */
    /*jslint nomen:true */
            ...
            sap.ushell.Container.getService("Ui5ComponentLoader").createComponent(oFixture.oTargetResolutionResult)
                .done(function (oActualAdjustedTargetResolutionResult) {
                    if (typeof oActualAdjustedTargetResolutionResult === "object") {
                        assert.deepEqual(oActualAdjustedTargetResolutionResult, oFixture.oExpectedAdjustedTargetResolutionResult, "promise resolved with expected result");
                    } else {
                        assert.strictEqual(oActualAdjustedTargetResolutionResult, oFixture.oExpectedAdjustedTargetResolutionResult, "promise resolved with expected result");
                    }
                })
                .fail(function () {
                ...
                .always(function () {
                    assert.ok(!sap.ui.component.load.called, "component factory was never called");
                    fnDone();
                });
        });
     });
                ...
                sap.ushell.Container.getService("Ui5ComponentLoader").createComponent(
                    ...
                    .done(function (oActualAdjustedTargetResolutionResult) {
                        ...
                        if (oFixture.hasOwnProperty("oExpectedAdjustedTargetResolutionResultWithoutComponentHandle")) {
                            ...
                            assert.deepEqual(oActualAdjustedTargetResolutionResult,
                                oFixture.oExpectedAdjustedTargetResolutionResultWithoutComponentHandle, "promise resolved with expected result (adjusted)");
                        }

                        assert.strictEqual(sap.ui.component.callCount, 1, "sap.ui.component called exactly 1 time");
    ...
    QUnit.test("createComponent handles failures of sap.ui.component correctly", function (assert) {
        ...
        sap.ushell.Container.getService("Ui5ComponentLoader").createComponent(
            ...
            .fail(function (oActualError) {
                assert.deepEqual(oActualError, oExpectedError, "error of sap.ui.component reject being passed");
            })
            .always(function () {
                fnDone();
            });
    ...
    QUnit.test("CoreResourcesComplement Loading - Event is listened on", function (assert) {
        ...
        assert.ok(oLoadCoreResourcesComplementStub.calledOnce, "CoreResourcesComplement Loading was triggered after corresponding event was fired");
    });
    ...
    [
        ...
        QUnit.test("CoreResourcesComplement Loading - loadCoreResourcesComplement loads the correct resources when " + oFixture.testDescription + " and NO Bundle is provided in the configuration", function (assert) {
            ...
            // Assert
            assert.deepEqual(oLoadBundleStub.firstCall.args[0], oExpectedBundle, "loadBundle called with correct bundle");
    ...
    [
        ...
        },
        {
            ...
            expectedResources: [
                "some/proper/path/someCustomBundleDebug.js"
            ],
            expectedBundleEventName: "CoreResourcesComplement"
        },
        {
            ...
            expectedResources: [
                "some/proper/path/someCustomBundle.js"
            ],
            expectedBundleEventName: "CoreResourcesComplement"
        },
        {
        ...
        QUnit.test("CoreResourcesComplement Loading - loadCoreResourcesComplement loads the correct resources when " + oFixture.testDescription + " and a Bundle is provided in the configuration", function (assert) {
            ...
            // Assert
            assert.deepEqual(oLoadBundleStub.firstCall.args[0], oExpectedBundle, "loadBundle called wih the correct Bundle");
    ...
    [
        ...
        QUnit.test("CoreResourcesComplement Loading - loadCoreResourcesComplement ", function (assert) {
            ...
            oPromise
                .done(function () {
                    assert.strictEqual(oFixture.shouldSucceed, true, "Promise was resolved");
                    assert.strictEqual(oEmitStub.calledWith("CoreResourcesComplementLoaded", { status: "success" }), true, "Event was listened on");
                    fnDone();
                })
                .fail(function () {
                    assert.strictEqual(oFixture.shouldSucceed, false, "Promise was rejected");
                    assert.strictEqual(oEmitStub.calledWith("CoreResourcesComplementLoaded", { status: "failed" }), true, "Event was listened on");
                    fnDone();
                });
        });
    });
    ...
    QUnit.test("CoreResourcesComplement Loading - loadCoreResourcesComplement returns a Promise and, on failure, tries to load the bundles again after a second call", function (assert) {
        ...
        jQuery.when(oPromise, oSecondPromise)
            .always(function () {
                assert.strictEqual(oPromise.state(), "rejected", "First promise rejected");
                assert.strictEqual(oSecondPromise.state(), "resolved", "Second promise resolved");
                fnDone();
            });
    });
