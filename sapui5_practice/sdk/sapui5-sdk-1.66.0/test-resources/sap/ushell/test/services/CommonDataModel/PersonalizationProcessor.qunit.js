...
function (PersonalizationProcessor) {
    ...
    /*global asyncTest, deepEqual, equal, module, ok, parseInt, QUnit, start,
      assert, stop, sinon, jQuery, sap, strictEqual, test, window */
     ...
     *
     * @param {string} sPath
     *   File name of the test data file which should be requested
     * @returns {object} promise
     *   The jQuery promise's done handler returns the parsed test data object.
    ...
    [
        ...
        asyncTest("_applyRenameGroup: " + oFixture.testDescription, function () {
            ...
            oTestDataPromise.done(function (oTestDataResponse) {
                ...
                start();
                equal(bGroupHasBeenRenamed, oFixture.bGroupHasBeenRenamed, "correct decision taken whether renaming should take place or not");
    ...
    [
        {
            "testDescription": "when the settings of a tile was previously changed,"
            + " another change can yet be made correctly.",
            "oSite": {
                modifiedTiles: {
                    "A_tile_0": {
                        id: "A_tile_0",
        ...
        {
            "testDescription": "when setting of tile was not changed previously, "
            + "the tile setting is correctly changed the first time.",
            "oSite": {
                modifiedTiles: {
                    "A_tile_0": {
                        id: "A_tile_0",
    ...
    // Test data that can be used to test mixin/extract alongside test data in
    // test/js/sap/ushell/cdmLiveSiteData
                ...
                .done(function (oOriginalSiteData, oLiveSiteData, oPersonalizationDeltaData) {
                    oPersonalizationProcessor.extractPersonalization(oLiveSiteData, oOriginalSiteData)
                        .done(function (oPersonalizationDeltaResult) {
                            ...
                            deepEqual(oPersonalizationDeltaResult, oPersonalizationDeltaData, "correct personalization delta calculated");
                        });
                })
                .fail(startBy1);
        });
                ...
                .done(function (oOriginalSiteData, oLiveSiteData, oPersonalizationDeltaData) {
                    ...
                    oPersonalizationProcessor.mixinPersonalization(oOriginalSiteData, oPersonalizationDeltaData)
                        .done(function (oLiveSiteResult) {
                            ...
                            start();
                            // check for correct personalization delta
                            deepEqual(oLiveSiteResult, oLiveSiteData, "correct live site created");

                            // check that all apply methods are called for its respective use case
                            if (oFixture.testDescription === "renameGroup") {
                                ok(oPersonalizationProcessor._applyRenameGroup.called, "_applyRenameGroup called");
                            } else if (oFixture.testDescription === "changeGroupVisibilityChanged") {
                                ok(oPersonalizationProcessor._applyGroupVisibility.called, "_applyGroupVisibility called");
                            } else if (oFixture.testDescription === "removeGroup") {
                                ok(oPersonalizationProcessor._applyRemoveGroup.called, "_applyRemoveGroup called");
                            } else if (oFixture.testDescription === "changeTileSettings" || oFixture.testDescription === "changeLinkSettings") {
                                ok(oPersonalizationProcessor._applyTileSettings.called, "_applyTileSettings called");
                            }
    ...
    /* Method was rewritten and needs completely new tests:
        // Begin of test for method "_checkRemoveGroup":
        [
            {
                "testDescription": "Extracts personalization delta correctly, one group with one tile was removed on personalized object.",
                "oOriginalSite": {
                    "site": {
                        "payload": {
                            "groupsOrder": [
                ...
                "expectedPersonalizationDelta": {
                    ...
                    "removedGroups": ["group2"]
                }
            },
            {
                "testDescription": "Extracts personalization delta correctly, but the delta does contain content in 'removedGroups' and other entries, too.",
                "oOriginalSite": {
                    "site": {
                        "payload": {
                            "groupsOrder": [
                ...
                "oInitialPersonalizationDelta": {
                    ...
                    "removedGroups": ["groupY"]
                },
                "expectedPersonalizationDelta": {
                    "groupOrder": [
                        "group1"
                    ],
                    "removedGroups": ["groupY", "group2"]
                }
            },
            //
            // Invalid Testcase: "Removed group is contained in Array 'removedGroups' already": Group id's are unique (gerd@2016-05-27).
            //
            {
                "testDescription": "Personalization delta unchanged, undefinded original CDM site object.",
                "oOriginalSite": undefined,
                "oPersonalizedSite": {
                    "site": {
                        "payload": {
                            "groupsOrder": [
            ...
            {
                "testDescription": "Personalization delta unchanged, undefinded personalized CDM site object",
                "oOriginalSite": {
                    "site": {
                        "payload": {
                            "groupsOrder": [
            ...
            {
                "testDescription": "Personalization delta unchanged, empty original CDM site object.",
                "oOriginalSite": {},
                "oPersonalizedSite": {
                    "site": {
                        "payload": {
            ...
            {
                "testDescription": "Personalization delta unchanged, empty personalized CDM site object",
                "oOriginalSite": {
                    "site": {
                        "payload": {
                            "groupsOrder": [
            ...
            test("_checkRemoveGroup: " + oFixture.testDescription, function () {
                var oPersonalizationProcessor = new PersonalizationProcessor(),
                    bSuccess;

                sinon.spy(oPersonalizationProcessor, "_checkRemoveGroup");
                bSuccess = oPersonalizationProcessor._checkRemoveGroup(
                        oFixture.oPersonalizedSite,
                        oFixture.oOriginalSite,
                        oFixture.oInitialPersonalizationDelta
                );
                deepEqual(oFixture.oInitialPersonalizationDelta, oFixture.expectedPersonalizationDelta, "correct personalization delta");

                oPersonalizationProcessor._checkRemoveGroup.restore();
            });
        });
    */
    ...
    [
        ...
        test("_addGroupToPersonalizationDelta: Creates correct personalization delta when " + oFixture.testDescription, function () {
            ...
            bSuccess ? ok(bSuccess, "Building delta should be successfully.") : ok(!bSuccess, "Failed intentionally.");
            deepEqual(
                oFixture.oInitialPersonalizationDelta,
                oFixture.expectedPersonalizationDelta,
                "correct personalization delta"
    ...
    [
        // testDescription: completes sentence like "Does something WHEN ...":
        {
            ...
            "oPersonalizedSite": {
                "site": {
                    "payload": {
                        "groupsOrder": ["group2", "group1"]
                    }
                }
            },
            "oInitialPersonalizationDelta": {},
            "expectedPersonalizationDelta": {
                "groupOrder": ["group2", "group1"]
            },
        ...
        test("_checkMoveGroup: Creates correct personalization delta when " + oFixture.testDescription, function () {
            ...
            bSuccess ? ok(bSuccess, "Building delta should be successfully.") : ok(!bSuccess, "Failed intentionally / personalized delta not changed.");
            deepEqual(
                oFixture.oInitialPersonalizationDelta,
                oFixture.expectedPersonalizationDelta,
                "correct personalization delta"
    ...
    [
        ...
        test("_applyMoveGroup: Correctly mixin a changed group order when  " + oFixture.testDescription, function () {
            ...
            bSuccess ? ok(bSuccess, "Mixin new group order successfully.") : ok(!bSuccess, "Failed intentionally / Group order not changed.");
            deepEqual(
                oFixture.oOriginalSite,
                oFixture.expectedChangedOriginalSite,
                "correct personalized Original site"
