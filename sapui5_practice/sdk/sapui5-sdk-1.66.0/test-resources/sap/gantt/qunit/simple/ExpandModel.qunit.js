...
sap.ui.define([
	...
	QUnit.test("expand & collapse single row", function(assert) {
		...
		new Promise(function(resolve){
			window.setTimeout(function(){
				that.sut.expand("break", iExpandIndex);
				sap.ui.getCore().applyChanges();

				var oMainShape = that.getMainShape(iExpandIndex);
				...
				var mExpanded = that.sut._oExpandModel.mExpanded;
				assert.notEqual(mExpanded, null, "mExpanded has values");
				assert.equal(Object.keys(mExpanded).length, 1, "only 1 key exists");

				resolve();
			}, 400);// leave 400 ms to render completely
		}).then(function(){
			return new Promise(function(resolve) {
				window.setTimeout(function(){
					that.sut.collapse("break", iExpandIndex);
					sap.ui.getCore().applyChanges();
					var oMainShape = that.getMainShape(iExpandIndex);
					oMainShape.getBreaks().forEach(function(oBreak){
	...
	QUnit.test("expand & collapse multiple rows", function(assert) {
		...
		new Promise(function(resolve){
			window.setTimeout(function(){
				that.sut.expand("break", aExpandIndex);
				sap.ui.getCore().applyChanges();

				aExpandIndex.forEach(function(iIndex){
					var oMainShape = that.getMainShape(iIndex);
					oMainShape.getBreaks().forEach(function(oBreak){
			...
			return new Promise(function(resolve) {
				window.setTimeout(function(){
					that.sut.collapse("break", [2]);
					sap.ui.getCore().applyChanges();

					var oFirstRowMainShape = that.getMainShape(0);
					...
					var oThirdRowMainShape = that.getMainShape(2);
