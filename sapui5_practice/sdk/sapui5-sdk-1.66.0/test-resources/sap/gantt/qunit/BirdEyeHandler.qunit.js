...
sap.ui.define([
	...
	QUnit.test("Test visible range bird eye", function (assert) {
		...
		setTimeout(function () {
			...
			assert.ok(oCurrentVisibleHorizon.getStartTime() < "20150919000000", "Start time flase.");
			assert.ok(oCurrentVisibleHorizon.getEndTime() > "20151022000000", "End time flase.");
			done();
		}, 1000);
	});
	...
	QUnit.test("Test container bird eye", function (assert) {
		...
		setTimeout(function () {
			...
			var oCurrentVisibleHorizon = oGanttChart1._oGanttChart.getAxisTimeStrategy().getVisibleHorizon();
			assert.ok(oCurrentVisibleHorizon.getStartTime() < "20140920000000", "Start time flase.");
			assert.ok(oCurrentVisibleHorizon.getEndTime() > "20151022000000", "End time flase.");
			done();
		}, 1000);
	});
