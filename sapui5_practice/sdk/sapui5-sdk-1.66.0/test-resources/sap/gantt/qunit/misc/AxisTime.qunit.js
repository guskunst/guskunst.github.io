...
sap.ui.define([
	...
	QUnit.test("test for AxisTime", function (assert) {
		...
		assert.deepEqual(axisX._getTimeIntervalTicks(oTimeInterval, visibleScale), visibleScale.ticks(jQuery.sap.getObject("d3.time.monday").range, 1), "axistime first day of week in different languages.");
		sap.ui.getCore().getConfiguration().setLanguage("zh_CN");
		assert.deepEqual(axisX._getTimeIntervalTicks(oTimeInterval, visibleScale), visibleScale.ticks(jQuery.sap.getObject("d3.time.sunday").range, 1), "axistime first day of week in different languages.");
