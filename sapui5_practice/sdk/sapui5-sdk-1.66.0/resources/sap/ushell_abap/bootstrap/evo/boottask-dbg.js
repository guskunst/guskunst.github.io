...
sap.ui.define([
    ...
    'use strict';
    /*global jQuery, location, Promise, sap, XMLHttpRequest, window */
    ...
    function processDirectStart (oStartupResult, bNoOData) {
        ...
        if (isDirectStart(sHash)) {
            ...
            oStartupHandler.requestDirectStart(oStartupResult, bNoOData, oRequestSegment, oInitialKeys).then(function (oDirectStart) {
                ...
                oAppStateConfig.initialAppStatesPromise = Promise.resolve(oInitialAppStates);

                if (oDirectStart.targetMappings) {
                    oClientTargetResolutionAdapterConfig.initialSegmentPromise = Promise.resolve([aSegment, oDirectStart.targetMappings, oDirectStart.systemAliases]);
                }
            }).catch(function (sReason) {
                oAppStateConfig.initialAppStatesPromise = Promise.reject(sReason);
                oClientTargetResolutionAdapterConfig.initialSegmentPromise = Promise.reject(sReason);
            });
        } else {
