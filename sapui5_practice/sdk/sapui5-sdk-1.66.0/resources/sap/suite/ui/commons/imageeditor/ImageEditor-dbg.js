sap.ui.define([
...
], function(jQuery, Control, library, CoreLibrary, MobileLibrary, Log, Device,
			Icon, ResizeHandler,
			ResizeHistoryItem, RotateHistoryItem, CropRectangleHistoryItem,
			CropEllipseHistoryItem, FilterHistoryItem, FlipHistoryItem, FilterUtils) {
	"use strict";
	...
	var ImageEditor = Control.extend("sap.suite.ui.commons.imageeditor.ImageEditor", {
		metadata: {
			...
			properties: {
				/**
				 * Image source.
				 * <br>Can be a URL address of the image, a Base64 data URL string, or any other source
			...
			events: {
				...
				/**
				 * This event is fired when the zoom level is changed by the user while in the <code>CropRectangle</code>
				 * or <code>CropEllipse</code> mode ({@link sap.suite.ui.commons.ImageEditorMode}).
				 */
				zoomChanged: {
					parameters: {
						zoom: {type: "int"}
	...
	ImageEditor.LIMITS = Object.freeze({
		ZOOM_MIN: 100, ZOOM_MAX: 500,
		WIDTH_MIN: 1, WIDTH_MAX: 8096,
		HEIGHT_MIN: 1, HEIGHT_MAX: 8096,
	...
	ImageEditor.prototype.onBeforeRendering = function() {
	...
	ImageEditor.prototype.onAfterRendering = function() {
		...
		this._sResizeHandlerId = ResizeHandler.register(this, this._refreshContainerSize.bind(this));
	...
	ImageEditor.prototype.exit = function() {
		ResizeHandler.deregister(this._sResizeHandlerId);
	};
	...
	/* Properties												   */
	/* =========================================================== */
	ImageEditor.prototype.setSrc = function(sSrc) {
	...
	ImageEditor.prototype.applyVisibleCrop = function(bPreview) {
		...
		switch (this.getMode()) {
			...
			case ImageEditorMode.CropCustom: {
	...
	ImageEditor.prototype.setCropAreaByRatio = function(iWidth, iHeight) {
		...
		// update based on current zoom
		Object.keys(oImgCoord).forEach(function(sKey) {
			oImgCoord[sKey] = oImgCoord[sKey] * 100 / this._iZoom;
		}, this);
	...
	/**
	 * Zooms in or out to the specified zoom level. Applicable range of values is from 100 to 500.
	 * @param {int} iZoom Level of zoom
	 * @return {sap.suite.ui.commons.imageeditor.ImageEditor} instance of the ImageEditor for method chaining
	 * @public
	 */
	ImageEditor.prototype.zoom = function(iZoom) {
		this._throwIfNotLoaded("setZoom");

		iZoom = this._constraintValue(iZoom, ImageEditor.LIMITS.ZOOM_MIN, ImageEditor.LIMITS.ZOOM_MAX);
		this._setZoom(iZoom);
	 ...
	 * @return {sap.suite.ui.commons.HistoryItem[]} Array of action history items
	 * @public
	 */
	...
	ImageEditor.prototype._onAfterIsReady = function() {
		...
		// done here instead of in the renderer, otherwise chrome doesn't properly refreshed size of the canvas inside inline-block parent
	...
	ImageEditor.prototype._refreshContainerSize = function() {
		if (!this._$Container || !this._$Container[0]) {
			return;
		}

		// remove containers absolute size so that inner canvas can recalculate its size to fit to current image editor size
		...
		// make that container fit to recalculated canvas again
		this._$Container.css({
			width: this._oCanvas.clientWidth,
			height: this._oCanvas.clientHeight
		});
	...
	ImageEditor.prototype._initCanvas = function() {
		...
		this._oCanvas.onmousedown = this._onMouseDown.bind(this);
		this._oCanvas.onmousewheel = this._onMouseWheel.bind(this);
	};
	...
	ImageEditor.prototype._onMouseDown = function(oEvent) {
		...
		if (oEvent.button !== 0 || this._iZoom === 100) {
			return;
		}

		var that = this,
			oCurrentTransform = that._oTransform,
		...
		function onMouseMove(oEvent) {
			var iEndX = oEvent.clientX,
				...
				// convert to percentage of canvas width scaled by current scale
				deltaX = 100 * (iEndX - iStartX) / that._oCanvas.clientWidth / oCurrentTransform.scale,
				deltaY = 100 * (iEndY - iStartY) / that._oCanvas.clientHeight / oCurrentTransform.scale;

			that._oTransform.translateX += deltaX;
			that._oTransform.translateY += deltaY;
			that._updateTransform();
	...
	ImageEditor.prototype._onMouseWheel = function(oEvent) {
		if (!(this.getMode() === ImageEditorMode.CropRectangle || this.getMode() === ImageEditorMode.CropEllipse)) {
			return;
		}
		...
		var iStep = 20,
			iSign = oEvent.wheelDeltaY > 0 ? 1 : -1,
			iZoom = this._iZoom + iStep * iSign;
	...
	ImageEditor.prototype._updateTransform = function() {
		// transformations are in %
		this._setTransform(this._oTransform); // _setTransform called twice so that the boundingClientRect gets changed in case of scale change
		...
		var oCanvasRect = this._oCanvas.getBoundingClientRect(),
			oContainerStyles = getComputedStyle(this._$Container[0]);

		var fXdelta = 100 * (parseFloat(oContainerStyles.width) - oCanvasRect.width) / this._oCanvas.clientWidth / this._oTransform.scale,
			fYdelta = 100 * (parseFloat(oContainerStyles.height) - oCanvasRect.height) / this._oCanvas.clientHeight / this._oTransform.scale,
			iWidthRatio = (oCanvasRect.width <= parseFloat(oContainerStyles.width)) ? 1 : -1,
			iHeightRatio = (oCanvasRect.height <= parseFloat(oContainerStyles.height)) ? 1 : -1;

		this._oTransform.translateX = Math.min(Math.max(this._oTransform.translateX, -fXdelta * iWidthRatio / 2), fXdelta * iWidthRatio / 2);
		this._oTransform.translateY = Math.min(Math.max(this._oTransform.translateY, -fYdelta * iHeightRatio / 2), fYdelta * iHeightRatio / 2);

		this._setTransform(this._oTransform);
	};

	ImageEditor.prototype._setTransform = function(oTranformations) {
		this._oCanvas.style.transform = "scale(" + oTranformations.scale + ") translateX(" + (oTranformations.translateX) + "%) translateY(" + (oTranformations.translateY) + "%)";
	};

	ImageEditor.prototype._limitCropArea = function(iX, iY, iWidth, iHeight) {
		var iMaxWidth = this._getMaxCropAreaWidth(),
			iMinWidth = this._getMinCropAreaWidth(),
			iMaxHeight = this._getMaxCropAreaHeight(),
			iMinHeight = this._getMinCropAreaHeight();

		iX = iX && iX < 0 ? 0 : iX;
		iY = iY && iY < 0 ? 0 : iY;
		iWidth = this._constraintValue(iWidth, iMinWidth, iMaxWidth - iX);
		iHeight = this._constraintValue(iHeight, iMinHeight, iMaxHeight - iY);
	...
	ImageEditor.prototype._getMaxCropAreaWidth = function(bRespectCurrentArea) {
		...
		if (bRespectCurrentArea && this.getKeepCropAspectRatio() && oCropArea && this.getWidth() / this.getHeight() > oCropArea.width / oCropArea.height) {
			...
			iMaxWidth = this.getWidth() * 100 / this._iZoom;
		}
	...
	ImageEditor.prototype._getMinCropAreaWidth = function(bRespectCurrentArea) {
		...
		if (bRespectCurrentArea && this.getKeepCropAspectRatio() && oCropArea && oCropArea.width > oCropArea.height) {
			...
			iMinWidth = this._transposeViewportCoordsToImageCoords(null, null, ImageEditor.LIMITS.CROP_AREA_MIN, null).width;
		}
	...
	ImageEditor.prototype._getMaxCropAreaHeight = function(bRespectCurrentArea) {
		...
		if (bRespectCurrentArea && this.getKeepCropAspectRatio() && oCropArea && this.getWidth() / this.getHeight() < oCropArea.width / oCropArea.height) {
			...
			iMaxHeight = this.getHeight() * 100 / this._iZoom;
		}
	...
	ImageEditor.prototype._getMinCropAreaHeight = function(bRespectCurrentArea) {
		...
		if (bRespectCurrentArea && this.getKeepCropAspectRatio() && oCropArea && oCropArea.width < oCropArea.height) {
			...
			iMinHeight = this._transposeViewportCoordsToImageCoords(null, null, null, ImageEditor.LIMITS.CROP_AREA_MIN).height;
		}
	...
	ImageEditor.prototype._setCropArea = function(fX, fY, fWidth, fHeight) {
		...
		var oPercentArea, $Crop;

		// if only width/height was set with setCropAreaBySize, compute the respective x and y
		if (typeof this._oCropArea.x !== "number") {
			this._oCropArea.x = (this.getWidth() * 100 / this._iZoom - fWidth) / 2;
			fX = this._oCropArea.x;
			this._oCropArea.y = (this.getHeight() * 100 / this._iZoom - fHeight) / 2;
			fY = this._oCropArea.y;
		}

		oPercentArea = this._transposeImageCoordsPxToPercent(fX, fY, fWidth, fHeight);

		// update based on current zoom
		Object.keys(oPercentArea).forEach(function(sKey) {
			oPercentArea[sKey] = oPercentArea[sKey] * this._iZoom / 100;
		}, this);
	...
	ImageEditor.prototype._setZoom = function(iZoom) {
		var iCurrentZoom = this._iZoom;

		this._iZoom = iZoom;
		this._oTransform.scale = this._iZoom / 100;
		this._updateTransform();

		// update crop area to current zoom
		var oCropArea = this.getCropArea();

		if (oCropArea) {
			Object.keys(oCropArea).forEach(function(sKey) {
				oCropArea[sKey] = oCropArea[sKey] * iCurrentZoom / this._iZoom;
			}, this);
	...
	ImageEditor.prototype._setRotation = function(iRotation) {
		...
		this._oContext.translate(-this._oCanvas.width / 2, -this._oCanvas.height / 2);
		this._oContext.drawImage(oLastImg, 0, 0, oLastImg.width, oLastImg.height, Math.round((this._oCanvas.width - oLastImg.width) / 2), Math.round((this._oCanvas.height - oLastImg.height) / 2), oLastImg.width, oLastImg.height);
		// this._oContext.drawImage(this._oOrigImg, 0, 0, fWidth, fHeight, 0, 0, this._oCanvas.width, this._oCanvas.height);
	...
	ImageEditor.prototype._transposeViewPortPxToPercent = function(fX, fY, fWidth, fHeight) {
		var oContainerRect = this.$("canvasInnerContainer")[0].getBoundingClientRect();
	...
	ImageEditor.prototype._getRectangleAreaAvg = function(iX, iY, iWidth, iHeight) {
		...
		var oImageData = this._oContext.getImageData(iX, iY, iWidth, iHeight).data,
			iSum = 0,
			fR, fG, fB, fAlpha;
	...
	ImageEditor.prototype._applyFilterHistory = function(oImageData) {
		...
		for (var i = this._aHistory.length - 1; i >= 0; i--) {
	...
	ImageEditor.prototype._drawImage = function(oSourceImg) {
		this._oCanvas.width = oSourceImg.width;
		this._oCanvas.height = oSourceImg.height;

		this._oContext.drawImage(oSourceImg, 0, 0);
		this._refreshContainerSize();
	};

	ImageEditor.prototype._redraw = function(oSize) {
		var oLastImg = this._getLastImg(),
			iWidth = oSize && oSize.width ? oSize.width : oLastImg.width,
			iHeight = oSize && oSize.height ? oSize.height : oLastImg.height;
