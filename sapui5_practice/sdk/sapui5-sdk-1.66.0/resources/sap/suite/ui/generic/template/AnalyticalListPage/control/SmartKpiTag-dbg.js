sap.ui.define([
	...
	"sap/m/GenericTag",
	"sap/m/ObjectNumber"
], function (Control, KpiTagController, KpiUtil, KpiAnnotationHelper, AnnotationHelper, FilterUtil, V4Terms, ODataModelUtil, KpiTag, KpiProvider, Device, SelectionVariant, Filter, JSONModel, Locale, ResponsivePopover, MessageStrip, NumberFormat, SapMLibrary, SapCoreLibrary, Log, GenericTag, ObjectNumber) {
	"use strict";
	var TARGET = "Target",
		MAXIMIZE = "Maximize",
		MINIMIZE = "Minimize";
	return GenericTag.extend("sap.suite.ui.generic.template.AnalyticalListPage.control.SmartKpiTag", {
		...
		onAfterRendering: function () {
			var oRb = this.getModel("i18n").getResourceBundle();
			var toolTipValue = this.getValue();
			if (toolTipValue === null) {  //setting it for now till the set tooltip api for generic tag is fixed
				return;
			}
			var rightTooltip = (this._isPercent) ? this.getValue().getNumber() + " " + this.getValue().getUnit() : this._unScaledValue + " " + this.getValue().getUnit(),
				KPITooltipKey;
