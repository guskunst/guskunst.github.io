		...
		switch (oTypeMap.type) {
			...
			case "Edm.Time":
				...
				fnGroupingFormatter = function (oValue) {
					if (oValue) { // e.g. {ms: 43980000, __edmType: "Edm.Time"}, interpreted in UTC
			...
			case "Edm.Decimal":
				...
				fnGroupingFormatter = function (oValue, sTextPropertyValue, oContext) {
					var sUnitValue = (sUnit && oContext) ? oContext.getProperty(sUnit) : "";
		...
		if (aGroupedColumns.length) {
			...
			for (var i = 0; i < aGroupedColumns.length; i++) {
				...
				if (oColumn) {
					...
					if (!oColumn.getGroupHeaderFormatter()) { // optimization: set formatter only if is not set
						...
						//Log.info("INFO: setGroupFunctionForAnalyticalTable: Reusing groupHeaderFormatter for " + sProperty + ": " + String(oColumn.getGroupHeaderFormatter()));
					}
				}
			}
		}
