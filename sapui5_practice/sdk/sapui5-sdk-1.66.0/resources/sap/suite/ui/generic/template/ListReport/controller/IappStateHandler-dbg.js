		...
		function fnMapEditableFieldFor(oSelectionVariant){
			...
			oEntityType.property.forEach(function(oProperty){
				if (oProperty["com.sap.vocabularies.Common.v1.EditableFieldFor"]){
					...
					if (oSelectionVariant.getParameter(sKeyProperty) && !oSelectionVariant.getParameter(sForEditProperty)){
						oSelectionVariant.addParameter(sForEditProperty, oSelectionVariant.getParameter(sKeyProperty));
					}
					if (oSelectionVariant.getSelectOption(sKeyProperty) && !oSelectionVariant.getSelectOption(sForEditProperty)){
						var aSelectOption = oSelectionVariant.getSelectOption(sKeyProperty);
						aSelectOption.forEach(function(oSelectOption){
							oSelectionVariant.addSelectOption(sForEditProperty, oSelectOption.Sign, oSelectOption.Option, oSelectOption.Low, oSelectOption.High);
						});
					}
					// and vice versa if field is mapped in FLP (formerly recommended), but original field used in SFB (never recommended)
					if (oSelectionVariant.getParameter(sForEditProperty) && !oSelectionVariant.getParameter(sKeyProperty)){
						oSelectionVariant.addParameter(sKeyProperty, oSelectionVariant.getParameter(sForEditProperty));
					}
					if (oSelectionVariant.getSelectOption(sForEditProperty) && !oSelectionVariant.getSelectOption(sKeyProperty)){
						var aSelectOption = oSelectionVariant.getSelectOption(sForEditProperty);
						aSelectOption.forEach(function(oSelectOption){
							oSelectionVariant.addSelectOption(sKeyProperty, oSelectOption.Sign, oSelectOption.Option, oSelectOption.Low, oSelectOption.High);
						});
					}
				}
			});
