...
sap.ui.define(["sap/suite/ui/generic/template/extensionAPI/UIMode"], function(UIMode) {
	...
	function checkEmbeddedSubSectionExists(oComponent, oFacet){
		if (oFacet) {
				if (oFacet.ID && oFacet.RecordType === "com.sap.vocabularies.UI.v1.CollectionFacet") {
					return oFacet.ID.String === oComponent.leadingSectionIdOrPath;
				} else if (!oFacet.ID && oFacet.RecordType === "com.sap.vocabularies.UI.v1.ReferenceFacet") {
					return oFacet.Target.AnnotationPath === oComponent.leadingSectionIdOrPath;
				}
		} 
		return false;
	}

	function checkParentExsists(oReuseComponent){
		if (oReuseComponent){
				var oModel = oReuseComponent.getModel();
				var leadingComponentContext = {};
				var oReuseComponentObject = oReuseComponent.getObject();
				leadingComponentContext.componentId = oReuseComponentObject.id;
				leadingComponentContext.isGroupedComponent = oReuseComponentObject.leadingSectionIdOrPath ? true : false;
				oModel.setProperty("/leadingComponentContext", leadingComponentContext);
				return "/leadingComponentContext";
		}
	}

	function setComponentSectionTitle(oReuseComponent){
		if (!oReuseComponent.leadingSectionIdOrPath) {
			return oReuseComponent.groupTitle ? oReuseComponent.groupTitle : oReuseComponent.title;
		} else {
			return oReuseComponent.title;
		}
	}

	function checkEmbeddedSectionisLeading(oReuseComponents,oCurrentComponent){
		if (!oCurrentComponent.leadingSectionIdOrPath){
			return true;
		} else { // Incase of wrong configuration the reuse component falls back to be a seperate section 
			for (var i = 0; i < Object.keys(oReuseComponents).length; i++) {
				if (oReuseComponents[Object.keys(oReuseComponents)[i]].id === oCurrentComponent.leadingSectionIdOrPath){
					return oReuseComponents[Object.keys(oReuseComponents)[i]].leadingSectionIdOrPath ? true : false;
				}
			}
	...
	return {
		...
		checkEmbeddedSubSectionExists: checkEmbeddedSubSectionExists,
			
		checkParentExsists: checkParentExsists,

		checkEmbeddedSectionisLeading: checkEmbeddedSectionisLeading,

		setComponentSectionTitle: setComponentSectionTitle
	};
}, /* bExport= */ true);