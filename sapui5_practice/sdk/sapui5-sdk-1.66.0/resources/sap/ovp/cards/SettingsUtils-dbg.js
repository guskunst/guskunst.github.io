sap.ui.define([
        ...
        function getVisibilityOfElement(oCardProperties, sElement, isViewSwitchEnabled, iIndex, sCardType) {
            ...
            switch (sElement) {
                ...
                case "dynamicSubTitle":
                    bVisibilityValue = showSubFields && !!oCardProperties.dynamicSubtitleAnnotationPath;
                    break;
        ...
        function setVisibilityForFormElements(oCardProperties) {
            ...
            this.oVisibility.setAnnotationCardProperties = getVisibilityOfElement(oCardProperties, "setAnnotationCardProperties", isViewSwitchEnabled);        
            ...
            this.oVisibility.setSubTitleValueState = false;
            ...
            this.oVisibility.subTitleValueState = "None";
            this.oVisibility.subTitleValueStateText = "";
        }
    ...
    //	            this.oLinkToTextMapping = oLinkToTextMapping;
        ...
        function validateMandatoryFields(oEvent) {
            var sFieldName = oEvent.getParameter("path"), sTitle = "",
                ...
                iCounterError = oMessagesModel.getProperty("/Counter/Error");
                var oVisibilityModel = this.dialogBox.getContent()[0].getModel("visibility");
                //This will validate all input fields when there is any entity or KPI checkBox selection change
            if (sFieldName === "/entitySet" || sFieldName === "/addKPIHeaderCheckBox") {
                var oData = this.dialogBox.getContent()[0].getModel().getData();
                if (oData.entitySet) {
                
                    this.oMandatoryInputFields.forEach( function(inputField) {                
                        bIsErrorExist = false;
                        if (inputField === "/title") {
                            if (oData.title === "") {
                                bIsErrorExist = true;  
                        ...
                        /** Check if KPI checkbox is selected */
                            if (oData.addKPIHeaderCheckBox) {
                                if (inputField === "/subTitle") {
                                    if (oData.subTitle === "") {
                                        bIsErrorExist = true;
                                        sTitle = OvpResources.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_SUBTITLE"); 
                                        oVisibilityModel.setProperty("/subTitleValueState", "Error");
                                        oVisibilityModel.setProperty("/subTitleValueStateText", OvpResources.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_SUBTITLE"));

                                    }
                                }
                                if (inputField === "/valueSelectionInfo") {
                                    if (oData.valueSelectionInfo === "") {
                                        bIsErrorExist = true;  
                                        sTitle = OvpResources.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_SELECTIONVALUEINFO");
                                    }
                                }
                            } else { //If checkBox is unselected then check if error exist for subtitle and valueselection info then remove it
                                for (i = 0; i < aMessages.length; i++) {
                                    if (aMessages[i].fieldName === "/subTitle" || aMessages[i].fieldName === "/valueSelectionInfo") {
                        ...
                        for (i = 0; i < aMessages.length; i++) {
                            if (aMessages[i].fieldName === inputField) {
                                bIsErrorExist = false;
                            }
                        }
                        if (bIsErrorExist) {
                ...
                } else if (sFieldName === "/title" || sFieldName === "/subTitle" || sFieldName === "/valueSelectionInfo") {
                    //When there is s change in input fields like title, subtitle and value selection info then this part will be executed
                        var sNewValue = oEvent.getParameter("value"),
                        bIsErrorExist = catchInputFieldError(sNewValue, sFieldName);
                        // Error Message for Card Title
                        if (sFieldName.indexOf("/title") !== -1) {
                            sTitle = OvpResources.getText("OVP_KEYUSER_INPUT_ERROR");
                        }

                        // Error Message for Card subTitle
                        //If subtitle is not mandatory then no need to show error message
                        if (sFieldName.indexOf("/subTitle") !== -1) {
                            if (oVisibilityModel.getData().requiredSubTitle) {
                                sTitle = OvpResources.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_SUBTITLE");   
                                bIsErrorExist = true;
                            } else {
                                bIsErrorExist = false;       
                        ...
                        // Error Message for Card Value selection info
                        if (sFieldName.indexOf("/valueSelectionInfo") !== -1) {
                            sTitle = OvpResources.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_SELECTIONVALUEINFO");
        ...
        var oSettingsUtils = {
            ...
            oMandatoryInputFields: SettingsConstants._oMandatoryInputFields,
            getDialogBox: function (oComponentContainer) {
                return new Promise(function (resolve, reject) {
                    ...
                    if (!this.bAddNewCardFlag) {
                        ...
                        oCardPropertiesModel.attachPropertyChange(validateMandatoryFields.bind(this));
                    }
