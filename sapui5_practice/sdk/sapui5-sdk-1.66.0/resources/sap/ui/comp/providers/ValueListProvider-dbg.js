...
sap.ui.define([
			...
			this._fValidator = function(oData) {
				...
				if (oRow) {
					...
					if (this.oControl.__sValidationText !== sInput) {
						...
						var sInputTruncated = this._truncateSearchText(sInput);
						if (sInput === sInputTruncated) {
							...
							// invalid entry if the sInput.length > maxLength.
						}
					} else {
	...
	ValueListProvider.prototype._truncateSearchText = function(sSearchText) {
		//because the Field itself allow to enter many characters, but the fieldMetadata has set a maxLength, we truncate the SearchText when we reach the maxLength
		var iMaxLength = -1;
