 ...
 */
 sap.ui.define(["sap/ui/mdc/ResourceModel", "sap/base/Log", "./StableIdHelper"], function(ResourceModel, Log, StableIdHelper) {
	"use strict";
	var Helper = {
		getStableId: StableIdHelper.generate,
		getTargetCollection: function(oContext) {
			var sPath = oContext.getPath();
			if (oContext.getObject('$kind') === 'EntitySet') {
				return sPath;
			}
			var aParts = sPath.split('/');
			//sPath can be of format '/<entitySet>/<navigationCollection>' <OR> '/<entitySet>/$Type/<navigationCollection>'
			//Both are supported.
			return ('/' + aParts[1] + '/$NavigationPropertyBinding/' + aParts[aParts.length - 1]);
		},
		...
		gotoTargetEntitySet: function(oContext) {
			var sPath = oContext.getPath().replace(/\$Type/g, "$NavigationPropertyBinding");
			return sPath + '/$';
		},
		gotoActionParameter: function(oContext) {
			var sPath = oContext.getPath(),
				sPropertyName = oContext.getObject(sPath + "/$Name"),
				aContext = sPath.split("@$ui5.overload");
