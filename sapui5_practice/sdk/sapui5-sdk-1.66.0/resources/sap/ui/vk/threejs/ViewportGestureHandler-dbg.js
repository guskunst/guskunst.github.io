...
sap.ui.define([
	"jquery.sap.global", "sap/ui/base/EventProvider", "./PerspectiveCamera", "./OrthographicCamera", "./thirdparty/three"
], function(jQuery, EventProvider, PerspectiveCamera, OrthographicCamera, threeJs) {
	"use strict";
			...
			var ThreeJsCameraController = function(vp) {
				...
				this.beginGesture = function(x, y) {
					...
					if (hit) {
						...
						*/
						...
						sceneRef._expandBoundingBox(boundingBox, true);
				...
				this.zoom = function(zoomFactorScaler) {
					...
					if (camera.isPerspectiveCamera) {
						...
						jQuery.sap.log.error(sap.ui.vk.getResourceBundle().getText("VIEWPORTGESTUREHANDLER_MSG_UNSUPPORTEDCAMERATYPE"));
					}
				...
				this.zoomObject = function(nodeRef, isIn, timeInterval) {
					...
					var boundingBox = new THREE.Box3();
					(isIn && nodeRef ? nodeRef : viewport.getScene().getSceneRef())._expandBoundingBox(boundingBox, true);
