		...
		var load = function() {
			...
			that._loadContentResources(scene, contentResources).then(
				function(values) { // onFulfilled
					...
					for (var n = 0; n < values.length; n++) {
						if (values[n].camera) {
							...
							content.camera = values[n].camera;
							break;
						}
					}
					...
					for (var i = 0; i < values.length; i++) {
						if (values[i].loader) {
							...
							content.loaders.push(values[i].loader);
						}
						if (values[i].builder) {
							content.builders = content.builders || [];
							content.builders.push(values[i].builder);
						}
					}

					if (content.loaders) {
						that._initSceneWithCDSLoaderIfExists(scene, content.loaders);
					}
				...
				function(reason) { // onRejected
					jQuery.sap.log.error(sap.ui.vk.getResourceBundle().getText("CONTENTMANAGER_MSG_CONTENTRESOURCESFAILEDTOLOAD"), reason);
					that.fireContentChangesFinished({
						content: null,
						failureReason: [
							{
