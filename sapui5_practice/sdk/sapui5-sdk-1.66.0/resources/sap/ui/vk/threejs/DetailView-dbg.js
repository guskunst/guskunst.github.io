...
sap.ui.define([
	...
	var DetailView = BaseObject.extend("sap.ui.vk.threejs.DetailView", /** @lends sap.ui.vk.threejs.DetailView.prototype */ {
		metadata: {
			properties: {
				...
				type: {
					...
					defaultValue: sap.ui.vk.DetailViewType.DetailView
				},
				shape: {
					type: "sap.ui.vk.DetailViewShape",
					defaultValue: sap.ui.vk.DetailViewShape.Box
				},
				borderWidth: {
					type: "float",
					defaultValue: 2
		...
		constructor: function(sId, mSettings, oScope) {
			...
			this._billboard = new THREE.Mesh(new THREE.BufferGeometry(), material);
			...
			this._node.isDetailView = true;

			this._node.traverse(function(node) {
				if (node.geometry !== undefined) {
					node.geometry.boundingBox = new THREE.Box3(); // set empty boinding box
				}
				node.matrixAutoUpdate = false;
			});

			this.setRenderOrder(this.getRenderOrder()); // update meshes render order
		}
	});

	DetailView.prototype.setCamera = function(value) {
		if (value instanceof sap.ui.vk.threejs.PerspectiveCamera || value instanceof sap.ui.vk.threejs.OrthographicCamera) {
	...
	DetailView.prototype.setRenderOrder = function(value) {
		this.setProperty("renderOrder", value, true);
		if (this._billboard) {
			this._billboard.renderOrder = value;
			this._border.renderOrder = value + 0.1;
			this._triangle.renderOrder = value - 0.1;
			this._line.renderOrder = value - 0.1;
		}
		return this;
	};
	...
	DetailView.prototype._createCircleBorder = function(vpPoints, rx, ry, attachmentPoint, shape) {
		...
		if (attachmentPoint && (shape !== sap.ui.vk.DetailViewShape.Circle && shape !== sap.ui.vk.DetailViewShape.CircleLine)) {
			...
			this._triangle.material.color.set(shape === sap.ui.vk.DetailViewShape.SolidPointer || shape === sap.ui.vk.DetailViewShape.SolidArrow ? this.getBorderColor() : this._backgroundColor);
		} else {
	...
	DetailView.prototype._updateGeometry = function(size, viewportSize, camera) {
		...
		this._border.visible = shape !== sap.ui.vk.DetailViewShape.BoxNoOutline;
		this._line.visible = shape === sap.ui.vk.DetailViewShape.BoxLine || shape === sap.ui.vk.DetailViewShape.CircleLine;
		...
		switch (shape) {
			default:
			case sap.ui.vk.DetailViewShape.Box:
			case sap.ui.vk.DetailViewShape.BoxLine:
			case sap.ui.vk.DetailViewShape.BoxNoOutline:
				this._createBoxViewport(size.x, size.y, attachmentPoint ? aPoint : null, shape);
				break;

			case sap.ui.vk.DetailViewShape.Circle:
			case sap.ui.vk.DetailViewShape.CircleLine:
			case sap.ui.vk.DetailViewShape.CirclePointer:
			case sap.ui.vk.DetailViewShape.CircleArrow:
			case sap.ui.vk.DetailViewShape.CircleBubbles:
			case sap.ui.vk.DetailViewShape.SolidPointer:
			case sap.ui.vk.DetailViewShape.SolidArrow:
	...
	DetailView.prototype._update = function(renderer, camera, scene, boundingBox) {
		...
		if (this.getType() === sap.ui.vk.DetailViewType.Cutaway) {
			mat4.copy(camera.projectionMatrix);
			setCutawayProjectionMatrix(camera, position, size, viewportSize);
		} else {
			camera = this.getCamera();
			camera.adjustClipPlanes(boundingBox);
			camera.update(size.x, size.y);
			camera = camera.getCameraRef();
		}
		...
		scene.children.forEach(function(root) {
			if (root.userData._vkCallouts) {
				root.userData._vkCallouts.forEach(function(object) {
					object._update(renderer, camera, size);
				});
			}
		});

		renderer.setClearColor(this._backgroundColor, 1);
		renderer.render(scene, camera, this._renderTarget, true);
		if (this.getType() === sap.ui.vk.DetailViewType.Cutaway) {
