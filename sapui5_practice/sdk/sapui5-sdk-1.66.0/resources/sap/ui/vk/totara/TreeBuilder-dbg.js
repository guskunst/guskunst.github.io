			...
			for (i = 0; i < treeNodes.length; i++) {
				...
				if (treeNode && treeNode.children) {
					if (treeNode.entityId != null && treeNode.children.length === 1) {
						// Skip single root element, this one should not be displayed in the tree
						...
						if (rootElementNode.entityId == null) {
							...
							if (rootElementNode.meshId) {
								treeNode.meshId = rootElementNode.meshId;
							}
							if (rootElementNode.materialId) {
								treeNode.materialId = rootElementNode.materialId;
							}
							if (rootElementNode.transform) {
								treeNode.transform = rootElementNode.transform;
							}
							treeNode.children = rootElementNode.children;
							treeNodes[ rootElementNodeIndex ] = null;
						}
					}
