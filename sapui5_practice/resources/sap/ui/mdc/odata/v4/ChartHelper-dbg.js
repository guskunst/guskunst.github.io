...
// ---------------------------------------------------------------------------------------
// ---------------------------------------------------------------------------------------
sap.ui.define(["sap/ui/model/json/JSONModel", "./ODataMetaModelUtil"
	...
	/**
	 * Helper class for sap.ui.mdc.odata.v4 Chart phantom control for prepecosseing.
	 * <h3><b>Note:</b></h3>
	 ...
	 * here we shall use sap.ui.mdc.odata.v4.ChartHelper that is especially tailored for V4
	 * meta model
	 *
	 * @author SAP SE
	 * @private
	 * @experimental
	 * @since 1.62
	 * @alias sap.ui.mdc.odata.v4.ChartHelper
	 */
	var ChartHelper = {};
	...
	ChartHelper.formatDimensions = function(oAnnotationContext) {
		...
		for (i = 0; i < oAnnotation.Dimensions.length; i++) {
			...
			oDimension.criticality =
				util.fetchCriticality(oMetaModel,
					oMetaModel.createBindingContext(sEntitySetPath + sKey)).then(formatJSONToString);
	...
	ChartHelper.formatMeasures = function(oAnnotationContext) {
		var oAnnotation = oAnnotationContext.getObject(""),
			oMetaModel = oAnnotationContext.getModel(),
			sEntitySetPath = getEntitySetPath(oAnnotationContext),
			aMeasures = [], i, j, sDataPoint, oDataPoint;

		//retrieve aggregation information from entity set
		var aAnalytics = oMetaModel.getObject(sEntitySetPath
				+ "@com.sap.vocabularies.Analytics.v1.AggregatedProperties") || [], mAnalytics = {};
