	...
	function(
		...
		function getModelFilterObject(oCondition, sFieldPath) {
			...
			if (oOperator.valueTypes.length == 1) {
				return new Filter({ path: sFieldPath, operator: oOperator.filterOperator, value1: oCondition.values[0] });
			} else {
				return new Filter({ path: sFieldPath, operator: oOperator.filterOperator, value1: oCondition.values[0], value2: oCondition.values[1] });
			}
		}
