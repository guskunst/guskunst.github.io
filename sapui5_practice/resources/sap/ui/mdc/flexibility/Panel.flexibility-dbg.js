...
sap.ui.define([
	'sap/ui/fl/changeHandler/Base', 'sap/ui/core/util/reflection/JsControlTreeModifier'
], function(Base, JsControlTreeModifier) {
	"use strict";
	...
	return {
		createChanges: function(oPanel, oAppComponent, aDeltaMItems) {
			...
			return aNotExistingItems.reduce(function(aResult, oDeltaMItem) {
				...
				var oSelector = JsControlTreeModifier.getSelector(oDeltaMItem.id, oAppComponent);
				if (!oSelector.idIsLocal) {
					throw new Error("Invalid 'idIsLocal'. The id " + oDeltaMItem.id + " should contain the app component id");
				}
				return {
					...
					changeSpecificData: {
						...
						content: {
							// Make sure that the component id is not persisted
							selector: oSelector
						}
					}
				};
