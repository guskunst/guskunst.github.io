...
sap.ui.define([
	...
	"./ViewGallery", "./Viewport", "./NativeViewport", "./RedlineDesign", "./ViewerRenderer"
], function(
	...
	Viewer.prototype._instantiateStepNavigation = function() {
		...
		if (this._mainScene instanceof sap.ui.vk.dvl.Scene) {
			this._stepNavigation = new StepNavigation({
				showThumbnails: this.getShowStepNavigationThumbnails(),
				contentConnector: this._contentConnector
			});
		} else if (this._mainScene instanceof sap.ui.vk.threejs.Scene) {
	...
	Viewer.prototype.activateRedlineDesign = function(redlineElements) {
		var vp = this.getViewport().getImplementation();
		if (vp instanceof sap.ui.vk.threejs.Viewport && vp.getCamera().getCameraRef().type === "PerspectiveCamera") {
			vp._viewportGestureHandler._activateRedline();
		}
		if (vp instanceof sap.ui.vk.dvl.Viewport && vp.getViewInfo().camera && vp.getViewInfo().camera.projectionType === "perspective") {
			vp._activateRedline();
		}
		redlineElements = redlineElements || [];
		...
		} else if (vp instanceof sap.ui.vk.threejs.Viewport && vp.getVisible()) {
	...
	Viewer.prototype.destroyRedlineDesign = function() {
		if (this.getRedlineDesign()) {
			var vp = this.getViewport().getImplementation();
			if (vp instanceof sap.ui.vk.dvl.Viewport) {
				vp._deactivateRedline();
			} else if (vp instanceof sap.ui.vk.threejs.Viewport) {
