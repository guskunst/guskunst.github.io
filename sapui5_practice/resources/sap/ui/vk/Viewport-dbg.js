...
sap.ui.define([
	...
	"./ContentConnector", "./ViewStateManager", "./ViewportRenderer"
], function(
	jQuery, ViewportBase,
	ContentConnector, ViewStateManager, ViewportRenderer
) {
	...
	Viewport.prototype.getRenderMode = function() {
		...
		return sap.ui.vk.RenderMode.Default;
	};
	...
	Viewport.prototype.playProcedure = function(modelViews, viewIndex, enableAnimations) {
		if (this._implementation && this._implementation.playProcedure) {
			this._implementation.playProcedure(modelViews, viewIndex, enableAnimations);
		}
	};
	...
	Viewport.prototype.toggleAnimationPauseStatus = function() {
		if (this._implementation && this._implementation.toggleAnimationPauseStatus) {
			return this._implementation.toggleAnimationPauseStatus();
		}
	};
			...
			if (!reuseImplemenation) {
				...
				if (newImplementationType) {
					...
					this._implementation = new (jQuery.sap.getObject(newImplementationType))({
						...
						animationTimeSlider: this.getAssociation("animationTimeSlider"),
						...
						nodesPicked: function(event) {
							that.fireNodesPicked({
								picked: event.getParameter("picked")
							});
						},
						nodeZoomed: function(event) {
							that.fireNodeZoomed({
						...
						viewActivated: function(event) {
							that.fireViewActivated({
								viewIndex: event.getParameter("viewIndex"),
								type: event.getParameter("type")
							});
						},
						viewFinished: function(event) {
							that.fireViewFinished({
								viewIndex: event.getParameter("viewIndex")
							});
						},
						procedureFinished: function(event) {
							that.fireProcedureFinished();
						},
	...
	Viewport.prototype.activateView = function(view, animationsEnabled) {
		if (this._implementation) {
			this._implementation.activateView(view, animationsEnabled);
			return this;
		} else {
	...
	/**
	 * Set animation time slider
	 *
	 * @param {sap.ui.vk.AnimationTimeSlider} slider Set animation time control.
	 * @returns {void}
	 * @private
	 */
	Viewport.prototype.setAnimationTimeSlider = function(slider) {
		this.setAssociation("animationTimeSlider", slider, true);

		if (this._implementation && this._implementation.setAnimationTimeSlider) {
			this._implementation.setAnimationTimeSlider(slider);
		}
	};

	/**
	 * Get animation time slider
	 *
	 * @returns {sap.ui.vk.AnimationTimeSlider} Animation time control.
	 * @private
	 */
	Viewport.prototype.getAnimationTimeSlider = function() {
		if (this._implementation && this._implementation.getAnimationTimeSlider) {
			return this._implementation.getAnimationTimeSlider();
		}

		return this.getAssociation("animationTimeSlider");
	};
