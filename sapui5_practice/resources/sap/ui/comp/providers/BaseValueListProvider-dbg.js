					...
					if (sValueListFieldName !== this.sKey) {
						...
						while (i--) {
							...
							if (oData[sValueListFieldName]) {
								...
								// if (oFieldViewMetadata.type === "Edm.DateTime" && this.oFilterProvider._oDateFormatSettings.UTC == true) {
								if (oFieldViewMetadata && oFieldViewMetadata.type === "Edm.DateTime" && this._oDateFormatSettings.UTC == true) {
									// update the time to 00:00:00 GMT +-x
									oNewData.key = new Date(oNewData.key.valueOf() + oNewData.key.getTimezoneOffset() * 60 * 1000);
								}
