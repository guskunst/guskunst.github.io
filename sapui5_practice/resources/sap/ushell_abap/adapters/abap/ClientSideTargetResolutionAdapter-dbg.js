   ...
   var ClientSideTargetResolutionAdapter = function (oSystem, sParameters, oAdapterConfig) {
    ...
    ClientSideTargetResolutionAdapter.prototype._formatDirectStart = function (oDirectStartResponse) {
        ...
        function mapOne(sSrcId, oSrc) {
            ...
            /*
             * Keep the systemAliasSemantics to "apply" for Shell-WCF URLs,
             * because these URLs, like other URLs are expressed under this
             * assumption (that the server does not already interpolate system
             * alias data). At least on ABAP. This fix is necessary because the
             * default behavior for the WCF URL resolver was changed in
             * ClientSideTargetResolution after support for WCF application
             * types was introduced.
             */
            var oResolutionResult = oTarget.resolutionResult;
            var sOriginalApplicationType = oResolutionResult.applicationType;
