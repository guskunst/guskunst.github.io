sap.ui.define([ "jquery.sap.global", "sap/ui/core/mvc/Controller",
    ...
    return Controller.extend("sap.ovp.cards.rta.SettingsDialog", {
        ...
        /*validate DataPoint ComboBox field
         */
        validateComboBox: function(oCardManifestSettings) {
            ...
            if (oCardManifestSettings.addKPIHeaderCheckBox && !oCardManifestSettings.dataPointAnnotationPath) {    
                var sTitle = OvpResources.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_DATAPOINT");
                this.bError = true;
                var bIsError = true;
                for (var i = 0; i < aMessages.length; i++) {
                    if (aMessages[i].fieldName === OVPUtils.Annotations.dataPoint) {
                        bIsError = false;
                    }
                }
                if (bIsError) {
                    aMessages.push({
                                "type": "Error",
                                "title": sTitle,
                                "fieldName": OVPUtils.Annotations.dataPoint,
                                "counter": iCounterError + 1
                            });
                    iCounterAll++;
                    iCounterError++;
                    this.getView().byId("sapOvpDataPoint").setValueState("Error");
                }
            } else {
                this.bError = false;
                for (var i = 0; i < aMessages.length; i++) {
                ...
                this.getView().byId("sapOvpDataPoint").setValueState("None");
            }
        ...
        onSaveButtonPress : function() {
            if (this._oCardManifestSettings.addNewCard ) {
                this.bError = false;
                this.validateComboBox.bind(this)(this._oCardManifestSettings); 
            }
            var aStaticParameters = this._oCardManifestSettings.aAllStaticParameters;
            if (aStaticParameters) {
                var oStaticParameters = {};
                for (var i = 0; i < aStaticParameters.length; i++) {
                    oStaticParameters[aStaticParameters[i].key] = aStaticParameters[i].value;
                }
                this._oCardManifestSettings.staticParameters = oStaticParameters;
            }
...
//            this.getView().byId("dialogCard").setBusy(bBusy);
        ...
        EnableKPIHeaderFields: function(bSelectedValue) {
            ...
            if (!bSelectedValue) {
                ...
                oVisibilityModel.setProperty("/subTitleValueState", "None");
                oVisibilityModel.setProperty("/subTitleValueStateText", "");
            } else {
                this.setMandatoryFieldState(true, oVisibilityModel);
                if (this.getView().byId("sapOvpCardSubTitle").getValue() === "") {
                    oVisibilityModel.setProperty("/subTitleValueState", "Error");
                    oVisibilityModel.setProperty("/subTitleValueStateText", this.oOvpResourceBundle.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_SUBTITLE"));
                }
            }
        },
        /* 
	...
	*/
        onDatasourceComboboxChanged: function(oEvent) {
            ...
            for (var i = 0; i < aDataSources.length; i++) {
                    if (aDataSources[i].name === this._oCardManifestSettings.model) {
                ...
                if (sSourceElementId.indexOf(this._updateManifestProperties[i].formElementId) > -1) {
                    ...
                    switch (sFormElement){
                        ...
                        case "dataPointAnnotationPath":
                            ...
                            this.validateComboBox(this._oCardManifestSettings); 
                            break; 
                        ...
                        case "navigationPath":
                            ...
                            break;
                        case "title":
                            var sTitleInput = oEvent.getParameter("value");
                            if (sTitleInput.trim().length) {
                                this.setEnablePropertyForResetAndSaveButton(true);
                            } else {
                                this.setEnablePropertyForResetAndSaveButton(false);
                            }
                        ...
                        case "subTitle":
                            ...
                            if (oData.requiredSubTitle && sTitleInput === "") {
                                oVisibilityModel.setProperty("/subTitleValueState", "Error");
                                oVisibilityModel.setProperty("/subTitleValueStateText", this.oOvpResourceBundle.getText("OVP_KEYUSER_INPUT_ERROR_VALUE_STATE_SUBTITLE"));
                            } else {
                                oVisibilityModel.setProperty("/subTitleValueState", "None");
       ...
       resetPropertiesVisibility : function(ElementId , oCardProperies ){
           //Reset all the fields except Data source dropdown
            if (ElementId.indexOf("sapOVPDataSource") !== -1) {
                this._updateManifestProperties.forEach(function (oProperty) {
                    if (!ElementId.includes(oProperty.formElementId)) {
