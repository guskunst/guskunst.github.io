sap.ui.define([
	...
	"./ImageEditorContainerRenderer" // renderer has to be imported in every control now
			...
			OverflowToolbarToggleButton, OverflowToolbarLayoutData, ToggleButton, ToolbarSeparator, Label, Slider, Input, CheckBox,
			JSONModel, MenuButton, SegmentedButton, SegmentedButtonItem, Select, Button, Item, ResponsivePopover, List,
			StandardListItem, CustomListItem, FlexBox, FlexItemData, IconTabBar, IconTabFilter, Icon, HTML) {
	"use strict";
	...
	var InputType = MobileLibrary.InputType,
		...
		mNumberInputHandlers = Object.freeze({
			ZOOM: {min: ImageEditor.LIMITS.ZOOM_MIN, max: ImageEditor.LIMITS.ZOOM_MAX, callback: "_setZoom"},
		...
		mIcons = Object.freeze({
			...
			CROPCUSTOM: "sap-icon://add-favorite",
	...
	ImageEditorContainer.prototype.destroyImageEditor = function() {
		...
		this._oModel.setProperty("/blocked", true);
	};
	 ...
	 *
	 * @return {{filter: string, transform: string, undo: string, history: string, separator: string, crop: string}} Ids of the default items in the header toolbar
	 * @public
	 */
	...
	ImageEditorContainer.prototype._onImageLoaded = function() {
		...
		this._oModel.setProperty("/blocked", !oImageEditor.getSrc());
		...
		this._oModel.setProperty("/crop/zoom", 100);
	...
	ImageEditorContainer.prototype._onZoomChange = function(oEvent) {
		...
		this._oModel.setProperty("/crop/zoom", iZoom);
	...
	ImageEditorContainer.prototype._getHeaderToolbar = function() {
		if (!this._headerToolbar) {
			this._headerToolbar = new OverflowToolbar({
				blocked: "{/blocked}"
			}).addStyleClass("sapSuiteUiCommonsImageEditorContainerToolbar");
			this._headerToolbar.setModel(this._oModel);
			this.addDependent(this._headerToolbar);
		}

		return this._headerToolbar;
	};

	ImageEditorContainer.prototype._initHeaderToolbar = function() {
	...
	ImageEditorContainer.prototype._getUndoButton = function() {
		...
		if (!this._oHistoryButton) {
			this._oHistoryButton = new Button({
				id: this.getId() + "-undoButton",
	...
	ImageEditorContainer.prototype._getHistoryArrowButton = function() {
		...
		if (!this._oHistoryArrowButton) {
			this._oHistoryArrowButton = new Button({
				id: this.getId() + "-historyButton",
				type: ButtonType.Transparent,
	...
	ImageEditorContainer.prototype._getTransformButton = function() {
		if (!this._oTransformButton) {
			this._oTransformButton = new OverflowToolbarToggleButton({
				id: this.getId() + "-transformButton",
	...
	ImageEditorContainer.prototype._getCropButton = function() {
		if (!this._oCropButton) {
			this._oCropButton = new OverflowToolbarToggleButton({
				id: this.getId() + "-cropButton",
				icon: mIcons.CROP,
				type: ButtonType.Transparent,
				pressed: {
					path: "/mode",
	...
	ImageEditorContainer.prototype._getFilterButton = function() {
		if (!this._oFilterButton) {
			this._oFilterButton = new OverflowToolbarToggleButton({
				id: this.getId() + "-filterButton",
				icon: mIcons.FILTER,
				type: ButtonType.Transparent,
				pressed: {
					path: "/mode",
	...
	ImageEditorContainer.prototype._changeOptionsPanelContent = function(fnGetPanelContent) {
		var oOptionsPanel = this._getOptionsPanel(),
			oPanelContent = fnGetPanelContent();

		oOptionsPanel.removeAllItems();
		oPanelContent.forEach(function(oContent) {
	...
	ImageEditorContainer.prototype._resetValueStates = function() {
		var aPaths = [
			"/transform/widthValueState", "/transform/heightValueState", "/transform/rotateValueState",
			"/crop/zoomValueState", "/crop/widthValueState", "/crop/heightValueState", "/filter/filterValueState"
		];
	...
	ImageEditorContainer.prototype._createGridList = function(sPath, fnFactory, fnSelectChange) {
		...
		if (!oList._oItemNavigation) { // TODO why can _oItemNavigation not exist
	...
	ImageEditorContainer.prototype._blockCrop = function() {
		this._oModel.setProperty("/crop/blocked", true);
	};

	ImageEditorContainer.prototype._unblockCrop = function() {
		if (this._oModel.getProperty("/crop/blocked")) {
			this._oModel.setProperty("/crop/blocked", false);
		}
	};
	...
	ImageEditorContainer.prototype._onRectCropGridListSelectionChange = function(oEvent) {
		...
		this.getImageEditor().setMode(ImageEditorMode.CropRectangle);
	...
	ImageEditorContainer.prototype._onEllipseCropGridListSelectionChange = function(oEvent) {
		...
		this.getImageEditor().setMode(ImageEditorMode.CropEllipse);
				...
				new FlexBox({
					...
					items: [
									...
									items: [
										...
										new Input({
											...
											change: function(oEvent) {
												var iCurrentWidth = that._oModel.getProperty("/crop/width"),
													iCurrentHeight = that._oModel.getProperty("/crop/height"),
													bKeepAspectRatio = that._getKeepCropAspectRatio(),
													iWidth = parseInt(oEvent.getParameter("value"), 10),
													iHeight, iMaxWidth, iMinWidth;

												that._resetValueStates();

												iWidth = isNaN(iWidth) ? 0 : iWidth;
												iHeight = bKeepAspectRatio ? iWidth * iCurrentHeight / iCurrentWidth : iCurrentHeight;
												iMaxWidth = that.getImageEditor()._getMaxCropAreaWidth(iWidth, iHeight);
												iMinWidth = that.getImageEditor()._getMinCropAreaWidth(iWidth, iHeight);

												if (iWidth > iMaxWidth || iWidth < iMinWidth) {
													this.setValue(iCurrentWidth); // force back last correct value
													that._oModel.setProperty("/crop/widthValueState", ValueState.Warning);
													that._oModel.setProperty("/crop/widthValueStateText",
														oResourceBundle.getText("IMGEDITOR_BAD_NUMBER", [iMinWidth, iMaxWidth]));
												} else {
													that._setCropAreaBySize(iWidth, iHeight);
												}
											}
										})
									]
										...
										new Input({
											...
											change: function(oEvent) {
												var iCurrentWidth = that._oModel.getProperty("/crop/width"),
													iCurrentHeight = that._oModel.getProperty("/crop/height"),
													bKeepAspectRatio = that._getKeepCropAspectRatio(),
													iHeight = parseInt(oEvent.getParameter("value"), 10),
													iWidth, iMaxHeight, iMinHeight;

												that._resetValueStates();

												iHeight = isNaN(iHeight) ? 0 : iHeight;
												iWidth = bKeepAspectRatio ? iHeight * iCurrentWidth / iCurrentHeight : iCurrentWidth;
												iMaxHeight = that.getImageEditor()._getMaxCropAreaHeight(true);
												iMinHeight = that.getImageEditor()._getMinCropAreaHeight(true);

												if (iHeight > iMaxHeight || iHeight < iMinHeight) {
													this.setValue(iCurrentHeight); // force back last correct value
													that._oModel.setProperty("/crop/heightValueState", ValueState.Warning);
													that._oModel.setProperty("/crop/heightValueStateText",
														oResourceBundle.getText("IMGEDITOR_BAD_NUMBER", [iMinHeight, iMaxHeight]));
												} else {
													that._setCropAreaBySize(iWidth, iHeight);
												}
											}
						...
						new FlexBox({
							...
							items: [
								new Label({
									id: this.getId() + "-cropZoom",
									text: oResourceBundle.getText("IMGEDITOR_CROP_SIZE")
								}),
								new Input({
									type: InputType.Number,
									textAlign: TextAlign.Right,
									...
									fieldWidth: "4.375rem",
									description: "%",
									change: this._onNumberInputChange.bind(this, mNumberInputHandlers.ZOOM),
									value: {
										path: "/crop/zoom", type: "sap.ui.model.type.Integer", mode: "OneWay"
									},
									valueState: "{/crop/zoomValueState}",
									valueStateText: "{/crop/zoomValueStateText}",
									ariaLabelledBy: [this.getId() + "-cropZoom"]
								})
							]
						}),
						new Slider({
							min: ImageEditor.LIMITS.ZOOM_MIN,
							max: ImageEditor.LIMITS.ZOOM_MAX,
							value: {path: "/crop/zoom", mode: "OneWay"},
							liveChange: this._onNumberInputChange.bind(this, mNumberInputHandlers.ZOOM),
							progress: true,
							ariaLabelledBy: [this.getId() + "-cropZoom"]
						}),
