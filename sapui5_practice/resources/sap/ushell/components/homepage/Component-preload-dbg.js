...
sap.ui.define(["sap/ushell/Config"], function (Config) {
    ...
    */
    ActionMode.prototype._openActionsMenu = function (oEvent, oView) {
        ...
        // NOTE - when removing this check (according to requirements by PO) - we must disable the tileSettings action in a different way
    ...
    ComponentKeysHandler.prototype = {
        ...
        goToLastVisitedTile: function (jqTileContainerToLookUnder, bLookInGivenGroup) {
            var jqDefaultContent,
                ...
                jqAllContent = jQuery(".sapUshellTile, .sapFCard, .sapMGTLineMode");
            if (bLookInGivenGroup) {
                ...
                var jqTileContainerContent = jqTileContainer.find(".sapUshellTile, .sapFCard, .sapMGTLineMode");
        ...
        _goToSiblingElementInTileContainer: function (sDirection, jqFocused) {
            ...
            if (jqTileContainerElement.length) {
                if (sDirection === 'up') {
                    ...
                    jqFirstTileInTileContainer = jqTileContainer.find(".sapUshellTile, .sapFCard, .sapUshellLinkTile").eq(0);
        ...
        setTileFocus: function (jqTile) {
            ...
            var jqAllContent = jQuery(".sapUshellTile, .sapFCard, .sapMGTLineMode, .sapUshellLinkTile, .sapMGT").filter("[tabindex=0]").get();
...
sap.ui.define([
    ...
    sap.ui.controller("sap.ushell.components.homepage.DashboardContent", {
        ...
        _resizeHandler: function () {
            ...
            if (sap.ushell.Layout && jQuery("#dashboardGroups").is(":visible")) {
            ...
            if (oViewGroups) {
                jQuery.each(oViewGroups, function (nIndex, oGroup) {
                    if (oGroup.getGroupId() === sGroupId) {
                        ...
                        ComponentKeysHandler.setTileFocus(oGroup.$().find("li").eq(0));
            ...
            if (this.oView.oDashboardGroupsBox.getGroups()) {
                ...
                jQuery.each(this.oView.oDashboardGroupsBox.getGroups(), function (nIndex, oGroup) {
                    if (oGroup.getGroupId() === sGroupId) {
                        var iY;
                        setTimeout(function () {
                            var oGroupElement = oGroup.getDomRef();
                            var oDashboardGroupsElement = sap.ui.getCore().byId("dashboardGroups").getDomRef();

                            iY = oGroupElement.getBoundingClientRect().top - oDashboardGroupsElement.getBoundingClientRect().top;
                            var groupHeaderHeight = oGroup.$().find(".sapUshellTileContainerHeader").height();
                            var groupBeforeContentHeight = oGroup.$().find(".sapUshellTileContainerBeforeContent").height();
                            ...
                            var iAnchorNavigationBarBottom = that.oView.oAnchorNavigationBar.getDomRef().getBoundingClientRect().bottom;
                            var iShellHeaderBottom = sap.ui.getCore().byId("shell-header").getDomRef().getBoundingClientRect().bottom;
                            var iAnchorNavigationBarHeight = iAnchorNavigationBarBottom - iShellHeaderBottom;

                            iY = groupHeaderHeight > 0 && !bIsActionsModeActive ? iY + iAnchorNavigationBarHeight : iY + groupBeforeContentHeight + 8;
...
//                    sap.ushell.Container.getRenderer('fiori2').showRightFloatingContainer(false);
        ...
        _onAfterDashboardShow : function (oEvent) {
            ...
            var aJqTileContainers = jQuery(".sapUshellTileContainer").filter(":visible"),
                oViewPortContainer = sap.ui.getCore().byId('viewPortContainer'),
            ...
            if (bIsInDashboard) {
                if (!bTileActionsModeActive) {
                    ...
                    var iTopGroupInViewPortIndex = this.oModel.getProperty('/topGroupInViewPortIndex'),
                        ...
                        jqFirstTile = jqTopGroupInViewPort.find(".sapUshellTile").eq(0),
                        jqElementToFocus;
