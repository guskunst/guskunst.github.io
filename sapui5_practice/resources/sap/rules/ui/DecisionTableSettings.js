...
sap.ui.define([
    ...
    function (jQuery, library, Control, SimpleForm, Label, Switch, Select, MessageBox, Table, Text, CheckBox, Input, Button,
        ExpressionAdvanced, VerticalLayout, ExpressionType, Constants, AstExpressionBasic, AstExpressionLanguage) {
        "use strict";
        ...
        sap.rules.ui.DecisionTableSettings.prototype._getASTExpressionBasicForPredefined = function (oContext, expressionID, expression,
            ...
            var astExpressionBasic = new AstExpressionBasic({
                ...
                change: function (oEvent) {
                    ...
                    var modifiedNodes = that._getASTNodes(astNodes, oContext);
                    model.setProperty(sPath + "/Result/ASTNodes", modifiedNodes);
                    that.getModel("settingsModel").setProperty("/resultAttributeChanged", true);
                    that._updateResultAttributeJSON(oContext, null, null, modifiedNodes);
                    oSource._validateControl();
                }.bind(this)
            }).setBindingContext(oContext);
        ...
        sap.rules.ui.DecisionTableSettings.prototype._getFixedOperatorDataForExpression = function (sExpression, bValueOnly, astControl) {
            ...
            if (this.getExpressionLanguage() && fixOperatorEnabled) {
                ...
                if (oSuggestions) {
                    if (oSuggestions.arithmetic && oSuggestions.arithmetic.length > 0) {
                        var arithmeticOperators = oSuggestions.arithmetic;
                        for (var i = 0; i < arithmeticOperators.length; i++) {
                            fixedOperatorData.fixOperatorEnumration.push({
                                key: arithmeticOperators[i].name,
                                value: arithmeticOperators[i].label
                            });
                        }
                    }
        ...
        sap.rules.ui.DecisionTableSettings.prototype._getPredefinedExpressionAdvanced = function (oContext, expressionID, expression,
            ...
            return new ExpressionAdvanced({
                ...
                change: function (oEvent) {
                    ...
                    this._updateResultAttributeJSON(oContext, sAccessMode, sExpression, null);
                }.bind(this)
            });
        };
        ...
        sap.rules.ui.DecisionTableSettings.prototype._predefinedResultsFactory = function (sId, oContext) {
            ...
            var astNodes = [];
           var label = oContext.getProperty("Result/DataObjectAttributeLabel") ? oContext.getProperty("Result/DataObjectAttributeLabel") :
				oContext.getProperty("Label");
			var name = oContext.getProperty("Result/DataObjectAttributeName") ? oContext.getProperty("Result/DataObjectAttributeName") :
				oContext.getProperty("Name");
			var displayText = label ? label : name;
			var attributeId = oContext.getProperty("Result/DataObjectAttributeId") ? oContext.getProperty("Result/DataObjectAttributeId") :
				oContext.getProperty("Id");
            var expression;
            var businessDataType = oContext.getProperty("Result/BusinessDataType");
            var sSelectedKey;
            ...
            if (this._internalModel.getProperty("/resultDataObjectChanged")) {
                this._updateResultAttributeJSON(oContext, Constants.EDITABLE, "", []);
                ...
                this._updateResultAttributeJSON(oContext, sSelectedKey, expression, astNodes);
            } else if (type === "RESULT") {
                expression = oContext.getProperty("Result/Expression");
                astNodes = oContext.getProperty("Result/DecisionTableColumnResultASTs/results");
                sSelectedKey = oContext.getProperty("Result/AccessMode");
                this._updateResultAttributeJSON(oContext, sSelectedKey, expression, astNodes);
            } else {
        ...
        sap.rules.ui.DecisionTableSettings.prototype._setColumnAccessMode = function (oContext, oEvent) {
            ...
            if (element instanceof ExpressionAdvanced) {
                if (SelectedMode === Constants.KEY_HIDDEN) {
                    ...
                    this._updateResultAttributeJSON(oContext, this.hiddenModeText, "", []);

                } else {
                    element.setValueStateText("");
                    element.addStyleClass("sapRULExpressionAdvancedResultTable");
                    this._updateResultAttributeJSON(oContext, this.editableModeText, null, null);
                }
            } else {
                if (SelectedMode === Constants.KEY_HIDDEN) {
		    element._input[0].classList.add("sapAstExpressionInputError");
                    this._updateResultAttributeJSON(oContext, this.hiddenModeText, null, null);
                } else {
        ...
        sap.rules.ui.DecisionTableSettings.prototype._setDefaultResult = function () {
            ...
            if (resultsEnumration.length === 2) {
				...
				modelData.ResultDataObjectLabel = resultsEnumration[1].label;
                modelData.ResultDataObjectStatus = "C";
            }
        };
                ...
                cells: [
                    ...
                    new sap.m.Select({
                        ...
                        change: function (oEvent) {
                            var oSelect = oEvent.getSource();
                            var model = oContext.getModel();
                            model.setProperty(oContext.getPath() + "/Condition/FixedOperator", oSelect.getSelectedKey());
                            //Mark column as "Updated"
                            this._changeColumnStatusToUpdate(oContext);
                        }.bind(this)
                    }),
        ...
        sap.rules.ui.DecisionTableSettings.prototype._updateResultAttributeJSON = function (
            oContext, sAccessMode, sExpression, astNodes) {
            var bResultChanged = this._internalModel.getProperty("/resultDataObjectChanged");
            var isRefreshed = this._internalModel.getProperty("/refreshButtonClicked");
            var sAttributeId = oContext.getProperty("Result/DataObjectAttributeId") ? oContext.getProperty("Result/DataObjectAttributeId") :
                oContext.getProperty("Id");
            ...
            if (this._internalModel.getProperty("/predefinedResults")) {
                if (this._internalModel.getProperty(sDataPath)) {
                    ...
                    if (!bResultChanged && sAccessMode) {
                        this._internalModel.setProperty(sDataPath + "/AccessMode", sAccessMode);
                        this._internalModel.setProperty(sDataPath + "/updatePredefinedResults", true);
                    }
                    if (!bResultChanged && (sExpression || sExpression === "")) {
                        this._internalModel.setProperty(sDataPath + "/Expression", sExpression);
                        this._internalModel.setProperty(sDataPath + "/updatePredefinedResults", true);
                    }
                    if (!bResultChanged && astNodes) {
                        this._internalModel.setProperty(sDataPath + "/ASTNodes", astNodes);
                        this._internalModel.setProperty(sDataPath + "/updatePredefinedResults", true);
                    }
                } else {
        ...
        sap.rules.ui.DecisionTableSettings.prototype._applySettingsModelChangesToOData = function (oDialog) {
                ...
                if (newResultJsonObjects) {
                    ...
                    for (attribute in newResultJsonObjects) {
                        if ((attribute.AttributeId != null || attribute.AttributeId !== "") && newResultJsonObjects[attribute].updatePredefinedResults) {
			                ...
			                if (newResultJsonObjects[attribute].ASTNodes) {
								var updateResultPath = "/DecisionTableColumnResults(RuleId='" + sRuleId + "',Version='" + sVersion + "',Id=" +
									newResultJsonObjects[attribute].Id + ")";
								var resultBody = {
								...
								_removeExistingAstNodes(newResultJsonObjects[attribute].Id, "/DeleteDTColResultASTDraft");
								_updateModelAstNodes(newResultJsonObjects[attribute].ASTNodes, "/DecisionTableColumnResultASTs");
							}
            ...
            /////////////////////////////////////////////astnode methods////////////////////////////////////////////////////////////
            var _updateModelAstNodes = function (astNodes, sPath) {
                for (var node in astNodes) {
                    var mParameters = {};
					if(astNodes[node].Type === "I") {
                        ...
                        if (settingsModelColumn.Status === "U") {
                            ...
                            if (settingsModelColumn.Condition) {
                                ...
                                if (that.getAstExpressionLanguage() && modelData[i].Condition.ASTNodes) {
                                    ...
                                    _updateModelAstNodes(modelData[i].Condition.ASTNodes, "/DecisionTableColumnConditionASTs");
                                }
                            }
			...
			var isResultChanged = this._internalModel.getProperty("/resultDataObjectChanged");
			var isResultAttributeParameterChanged = this._internalModel.getProperty("/resultAttributeChanged");
			...
			_updateODataModelColumns(aSettingsModelColumns, isResultChanged, modelData);
			...
			if (!isResultChanged && isResultAttributeParameterChanged) {
				_updateModelPredefinedResultAttributes(oModel.oData.ResultDataObjectId);
