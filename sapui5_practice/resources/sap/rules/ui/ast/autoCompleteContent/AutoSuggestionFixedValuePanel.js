sap.ui.define([
	...
	var autoSuggestionFixedValuePanel = Control.extend("sap.rules.ui.ast.autoCompleteContent.AutoSuggestionFixedValuePanel", {
		...
		_getStructureObjectForTableType: function(attributeId, valueHelpInfo){
			var valueSources = valueHelpInfo.expressionLanguage._data.ValueSources;
			var valueHelpData = valueSources ? valueSources.results: [];
			for(var entry in valueHelpData){
				if(attributeId === valueHelpData[entry].AttributeId){
					return valueHelpData[entry].DataObjectId;
				}
			}
