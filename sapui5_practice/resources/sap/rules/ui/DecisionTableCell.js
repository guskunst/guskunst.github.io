...
sap.ui.define([
    ...
    function (jQuery, library, DecisionTableCellExpressionAdvanced, DecisionTableCellFormatter, DecisionTableCellExpressionBasic, Popover,
        Constants, ConstantUtils, AstExpressionLanguage) {
        "use strict";
                ...
                if (!this.getTypePath()) {
                    if (this.getAstExpressionLanguage()) {
						...
						if (oInputControl.relString) {
							if (oInputControl.relString.includes(oExpressionEditor.markerString)) {
	    ...
	    var sFixedOperator = this.getModel().getProperty(this.getFixedOperatorPath());
			...
			if (that.getAstExpressionLanguage()) {
				var astExpression = that._getExpressionFromAstNodes(oCellContext);
				if (astExpression && bCondition) {
					if (fillerString !== "" && astExpression.includes(fillerString)) {
						return astExpression.split(fillerString)[1];
					} else if (astExpression.startsWith(sFixedOperator + " ")) {
						return astExpression.split(sFixedOperator + " ")[1];
					} else {
        ...
        DecisionTableCell.prototype._getExpressionFromAstNodes = function (oContext) {
            var that = this;
            var oAstExpressionLanguage = sap.ui.getCore().byId(this.getAstExpressionLanguage());
            ...
            if (astList && astList.__list && astList.__list.length > 0) {
                ...
                for (var term in termsArray) {
                    ...
                    if (termsArray[term].startsWith("./") || termsArray[term].startsWith("/") && !termsArray[term].includes(Constants.MARKER_STRING)) {
                    ...
                    if (referenceExpression !== "") {
						...
						if (termsArray[term].includes("/" + Constants.MARKER_STRING)) {
                            termsArray[term] = termsArray[term].replace("/" + Constants.MARKER_STRING, "");
                        }
						if (this.getAggregation("_displayedControl").getValueState() === "Error" && termsArray[term].includes("====")) {
                        ...
                        if(aFunction.includes(splitArray[0]) && len < jsonLength){
		...
		var oComparisonOperators = ["=", "!=", ">", ">=", "<", "<="];
		var oFunctionOperators = ["IN", "NOTIN", "EXISTSIN", "NOTEXISTSIN", "MATCHES", "NOTMATCHES",
			"CONTAINS", "NOTCONTAINS", "STARTSWITH",
			"NOTSTARTSWITH", "ENDSWITH", "NOTENDSWITH", "ISWITHIN", "ISNOTWITHIN"
		];
		if (oComparisonOperators.indexOf(lastString) > -1) {
			markerString = "/" + Constants.MARKER_STRING + " ==== ";
		} else if (oFunctionOperators.indexOf(lastString) > -1) {
			markerString = "/" + Constants.MARKER_STRING + " " + lastString + " ";
		} else {
			markerString = "/" + Constants.MARKER_STRING + " ";
		}
		return markerString;
            ...
            var fnAfterPopOverClose = function (oEvent) {
                if (this._oPopover) {
					...
					if (this.getAstExpressionLanguage()) {
                        ...
                        oDecisionTableCellExpression.setValue(this._oAstExpressionLanguage._astUtil.toAstExpressionString(astNodes));
                    }
